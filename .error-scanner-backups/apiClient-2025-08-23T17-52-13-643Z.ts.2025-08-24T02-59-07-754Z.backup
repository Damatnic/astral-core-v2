...
// Custom API Error }
 ...
 AstralCoreApiError extends Error { status?: number;}
  code?: string;
  details?: any,
  timestamp: string
...
const data = await this.parseResponse<T>(response );
      ...
      return { data,
        ...
        headers: response.headers} catch (error) { clearTimeout(timeoutId);
      this.abortControllers.delete(requestId );
...
const errorMessage = error instanceof Error ? error.message :;
    ...
    return new AstralCoreApiError({ message: errorMessage,
      code: "UNKNOWN_ERROR',""'""'"'
      details: error,
      timestamp: new Date().toISOString()});
