...
const ServiceWorkerUpdate: React.FC = () =} {,
{
    ...
    isOfflineReady,
    skipWaiting,
    cacheStatus } = useServiceWorker();
const [showDetails, setShowDetails] = useState(false);
const [isUpdating, setIsUpdating] = useState(false);

  if (!updateAvailable && !isOfflineReady) { return null };
const handleUpdate = async () =} { setIsUpdating(true);
    await skipWaiting();
    // The page will reload automatically
  return(<div className="sw-update-container'>"")''"""'
