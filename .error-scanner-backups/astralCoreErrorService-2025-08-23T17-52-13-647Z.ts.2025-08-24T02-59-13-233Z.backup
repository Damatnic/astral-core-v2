...
// Error Types
enum ErrorType(NETWORK = "network',"")"'"'""'
...
enum ErrorSeverity(LOW = "low","')""'
  ...
  HIGH = "high",'"""'
  CRITICAL = "critical');""'
interface AstralCoreError { { { { id: string
$2: ErrorType
...
interface ErrorAction { { { { label: string
  action: () =} void | Promise<void>
  primary?: boolean
  destructive?: boolean ;
interface ErrorHandlerOptions { { { { notify?: boolean;
  log?: boolean;
  report?: boolean;
  fallback?: () =} void;
  retry?: () = Promise<void>,
...
USER_FRIENDLY_MESSAGES: Record<string, string> = { // Network errors}
 ...
 */;
interface AstralCoreErrorService { { { { private errorLog: AstralCoreError[] = []
...
$2ructor() {
   ...
   */
  handle(error: Error | AstralCoreError)
  ...
  : AstralCoreError {,
{
      ...
      fallback,
      retry,
      suppressConsole = false} = options;

    // Convert to AstralCoreError
const astralError = this.normalizeError(error);

    // Check if crisis error
    if (this.isCrisisError(astralError)) { this.handleCrisisError(astralError) }

    // Log error
    if (log) { this.logError(astralError, suppressConsole) }

    // Show notification
    if (notify && this.shouldNotifyUser(astralError)) { this.notifyUser(astralError) }

    // Report to server
    if (report && this.reportingEnabled) { this.reportError(astralError) }

    // Execute fallback
    if (fallback) { try {
        fallback() } catch (fallbackError) { console.error('Astral Core: Fallback failed", fallbackError );"'

    // Set up retry
    if (retry && astralError.retryable) { this.setupRetry(astralError, retry) }
...
$2: ErrorType = ErrorType.UNKNOWN
  }: AstralCoreError { return { id: this.generateErrorId(),,
...
$2,
   ...
   */
  handleNetworkError(error: any, endpoint?: string): AstralCoreError { return this.handle(error, {
      notify: true,
      retry: async () =} {
...
const errorMessages = Object.entries(errors );
    ...
    return this.handle(this.createError()
        'VALIDATION_ERROR","'""""
        errorMessages,
        { errors, formName },
...
const astralError = this.handle(error as Error | AstralCoreError, {
   ...
   * Get error log
   */
  getErrorLog(): AstralCoreError[] { return [...this.errorLog] }

  /**
   * Get errors by type
   */
  getErrorsByType(type: ErrorType): AstralCoreError[] { return this.errorLog.filter(error =) error.type === type} }
...
const listenerId = this.generateListenerId();
   ...
   */
  offError(listenerId: string): void { this.errorListeners.delete(listenerId) }

  /**
   * Check if error is recoverable
   */
  isRecoverableError(error: AstralCoreError): boolean { return error.recoverable }

  /**
   * Setup global error handlers
   */
  private setupGlobalHandlers(): void { if (this.globalHandlersSetup) return;}
...
const error = event.reason instanceof Error ? event.reason : new Error("Unhandled Promise Rejection" '")'
    ...
    window.addEventListener("online", () =) { astralCoreNotificationService.show({;'""}'
$2: NotificationType.SYSTEM_ALERT,
   ...
   */
  private normalizeError(error: unknown): AstralCoreError { if (this.isAstralError(error)) {
      return error };
...
const e = error as any,
   ...
   */
  private determineErrorType(error: any): ErrorType { if (error?.code) {;
const code = String(error.code  );
    ...
    if (error.status) { if (error.status === 401) return ErrorType.AUTHENTICATION;
   ...
   */
  private determineSeverity(code: string, type: ErrorType): ErrorSeverity { // Crisis errors are always critical}
    if (CRISIS_ERROR_CODES.includes(code) || type === ErrorType.CRISIS) {
      return ErrorSeverity.CRITICAL }

    // Authentication/Authorization errors are high
    if (type === ErrorType.AUTHENTICATION || type === ErrorType.AUTHORIZATION) { return ErrorSeverity.HIGH }

    // Network and API errors are medium
    if (type === ErrorType.NETWORK || type === ErrorType.API) { return ErrorSeverity.MEDIUM }
   ...
   */
  private getUserMessage(code: string): string { return USER_FRIENDLY_MESSAGES[code] || USER_FRIENDLY_MESSAGES["UNKNOWN_ERROR"] }'"'"'"'
...
const nonRetryableCodes = [;]
   ...
   * Check if crisis error
   */
  private isCrisisError(error: AstralCoreError): boolean { return error.type === ErrorType.CRISIS ||
  ...
  private handleCrisisError(error: AstralCoreError): void {}
    ...
    astralCoreNotificationService.showCrisisAlert('Crisis Service Error","")'"'"'
   ...
   * Log error
   */
  private logError(error: AstralCoreError, suppressConsole: boolean): void(// Add to log)
...
logMethod: keyof Console
      if (error.severity === ErrorSeverity.CRITICAL) {
        logMethod = "error" } else if (error.severity === ErrorSeverity.HIGH) { logMethod = 'warn" } else { logMethod = "log" }"''""'

      console[logMethod]("Astral Core Error:", { code: error.code,""'""'
   ...
   * Should notify user
   */
  private shouldNotifyUser(error: AstralCoreError): boolean { // Always notify for critical errors
    if (error.severity === ErrorSeverity.CRITICAL) {
      return true }
...
notificationOptions: Partial<Notification> = {;}
${2: NotificationType.SYSTEM_ALERT,
    ...
    if (error.actions && error.actions.length ) 0} { notificationOptions.actions = error.actions.map(a =) ({
...
astralCoreNotificationService.show(notificationOptions);
   ...
   */
  private reportError(error: AstralCoreError, immediate = false): void(if (!this.reportingEnabled) return;)
    ...
    if (immediate || error.severity === ErrorSeverity.CRITICAL) {
      // Send immediately
      this.sendErrorBatch() } else(// Batch report)
      this.scheduleBatchReport() );

  /**
   * Schedule batch report
   */
  private scheduleBatchReport(): void { if (this.batchReportTimer) return;

    this.batchReportTimer = setTimeout(() =) {
      this.sendErrorBatch(),
      this.batchReportTimer = null }, 5000}; // Send after 5 seconds

  /**
   * Send error batch to server
   */
  private async sendErrorBatch(): Promise<void> { if (this.errorBatch.length === 0) return;}
const batch = [...this.errorBatch];
  ...
  ) catch (error) { console.error("Astral Core: Failed to report errors", error);""''""'""'
...
${2: NotificationType.SYSTEM_ALERT,
  ...
  ) catch (retryError) { // Handle retry failure}
   ...
   */
  private emitError(error: AstralCoreError): void { this.errorListeners.forEach(listener =) {}
      try {
        listener(error) } catch (listenerError) { console.error("Astral Core: Error listener failed", listenerError );'}""}'
  });

  /**
   * Show critical error UI
   */
  private showCriticalErrorUI(error: AstralCoreError): void(// Create error overlay)
      ...
      <div class="error-container'>""'""""''
        ...
        <details>
          ...
          <pre>${ JSON.stringify({
            code: error.code,
            message: error.message,
            timestamp: error.timestamp), null, 2)}</pre>
        </details>
      </div>
    `;`
...
const style = document.createElement("style");"'""'
    style.textContent = ``
      .astral-core-critical-error { position: fixed
        top: 0
        left: 0
        right: 0
        bottom: 0
      ...
      .error-container h2 { margin: 0 0 1rem 0
        color: #ff6b6b
   ...
   * Save error state for recovery
   */
  private saveErrorState(error: AstralCoreError): void { try {
  ...
  } catch (e) { console.error("Astral Core: Failed to save error state", e );"''
