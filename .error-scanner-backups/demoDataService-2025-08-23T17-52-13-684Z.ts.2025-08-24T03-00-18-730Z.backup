...
import { Dilemma,
    ...
    HelpSession,
    ModerationAction,
    CommunityStats  } from "../types';""""'
class DemoDataService { private static instance: DemoDataService}
...
getStarkeeperAssessments(userToken: string): Assessment[] { return []
            {
                id: "assessment-001',"""'"'""'
                userToken,;
...
getHelperSessions(): HelpSession[] { return []}
            {
                id: "session-001",'"'"'""'
                dilemmaId: "dilemma-001",'"'"'""'
    ...
    getInterconnectedWorkflowData(): any {}
        return {
            ...
            aiDetectionResponse: {
                ...
                automatedActions: [ 'Crisis specialist immediately notified","'""""'"'
                    "Post prioritized in helper queue',"""'"'""'
                    "Safety resources automatically shared",""''import "Admin alerted for potential escalation" ],'""""'
            ...
            helperIntervention: {
                ...
                interventionActions: [ "Immediate safety assessment conducted',""''""'
                    "Crisis resources provided","'""'
                    'Safety plan development initiated",""'"'"'
                    "Professional referral offered",'""'import "Follow-up session scheduled" },''""''
                safetyPlan: {
                    immediateSteps: [ "If in immediate danger, call 988 (Suicide & Crisis Lifeline)",""'""'
                        "Reach out to trusted friend/family member",""''""'""'
                        "Remove any means of self-harm from immediate environment",'""'import 'Stay in public/safe spaces" ],"'""'"'
                    copingStrategies: [ "Deep breathing exercises (4-7-8 technique)',""""'
                        'Grounding technique: 5-4-3-2-1 sensory awareness","'""
                        "Listen to calming music or favorite podcast",'""'import 'Take a warm shower or bath" ],"'""'"'
                    supportContacts: [
                        "Crisis Lifeline: 988',""'""'"'
                        "Crisis Text Line: Text HOME to 741741',"""'"'""'
                        'Emergency Services: 911","""'import 'Platform Crisis Support: Available 24/7""'""
                    ]
  ],
            ...
            adminReview: {
                ...
                adminActions: [ "Helper performance commended',""'""'"'
                    "Crisis response documented for training',"""'"'""'
                    'User added to enhanced monitoring list",""'"'""'
                    'Quality assurance metrics updated","""'import 'Follow-up reminder scheduled" ],"''"""'
                qualityScore: 9.7,
                recommendedImprovements: [ "Consider shorter initial response time target',""'import "Add automated resource delivery enhancement" },""'""'
            ...
            resolutionTracking: {
                ...
                interventionsUsed: [ "Crisis counselor immediate response",'""''""""'
                    'Safety plan implementation","'""""
                    'Professional referral accepted","'import "Ongoing platform support established" ],""'""'
                userFeedback: {
                    rating: 5,
        ...
        return {
            ...
            scenario1: {
                userReport: {
                    ...
                    timestamp: new Date(Date.now() - 8 * 60 * 60 * 1000).toISOString(),
                    evidence: [ 'Chat logs showing personal contact sharing","""'import 'Screenshot of meeting suggestion message" },"''"""'
                ...
                adminResponse: {
                    ...
                    investigationFindings: [ 'Clear boundary violation confirmed",""'"'""'
                        'User safety compromised",""'"'"'
                        "Helper training deficiency identified",'""'import "No malicious intent detected" },''""''
                    actionsTaken: [ "Helper temporarily suspended",""''""'"'
                        "Mandatory retraining scheduled","''""'"'
                        "User provided additional support","''""'"'
                        "Platform guidelines clarified","''import "Monitoring increased for this helper" ],'""'
            ...
            scenario2: { communityPost: {
                ...
                helperModeration: {
                    ...
                    immediateActions: [ "Post removed from community","''""'"'
                        "User notified of violation","'""'
                        'Case escalated to admin team","""'import 'Similar content scan initiated" },"''""'
                ...
                adminPolicyResponse: {
                    ...
                    policyImplications: [ "Medication advice guidelines need strengthening',""''"""'
                        "Automated detection filters require enhancement',""'import 'Helper training modules need updating" ],"""'
                    systemChanges: [ 'Enhanced keyword detection for medication-related content","'""
                        "Automatic escalation for drug-related posts",'""''"""'
                        "Community guidelines updated',""'import 'Helper training curriculum revised" ],""'"'"'
                    preventiveMeasures: [ "Proactive content scanning implemented",'"'"""''
                        "Educational resources about medication safety added",'""'import "Community awareness campaign launched" ],''""'"'
...
const crisisScenarios = this.getCrisisEscalationScenarios();
        ...
        if (userType === "user") { data.journalEntries = this.getStarkeeperJournalEntries(userToken);'"'"'"'
            ...
            data.platformExamples = { crisisSupport: workflowData}
  ...
  }} else if (userType === "helper") { data.profile = this.getConstellationGuideProfile();""''""'"'
            ...
            data.helperScenarios = { crisisIntervention: workflowData.helperIntervention}
            ...
            data.crisisInterventions = {}
                ...
                medicalEmergencyIntervention: {
                    ...
                    outcome: crisisScenarios.substanceAbuseCrisis.resolutionTracking
  }} else if (userType === "admin") { data.profile = this.getAstralAdminProfile();"''""'"'
            ...
            data.adminOversight = { crisisManagement: workflowData.adminReview}
            ...
            data.crisisManagement = {}
                ...
                medicalEmergencyCrisisOversight: { incident: crisisScenarios.substanceAbuseCrisis.userPost,
...
const stored = localStorage.getItem(`demo_data_${userType)`);
            if (stored) { try {
                    return JSON.parse(stored) } catch (parseError) { // If stored data is malformed, regenerate it
                    console.warn("Malformed demo data found, regenerating..." );'""'
  );
  ) catch (storageError) { // If localStorage is not available or throws an error
...
const parsed = JSON.parse(demoUser  );
  ...
  } catch (error) { // Use default token if parsing failsreturn this.initializeDemoData(userType, userToken)}
    // Clear demo data (for reset/logout)
    clearDemoData() { localStorage.removeItem("demo_data_user");"'""'
        ...
        return {}
            ...
            immediateRisk: {}
                ...
                resolutionTracking: { currentStatus: "crisis_stabilized_monitoring",'""''"""'
                    ...
                    followUpScheduled: [ "2-hour safety check-in","'"'"'""'
                        "24-hour crisis counselor session",'"'"'""'
                        "72-hour platform welfare check",'""'import 'Weekly therapy referral appointment" ] };"""'
  },
            ...
            domesticViolence: {
                ...
                aiDetection: { detectionTime: new Date(Date.now() - 14 * 60 * 1000).toISOString(),
                    algorithm: "Domestic Violence Crisis Detection v2.1',""'""'"'
                    confidenceScore: 0.94,
                    riskFactors: [ "Escalating abuser behavior',""'""'"'
                        "Physical isolation (keys taken)',""""'
                        'Children present in dangerous situation","'""""''
                        "Fear for immediate safety",'"'import "Potential goodbye message" ],"'""'
                    automatedActions: [ 'Domestic violence specialist notified",""'"'""'
                        'Safety planning resources deployed",""'"'""'
                        'Local DV hotline information provided",""'"'"'
                        "Emergency exit strategy resources",'""'import "Admin crisis team alerted" ],''""''

                helperResponse: { responderId: "helper-dv-specialist-001",""''""'"'
                    ...
                    safetyActions: [ 'Immediate safety planning activated",""'"'""'
                        'DV hotline: 1-800-799-7233 provided",""'"'"'
                        "Local shelter information shared",'""'""'"'
                        "Safety bag checklist provided',""'import "Emergency contact strategy discussed" },'"'"'"'
                    emergencyResources: [ "National DV Hotline with text option",""'""'
                        "Local shelter with immediate availability",""'""'
                        "Legal advocacy resources",""'""'
                        "Child protective safety planning',"""'import "Emergency transportation assistance' ] },"'"'"'

                adminEscalation: { escalatedBy: "helper-dv-specialist-001","'"'"'""'
                    ...
                    emergencyProtocols: [ "Child protection services consultation",""''""'"'
                        "Local law enforcement liaison","''""'"'
                        "Emergency shelter coordination","''""'"'
                        "Legal advocacy team notification","''import "Crisis intervention team deployment" ],'""'
                    legalConsiderations: "Mandatory reporting protocols for children at risk",'"'"'"'
                    resourcesDeployed: [ "DV crisis team (2 specialists)",""''""'"'
                        "Child safety advocate","''""'"'
                        "Emergency shelter coordination","''""'"'
                        "Legal advocacy referral","''import "Transportation assistance network" ] },'""'

                resolutionTracking: { currentStatus: "emergency_safety_plan_implemented",'"'"'"'
                    ...
                    followUpScheduled: [ "Daily safety check-ins for 1 week","''""'"'
                        "Legal advocacy appointment scheduled","'""'
                        'Counseling for children arranged","""'import 'Long-term safety planning session" } ];"'""
  ],
            ...
            substanceAbuseCrisis: {
                ...
                aiDetection: { detectionTime: new Date(Date.now() - 7.5 * 60 * 1000).toISOString()}
                    algorithm: "Medical Emergency Detection v1.8',"'""
                    confidenceScore: 0.96,
                    riskFactors: [ "Physical overdose symptoms described',"'"]"
                        "Fear preventing emergency contact',"'""
                        "Substance relapse with quantity concern',"'""
                        "Medical distress indicators",'"'import "Isolation due to stigma' ],"""'
                    automatedActions: [ "Medical emergency specialist notified',""''}"""'
                        "Overdose prevention resources provided',""'"""
                        "Emergency medical guidance initiated',""'""""''
                        "Crisis medical team alerted",'""'import "Harm reduction resources deployed" ],''""''

                helperResponse: { responderId: "helper-medical-crisis-001",""'}""'
                    ...
                    medicalGuidance: [ 'Immediate 911 call strongly advised',"""]"
                        'Stay awake and try to stay calm',""""
                        'If you have someone nearby, ask them to stay with you',""""
                        'Do not take any more substances","'import "Try to remember what and how much you took" ],""'"'
                    emergencyActions: [ "Medical emergency protocol activated",'"'"}""''
                        "Emergency services coordination offered",'""'""''
                        "Poison control center information provided",'""'""'""'
                        'Emergency room escort service offered","""'import 'Family notification support offered" } },"''""'

                adminEscalation: { escalatedBy: "helper-medical-crisis-001",'"'"}'"'
...
getPeerConnections(): unknown[] {
        return [ {
                ...
                sharedGoals: [
                    ...
                    "Journaling before bed",""''import "Weekly anxiety check-ins" },'""""'
                status: 'active","'""""'"'
                interactionCount: 28,
                isMatched: true,
            {
                ...
                sharedGoals: [ "90-day sobriety milestone",'""""'
                    'Building healthy coping strategies","'import "Rebuilding family relationships" ],""'"'
                status: "active",'"""'
                interactionCount: 45,
                isMatched: true
  ],
            {
                ...
                sharedGoals: [ 'DBT skills practice","'""""
                    'Anxiety management techniques","'import "Academic stress reduction" ],""'""'
                status: "active',""'""''
                interactionCount: 12,
                isMatched: true
