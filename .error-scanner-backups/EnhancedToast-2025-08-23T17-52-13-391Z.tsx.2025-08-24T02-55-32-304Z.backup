...
interface ToastProps { { { { id: string
$2: ToastType
  title?: string
  message: string
  duration?: number
...
const TOAST_ICONS = { success: CheckIcon || (() =) <span>✓</span}
...
className={enhanced ? "btn-enhanced ghost sm' : "toast-close"}'"""
  ...
  ;
  ;
interface ToastContainerProps { { { { toasts: Array<Omit<ToastProps, "onClose'>>;""''""'
  onClose: (id: string) = void
  enhanced?: boolean
  position?: "top-right" | 'top-left" | "bottom-right' | "bottom-left" ;"
