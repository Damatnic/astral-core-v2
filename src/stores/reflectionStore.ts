...
setReflections: (reflections) => set({ reflections }),
    ...
    addReflection: (reflection) => set((state) => ({ reflections: [reflection, ...state.reflections] })),
...
const updatedReactions = { ...r.reactions };
                if (userReaction && !r.myReaction) { updatedReactions[reactionType] = (updatedReactions[reactionType] || 0) + 1 }
                return {
  ...r,
};
