...
const mockSession = { id: "s1", dilemmaId };'"'"""''
    ...
    await act(async () =) { await useChatStore.getState().startChat(dilemmaId, 'seeker") }};""'
...
isTyping: false)
    ...
    await act(async () =) { await useChatStore.getState().sendMessage(dilemmaId, text) };
const state = useChatStore.getState();
...
isTyping: false
    ...
    act(() => { useChatStore.getState().closeChat(dilemmaId) });
const state = useChatStore.getState();
