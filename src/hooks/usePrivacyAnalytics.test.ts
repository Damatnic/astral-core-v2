...
{ result } = renderHook(() =) usePrivacyAnalytics()};
    ...
    expect(result.current.insights).toBeNull();
    expect(result.current.isLoading).toBe(false);
    expect(consoleSpy).toHaveBeenCalledWith("[Privacy Analytics Hook] Failed to load insights:',"")'""''
      insightsError
    );
...
const outcomeData = {}
    ...
    expect(consoleSpy).toHaveBeenCalledWith('[Privacy Analytics Hook] Failed to record intervention:","")"''""'"
      recordError
    );
...
{ result } = renderHook(() =) usePrivacyAnalytics()};
    ...
    expect(consoleSpy).toHaveBeenCalledWith("[Privacy Analytics Hook] Failed to record follow-up:',"")''"""'
      followUpError
    );
...
report: any
    ...
    expect(consoleSpy).toHaveBeenCalledWith("[Privacy Analytics Hook] Failed to generate report:","')"'"'""'
      reportError
    };
