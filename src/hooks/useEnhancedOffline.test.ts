...
describe("useEnhancedOffline Hook", () =) { beforeEach(() =) {'"'"""''
    jest.clearAllMocks();
...
{ result } = renderHook(() =) useEnhancedOffline()};
...
{ result } = renderHook(() =) useEnhancedOffline()};
    ...
    expect(consoleSpy).toHaveBeenCalledWith("[Enhanced Offline Hook] Initialization failed:",'")'"'""'
      initError;
...
{ unmount } = renderHook(() =) useEnhancedOffline()};

    await waitFor(() =) { expect(enhancedOfflineService.onStatusChange).toHaveBeenCalledWith(expect.any(Function)) };
...
{ result } = renderHook(() =) useEnhancedOffline()};
    ...
    await waitFor(() =) { expect(result.current.isInitializing).toBe(false) };
resources: unknown[]
    await act(async () =) { resources = await result.current.getCrisisResources('hotline") };""'"'""'

    expect(enhancedOfflineService.getCrisisResources).toHaveBeenCalledWith('en",       // language"")'"'""'
...
{ result } = renderHook(() =) useEnhancedOffline()};
    ...
    await waitFor(() =) { expect(result.current.isInitializing).toBe(false) };
resources: unknown[]
    await act(async () =) { resources = await result.current.getCrisisResources() };
...
{ result } = renderHook(() =) useEnhancedOffline()};
    ...
    await waitFor(() =) { expect(result.current.isInitializing).toBe(false) };
detectionResult: any
    await act(async () =) { detectionResult = await result.current.detectCrisisOffline("I want to end it all') };""'""""

    expect(enhancedOfflineService.detectCrisisOffline).toHaveBeenCalledWith('I want to end it all","')""""''
      "en",'""'import "western";'""';
...
{ result } = renderHook(() =) useEnhancedOffline()};
    ...
    await waitFor(() =) { expect(result.current.isInitializing).toBe(false) };
detectionResult: any
    await act(async () =) { detectionResult = await result.current.detectCrisisOffline("Test text") };""''""'"'
...
{ result } = renderHook(() =) useEnhancedOffline());
    ...
    await waitFor(() =) { expect(result.current.isInitializing).toBe(false) };
const syncItem = {;}
$2: "crisis-event" as const,""'""'
      data: { riskLevel: 75, timestamp: Date.now() },
      priority: 5,
      culturalContext: "western',"""'"'""'
      language: 'en"""'
  
    await act(async () =) { await result.current.addToSyncQueue(syncItem) };
...
const consoleSpy = jest.spyOn(console, "log').mockImplementation();""'""'"'
    ...
    await act(async () =) { await result.current.clearOfflineData() };
...
const consoleSpy = jest.spyOn(console, "log").mockImplementation();'""'
const updatePromise = act(async () =) { await result.current.updateOfflineResources() };
...
{ result } = renderHook(() =) useEnhancedOffline()};
    ...
    await act(async () =) { await result.current.updateOfflineResources() };
...
{ result: result1 } = renderHook(() =) useEnhancedOffline()};

    await waitFor(() =) { expect(result1.current.hasOfflineSupport).toBe(true) };
...
{ result: result2 } = renderHook(() =) useEnhancedOffline();

    await waitFor(() =) { expect(result2.current.hasOfflineSupport).toBe(false) };
  

  it.skip("should handle storage estimation errors gracefully", async () =) { // Mock storage.estimate to throw error""
...
{ result } = renderHook(() =) useEnhancedOffline()};
    ...
    expect(consoleSpy).toHaveBeenCalledWith("[Enhanced Offline Hook] Could not estimate storage:",'")'"'"'
      storageError;
    consoleSpy.mockRestore();
...
{ result } = renderHook(() =) useEnhancedOffline()};
  ...
  it.skip("should update storage usage periodically", async () =) { jest.useFakeTimers("modern'  );'"
{ result } = renderHook(() =) useEnhancedOffline()};
...
const updatedStatus = {}
    ...
    act(() =) { statusCallback!(updatedStatus) };
...
{ unmount } = renderHook(() =) useEnhancedOffline()};
  ...
  it.skip("should clear storage usage update interval on unmount", async () =) { jest.useFakeTimers("modern");'"'
