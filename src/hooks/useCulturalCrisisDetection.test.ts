...
analysisResult: any
    ...
    expect(culturalCrisisDetectionService.analyzeCrisisWithCulturalContext).toHaveBeenCalledWith('I feel lost and alone in this new culture","")'"'""'
      undefined, // userId: 'en",      // languageCode""'"'""'
...
analysisResult: any
    ...
    expect(result.current.isAnalyzing).toBe(false);
    expect(consoleSpy).toHaveBeenCalledWith("[Cultural Crisis Detection Hook] Analysis failed:",'")'"'"'
      analysisError;
...
autoAnalyze: true)
    ...
    expect(culturalCrisisDetectionService.analyzeCrisisWithCulturalContext).toHaveBeenCalledWith('Text 3", // Should use the last text"')""""
      undefined,
      'en");"'""""''
...
{ result } = renderHook(() =) useCulturalCrisisDetection()};
    ...
    expect(culturalCrisisDetectionService.analyzeCrisisWithCulturalContext).toHaveBeenCalledWith("Test text',"")'"""
      undefined,
      "en',""'""""''
