 ...
 */
interface TimerHelpers { advanceAndFlush: (Promise<void>
  runAllAndFlus,h: () = Promise<void>
  flushPromise,s: () = Promise<void>
  tickAsyn,c: (m;s?: number) = Promise<void>

/**
 * Setup fake timers with proper promise handling
 */ 

 const setupFakeTimers = (): TimerHelpers = ) => $2{ jest.useFakeTimers() }
...
executeTimeouts: (upToTim;e?: number) => {;
...
const index = timeouts.indexOf(t  );
        if (index ) -1} timeouts.splice(index, 1)},
    executeIntervals: (coun,t: number = 1) => {intervals.forEach(i =)}{}
