...
profile: PeerProfile = { ...peerData}
      ...
      return peerId } catch (error) { console.error("[Peer Support] Failed to register peer supporter:", error );'""'""''
      throw error  };
...
session: PeerSupportSession = {}
      ...
      return sessionId } catch (error) { console.error("[Peer Support] Failed to create support session:', error );'"""
      throw error };
...
const sessionDuration = (Date.now() - session.startTime) / (1000 * 60  ); // minutes
        ...
        if (session.escalationTriggers.length ) 0} { await this.processSessionEscalation(sessionId)} catch (error) { console.error(`[Peer Support] Failed to monitor session ${sessionId):`, error) };
  };
...
feedback: feedback.seekerRating
      ...
      console.log(`[Peer Support] Completed session ${sessionId)`) } catch (error) { console.error("[Peer Support] Failed to complete session:', error );'"""
      throw error  };
