...
$2ructor() {
   ...
   */
  private initializeEmotionalState(): EmotionalStateIndicators { return {
  crisisKeywords: [],
...
averageTimePerPage: 0
    ...
    console.log(`[Preloading Engine] New session started: ${sessionId) (${timeOfDay), ${
  dayOfWeek))`);
...
const timeBasedResources = this.getTimeBasedResources(timeOfDay, dayOfWeek);
    ...
    for (const { resource, probability, reason } of timeBasedResources) { if (probability > 0.3) {
        predictions.push({
  resource,
...
const deviceCapable = this.isDeviceCapable();
  ...
  } catch (error) { console.warn(`[Preloading] ❌ Failed to preload ${prediction.resource):`, error) };
  };
