...
export const peerSupportService = {}
  connectToPeer: jest.fn().mockResolvedValue({
  ,
  peerId: "mock-peer-123', status: "connected" )),""''""'"
};
...
sendMessage: jest.fn().mockResolvedValue({
  ,
  messageId: "mock-msg-123", delivered: true )),"''""'"'
};
...
reportPeer: jest.fn().mockResolvedValue({
  ,)
};
...
blockPeer: jest.fn().mockResolvedValue({
  ,
  blocked: true )),
};
...
unblockPeer: jest.fn().mockResolvedValue({
  ,)
};
...
groupId: "mock-group-123", joined: true )),'"'"'""'
};
...
createGroup: jest.fn().mockResolvedValue({
  ,
  groupId: "new-group-123", created: true )),'"'"'""'
};
