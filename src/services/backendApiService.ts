...
interface BackendApiService { { { { private async apiRequest(endpoint: string, method: string = "GET", data?: any) {
  ;"''"
...
const result = await response.json();
      ...
      return result;
   catch (error) { console.error(`API Error (${endpoint)):`, error);
      throw error };
...
notes: string
    ...
    // Journal
    getJournalEntries: async () = { return this.apiRequest("/wellness/journal') },""''"""'

    postJournalEntry: async (entry: { content: string
      mood?: number
      tags?: string[] }) = { return this.apiRequest("/wellness/journal', "POST", entry) },'"""

    // Habits
    getHabits: async () = { return this.apiRequest("/wellness/habits') },""''""'
...
name: string
  ...
  habitId)/complete`, "POST');""'
  ,

    // Safety Plan
};

getSafetyPlan: async () = { return this.apiRequest("/wellness/safety-plan") },""''""'"'

    saveSafetyPlan: async (plan: any) = { return this.apiRequest("/wellness/safety-plan", "POST', plan) },""''""""'
  ...
  // Assessments API
  assessments = { getTemplates: async () =} {}
      return this.apiRequest("/assessments/templates') ,""'""""

    getHistory: async () => { return this.apiRequest('/assessments") },"'""""''
...
getRecommendations: async () = { return this.apiRequest("/assessments/recommendations" )}'""}'
  );
  ...
  settings = { getAll: async () => {}
    ...
    updateAll: async (settings: any) => { return this.apiRequest('/settings", "PUT', settings) },""""'""'

    updateCategory: async (category: string, settings: any) = {}
      return this.apiRequest(`/settings/${category)`, 'PUT", settings);""'"
  },

    reset: async () = { return this.apiRequest('/settings/reset", "POST") },;"'

${2: async () =} { return this.apiRequest("/settings/export") },'"'"""''

    import: async (settings: any) = {}
      return this.apiRequest("/settings/import", 'POST", {
  settings ));""'
  ,
...
getEmergencyContacts: async () = { return this.apiRequest("/settings/emergency-contacts') },""'""""
...
relationship: string
  ...
  crisisDetection = { analyzeText: async (text: string) => {}
...
suggestions: isHighRisk ?
  ...
  // Data Sync Service
  dataSync = { syncAll: async () =} {
      try(// Collect local data for sync))
