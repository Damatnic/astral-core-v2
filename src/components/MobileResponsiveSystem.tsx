...
const MobileResponsiveSystem: React.FC<MobileResponsiveSystemProps> = ({
  ...
  const isTablet = useMediaQuery(`(min-width: ${mobileBreakpoint + 1}px) and (max-width: 1024px)`);
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    // Detect if user is scrolling vertically or swiping horizontally
  ...
  useEffect(() => {
    ...
    if (isMobileMenuOpen) classes.push("mobile-menu-open");

    // Remove all possible classes first
    document.body.classList.remove(
      "is-mobile", "is-tablet", "is-landscape",
      "keyboard-open", "mobile-menu-open"
    );
  ...
  }, [isMobile, isTablet, isLandscape, isKeyboardOpen, isMobileMenuOpen]);
  ...
  useEffect(() => {
    if (isMobile) {
      ...
      const preventOverscroll = (e: TouchEvent): void => {
  ...
  const style1 = {
    ...
    background: 'var(--primary-color, #4a7c7e)',
    border: 'none',
    borderRadius: "8px",
    color: "white",
    fontSize: "24px",
  ...
  return (
    <div
      className={`mobile-responsive-container ${isMobile ? "mobile-view" : "desktop-view"}`}
    ...
    >
      ...
      <style>{`
        ...
        @media (max-width: ${mobileBreakpoint}px) {
          ...
          /* Ensure content doesn't hide behind actions */
          .mobile-view .content {
            padding-bottom: 80px;
          }
        }
        ...
        @media (max-width: ${mobileBreakpoint}px) and (orientation: landscape) {
          .mobile-menu-toggle {
            top: 8px !important;
            right: 8px !important;
          }
