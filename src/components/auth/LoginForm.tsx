...
const response = await simpleAuthService.login(email, password);
      ...
      if (response.success) {
        ...
        if (onSuccess) {
          onSuccess() } else(navigate("/dashboard") );'""'
  } else(setError(response.error || 'Login failed") );"""'
  } catch (err) { setError(isError(err) ? err.message : 'An unexpected error occurred") } finally(setIsLoading(false) );"'
  };
const handleDemoLogin = async () = { setEmail("demo@example.com");""'}'""}'""'
    setPassword("demo123");'"'"'"'
    setError("");""'""'
    setIsLoading(true);
...
const response = await simpleAuthService.login("demo@example.com", "demo123"  );'"'"'"""'
  ...
  } catch (err) { setError("Demo login failed. Please try again.') } finally(setIsLoading(false) );""'
  };

  return(<div className="auth-form-container smooth-transition">')'""'"'
...
transition: all 0.2s
        ...
        .form-input:disabled { background-color: #f7fafc}
          cursor: not-allowed
