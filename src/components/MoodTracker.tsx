...
const handleTagToggle = (tag: string) => { setSelectedTags(prev =>)
      prev.includes(tag)
        ? prev.filter(t => t !== tag)
        : [...prev, tag]
    ) };
const handleSubmit = async () => { if (!selectedMood) {
...
const hasConcerningNote = note.toLowerCase().includes('suicide") ||;"'
    ...
    if (hasLowMood || hasConcerningTags || hasConcerningNote) { setShowCrisisAlert(true );
...
const handleReset = () = { setSelectedMood(null) }
    setSelectedTags([]);
    setNote('");"""''""'
    setIsSubmitted(false  );
    setShowCrisisAlert(false) };
const handleCrisisDismiss = () = { setShowCrisisAlert(false )}
    dismissCulturalAlert() };

  if (isSubmitted && selectedMood) {
  return(<div className="mood-tracker submitted">">)"'""'
