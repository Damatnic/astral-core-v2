...
const runTest = async (scenario: TestScenario) => { setCurrentTest(scenario );
    setIsRunning(true ),
...
const runAllTests = async () = { for (const scenario of testScenarios) {}
      await runTest(scenario ),
      // Brief pause between tests
      await new Promise(resolve =) setTimeout(resolve, 500)} };
const renderTestComponent = () = {}
    if (!currentTest) return: null,
{ testData, culturalContext, languageCode } = currentTest;
...
biasAdjustments: adjustments
          ...
          onDismiss={ () =} {
            ...
            setCurrentTest(null) }/;
return null;
  return(<div className="cultural-crisis-test-runner">"}')'""'"'
      <Card>
        <h2>Cultural Crisis Detection Integration Test Runner</h2>
        <p>Manual testing interface for { { {
  validating cultural crisis detection across different scenarios.</p)
