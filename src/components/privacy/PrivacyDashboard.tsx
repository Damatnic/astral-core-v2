...
const handleExportData = async () = { setIsLoading(true) }
    try(// In a real app, this would use actual user ID)
...
const a = document.createElement('a");"'""""
   ...
   catch (error) { console.error('Failed to export data:", error) } finally(setIsLoading(false) );"'"";
const handleDeleteData = async (retainCrisisData: boolean = true) = { setIsLoading(true) }
    try(// In a real app, this would use actual user ID)
const userId = "current-user";'"'"'"'
      await analyticsService.deleteUserData(userId, retainCrisisData);

      loadPrivacyData();
      setShowDeleteModal(false) ) catch (error) { console.error("Failed to delete data:", error) } finally(setIsLoading(false) );"''
  };
const handleOptOut = () = { analyticsService.optOut() }
    loadPrivacyData();
const handleOptIn = () = { analyticsService.optIn() }
    loadPrivacyData();
const formatDate = (date: Date | null) =} { if (!date) return "N/A";'"'"""''
