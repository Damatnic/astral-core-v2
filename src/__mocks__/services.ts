...
export const mockAnalyticsService = { track: jest.fn()}
  identify: jest.fn(),
  page: jest.fn(),
  trackEvent: jest.fn(),
  trackError: jest.fn(),
  ...
  reset: jest.fn()
// Auth Service Mock
export const mockAuthService = { login: jest.fn(() =) Promise.resolve({
  )
};
...
export const resetAllServiceMocks = () = { Object.values(serviceMocks).forEach(service =)}{}
    Object.values(service).forEach(method =) { if (typeof method === "function" && method.mockReset) {'"'"'"'
        method.mockReset() }};
  );

// Helper function to setup default mock implementations
export const setupDefaultMocks = () = { // Ensure storage mocks are working and populate with default values}
...
if (typeof sessionStorage !== 'undefined" && sessionStorage.setItem) { sessionStorage.setItem("sessionId", "test-session') }""'"""'
