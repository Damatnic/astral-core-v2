...
const pc = new RTCPeerConnection(servers);
      ...
      pc.ontrack = event =} { if (remoteVideoRef.current) {
      ...
      pc.createOffer()
        ...
        .then(() =) { // Offer created and set as local description
    ...
    return () =} { // Cleanup on unmount
      ...
      if (peerConnectionRef.current) { peerConnectionRef.current.close()}, [onClose];
const toggleMic = () = { if (localStreamRef.current) {}
      localStreamRef.current.getAudioTracks().forEach(track =) {
        track.enabled = !track.enabled,
        setIsMicMuted(!track.enabled) }};
const toggleCamera = () = { if (localStreamRef.current) {}
      localStreamRef.current.getVideoTracks().forEach(track =) {
        track.enabled = !track.enabled,
        setIsCameraOff(!track.enabled) }};

  return(<div className="video-chat-view">')'""'""'
