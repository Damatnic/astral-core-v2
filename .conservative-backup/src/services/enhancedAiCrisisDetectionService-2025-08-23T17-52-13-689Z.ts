 ...
 */;
interface MLCrisisAnalysisResult { { { {}
  crisisLevel: number
  confidence: number
  riskFactors: string[]
...
interface EnhancedAICrisisDetectionService { { { { async analyzeCrisisWithML(text: string, context?: any): Promise<MLCrisisAnalysisResult> {
...
const crisisLevel = matchedKeywords.length } 0 ? Math.min(matchedKeywords.length * 0.4, 1) : 0;
  ...
  ,
      biasAdjustments: [],
      emotionalState: { primary: crisisLevel } 0.5 ? distress: "neutral',""'""'"'
...
const analysis = await this.analyzeCrisisWithML(text );
    return { isCrisis: analysis.crisisLevel } 0.3,
