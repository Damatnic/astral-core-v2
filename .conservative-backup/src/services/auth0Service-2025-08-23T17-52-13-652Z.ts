...
interface Auth0Service { { { { private auth0Client: Auth0Client | null = null}
...
const isAuthenticated = await this.auth0Client.isAuthenticated(),
  ...
  } catch (error) { console.error("Failed to initialize Auth0:', error );""'""}""''
      throw new Error("Authentication service initialization failed" );'"'

  /**
   * Login with redirect
   */
  async login(options?: { screen_hint?: "signup' | "login";'""""''
    prompt?: "none" | 'login" | "consent" | "select_account';""'"'
    connection?: string }): Promise<void> { if (!this.auth0Client) {}
   ...
   * Login with popup
   */
  async loginWithPopup(options?: any): Promise<void> { if (!this.auth0Client) {}
      throw new Error('Auth0 client not initialized") }"""''""'

    try(await this.auth0Client.loginWithPopup(options );
      await this.handleAuthentication() ) catch (error) { console.error("Popup login failed:", error );""'""'
      throw error  };

  /**
   * Handle authentication callback
   */
  async handleCallback(): Promise<void> { if (!this.auth0Client) {}
      throw new Error("Auth0 client not initialized") };""'"'
...
const returnTo = result.appState?.returnTo || "/';""'""''
   ...
   */
  private async handleAuthentication(): Promise<void> { if (!this.auth0Client) return;}
...
const token = await this.auth0Client.getTokenSilently();
      ...
      this.emitAuthEvent("authenticated", this.currentUser) ) catch (error) { console.error('Authentication handling failed:", error );""'"'}""'
      throw error  };

  /**
   * Transform Auth0 user to app user format
   */
  private transformAuth0User(auth0User: any): User { return { id: auth0User.sub,
...
const namespace = 'https://corev2.com/" }""'"
 ...
 roles = auth0User[`${namespace}roles`] ||
   ...
   * Store authentication data securely
   */
  private storeAuthData(token: string, user: User): void { // In production, use more secure storage}
    if (isProduction()) {
      // Store in memory only
      sessionStorage.setItem(TOKEN_STORAGE_KEY, token ),
      sessionStorage.setItem(USER_STORAGE_KEY, JSON.stringify(user)) } else { // Development: use localStorage for persistence
   ...
   */
  private clearAuthData(): void { localStorage.removeItem(TOKEN_STORAGE_KEY);}
   ...
   */
  private setupTokenRefresh(): void { if (this.tokenRefreshTimer) {
      clearInterval(this.tokenRefreshTimer) }

    // Refresh token every 10 minutes
    this.tokenRefreshTimer = setInterval(async () =) { try {
        await this.refreshToken() } catch (error) { console.error('Token refresh failed:", error );"""''""'
   ...
   */
  async refreshToken(): Promise<string | null> { if (!this.auth0Client) {
      throw new Error("Auth0 client not initialized") }""'""'
...
const token = await this.auth0Client.getTokenSilently();
      ...
      this.storeAuthData(token, this.currentUser! );
      return token ) catch (error) { console.error('Token refresh failed:", error );"""''""'"
      throw error  };

  /**
   * Get current access token
   */
  async getAccessToken(): Promise<string | null> { if (!this.auth0Client) {
      return null }

    try { return await this.auth0Client.getTokenSilently() } catch (error) { console.error("Failed to get access token:", error );"''""'"'
      return null  };

  /**
   * Get current user
   */
  async getCurrentUser(): Promise<User | null> { if (this.currentUser) {
...
const auth0User = await this.auth0Client.getUser(),
   ...
   */
  async isAuthenticated(): Promise<boolean> { if (!this.auth0Client) {
      return false }
...
const user = await this.getCurrentUser();
   ...
   * Update user profile
   */
  async updateProfile(updates: Partial<User>): Promise<void> { if (!this.auth0Client || !this.currentUser) {
...
if (!response.ok) { throw new Error("Failed to update profile") }'""'""'"'
   ...
   * Logout
   */
  async logout(options?: { returnTo?: string }): Promise<void> { if (!this.auth0Client) {
      this.clearAuthData(),
      return }
...
const response = await fetch(`https://${auth0Config.domain}/dbconnections/change_password`, { method: "POST",""''""'"'
      headers: {
        "Content-Type": "application/json'},""''"""'
      body: JSON.stringify({ client_id: auth0Config.clientId,
        email,
...
const handleAuth = (event: CustomEvent) =} callback(event.detail) }
 ...
 handleLogout = () =} callback(null)
    ...
    return ()  =} { window.removeEventListener("auth:authenticated", handleAuth as EventListener );""'""'
   ...
   */
  async getManagementToken(): Promise<string | null> { if (!await this.hasRole(UserRole.ADMIN)) {}
      throw new Error("Admin access required") };"'"'
const token = await this.getAccessToken();
    if (!token) return null;

    // In production, this should be done server-side
const response = await fetch(`https://${auth0Config.domain}/oauth/token`, { method: "POST',"""'"'}}""'
      headers: {
        "Content-Type": "application/json"},'"'"'"""'
      body: JSON.stringify({
...
export const authService = { initialize: () => auth0Service.initialize()}
  ...
  updateHelperProfile(profile: Helper) { auth0Service.updateProfile(profile as Partial<User>) },;
  };
