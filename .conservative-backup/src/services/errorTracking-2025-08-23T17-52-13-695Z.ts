...
interface ErrorContext { { { { errorType: "system' | "user-action" | "network" | 'security" | "crisis'"
...
return value;
    ...
    try { sanitizedContext = JSON.parse(sanitizedStr) } catch {
...
const getEnvironmentName = (): string =} { if (isProduction) return "production';""'""'"'
  if (isDevelopment) return "development';"""'"'""'
  return "staging" };""'""'

// Initialize Sentry
export const initializeSentry = (dsn?: string) = { if (!dsn) {}
