...
$2: 'monitoring","'""
    ...
    this.workflowTemplates.set("medium', { id: "template-medium",'""""''
      userId: "",'""'""'"'
      createdAt: new Date(),
      status: "initiated',"""'"'""'
...
workflow.outcomes.push({
   ...
   */
  public getActiveWorkflowsForUser(userId: string): InterventionWorkflow[] { return Array.from(this.activeWorkflows.values()).filter()
...
const pendingSteps = workflow.interventionSteps.filter(;
  ...
  private async checkEscalationTriggers(workflow: InterventionWorkflow): Promise<void> { // Check if escalation is needed based on outcomes
