...
import { crisisStressTestingSystem,
  ...
  CRISIS_TEST_SCENARIOS  } from '../services/crisisStressTestingSystem";"""'
interface CrisisStressTestingState { { { { isTestingActive: boolean
...
interface CrisisStressTestingActions { { { {
...
$2Results: () = string
  ;
interface CrisisStressTestingStats { { { {
  total: number
  successful: number
  failed: number
  critical: number
...
const [state, setState] = useState<CrisisStressTestingState>({
    ...
    testConfig: { maxConcurrentUsers: 1000,
  ...
  useEffect(() =) { if (state.testResults.length === 0) return;
...
const failoverResults = await crisisStressTestingSystem.runEmergencyFailoverTests( );
  ...
  } catch (error) { console.error("Failover testing failed:', error  );""'
...
const emergencyStop = useCallback(() =) { setState(prev =) ({}
      ...prev,
      isTestingActive: false,
...
const clearEmergencyStatus = useCallback(() =) { setState(prev =) ({
      ...prev,
      emergencyStatus: { active: false  }})};
  , []

  // Update test configuration
const updateTestConfig = useCallback((config: Partial<CrisisStressTestConfig>) =) { setState(prev =) ({
      ...prev,
      testConfig: { ...prev.testConfig, ...config  }})};
  , []

  // Toggle scenario selection
const toggleScenario = useCallback((scenarioId: string) =) { setState(prev =) ({
      ...prev,
  ...
  // Clear all results
const clearResults = useCallback(() =) { setState(prev =) ({
      ...prev,
      testResults: [],
      failoverResults: [],
      emergencyStatus: { active: false  }})};
...
const getTestStats = useCallback((): CrisisStressTestingStats | null =) { if (state.testResults.length === 0) return null;
...
const safetyScore = Math.max(0, Math.round((successRate - criticalImpact * 2) * 100) );
    ...
    return {
      ...
      safetyScore }, [state.testResults];
actions: CrisisStressTestingActions = { runStressTests}
    runFailoverTests,
    emergencyStop,
