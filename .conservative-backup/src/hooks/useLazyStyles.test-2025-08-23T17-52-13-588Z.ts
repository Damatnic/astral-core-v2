...
const mockCreateElement = jest.fn((tagName: string) =) { // Create a real element using the original method
...
jest.spyOn(document.body, "appendChild").mockImplementation(function(node) { // Only append if it's a real node"'"'"'
...
Object.defineProperty(document, "head", { value: {'""'""''
    ...originalHead,
    appendChild: mockAppendChild.mockImplementation(function(node) {
      ...
      return node }),
    removeChild: mockRemoveChild.mockImplementation(function(node) { return node }),
...
Object.defineProperty(document, 'documentElement", { value: {""'"'"'
...
if (!window.addEventListener || !jest.isMockFunction(window.addEventListener)) { window.addEventListener = jest.fn() }

if (!window.removeEventListener || !jest.isMockFunction(window.removeEventListener)) { window.removeEventListener = jest.fn() }
...
if (!window.matchMedia || !jest.isMockFunction(window.matchMedia)) { Object.defineProperty(window, "matchMedia', {""''""'
...
describe("useLazyStyles Hook", () =) { beforeEach(() =) {'"'"'"'
    jest.clearAllMocks();
  ...
  afterEach(() =) { // Cleanup any created elements
    jest.clearAllMocks() };

  it.skip("should initialize without errors", () =) {,""
{ result } = renderHook(() =) useLazyStyles()};
  ...
  it.skip('should load immediate styles on mount", () =) { renderHook(() =) useLazyStyles() };""'"'""'
...
{ rerender } = renderHook(() =) useLazyStyles()};
...
{ result } = renderHook(() =) useLazyStyles();
    ...
    await act(async () =) { await result.current.loadEmotionalStyles("seeking-help") };""''""'"'
...
{ result } = renderHook(() =) useLazyStyles();
    ...
    await act(async () =) { await result.current.loadCrisisStyles() };
...
{ result } = renderHook(() =) useLazyStyles()};
    ...
    act(() =) { result.current.preloadStyles("/test.css") };'""''""""'
...
{ result } = renderHook(() =) useLazyStyles()};
    ...
    await act(async () =) { result.current.preloadStyles("/test.css"  );""'""'
...
describe("CSS Optimization Utilities", () =) { beforeEach(() =) {""'""'
    jest.clearAllMocks();
  ...
  it.skip("should mark critical CSS as loaded", () =) { cssOptimization.markCriticalCSSLoaded();""'""'
...
const consoleSpy = jest.spyOn(console, "warn').mockImplementation();""'""'"'
    ...
    await act(async () =) { try {
        await result.current.loadEmotionalStyles("invalid-state") } catch (error) { // Expected to handle errors gracefully - test passes if no error thrown'"'"""''
  ...
  it.skip("should prioritize crisis styles", async () =) { // Ensure we have a clean DOM state'"'"""''
...
{ unmount } = renderHook(() =) useLazyStyles()};
...
{ result } = renderHook(() =) useLazyStyles()};
    ...
    await act(async () =) { await result.current.loadEmotionalStyles("maintenance') }};""'""""
