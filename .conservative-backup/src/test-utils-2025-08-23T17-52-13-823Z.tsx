...
import { AllMockProviders,
  MockAuthProvider,
  MockNotificationProvider,
  MockThemeProvider,
  MockAuthContextType,
  MockNotificationContextType,
  MockThemeContextType  } from './test-utils/mockContexts";"""''""'
...
const AllTheProviders: React.FC<{ children: React.ReactNode }> = ({ children }) = {}
  return <AllMockProviders>{children}</$1;
...
root: ReactDOM.Root | null = null
  ...
  act(() =) { root = ReactDOM.createRoot(container  );
const Wrapper = options?.wrapper || AllTheProviders;
const element = <Wrapper}{ui}</$1}root.render(element);
  }};
...
const boundQueries = Object.entries(queries).reduce((acc, [key, query]) =) { if (typeof query === "function') {""''""'
      acc[key] = query.bind(null, baseElement) }
    return acc;
  }, {} as any;

  // Update screen object to use our container
  Object.keys(boundQueries).forEach(key =) { if ((screen as any)[key]) {
      (screen as any)[key] = boundQueries[key] };
  };

  // Ensure root was created
  if (!root) { throw new Error("Failed to create React root") };"'"'
...
const Wrapper = options?.wrapper || AllTheProviders;
  ...
  ,
    rerender: (newUi: ReactElement) = {}
      act(() =) {,
const newElement = <Wrapper}{newUi}</$1finalRoot.render(newElement);
...
const template = document.createElement("template'  );"""'"'""'
    ...
    ...Object.entries(queries).reduce((acc, [key, query]) =) { if (typeof query === "function") {""'""'
...
error: Error | null = null
 ...
 TestComponent: React.FC<{ props?: TProps }> = ({ props }) =} { try(;}
const hookResult = hook(props ),
      result = { current: hookResult } catch (error) { error = e as Error }return null;
  );
...
const Wrapper = options?.wrapper || AllTheProviders;
  ...
  if (typeof document === "undefined') { throw new Error("Document is not defined. Check jest configuration.") }""''""'
...
const body = document.createElement("body" );""''""'""'
  ...
  // Create container
container;
  try(container = document.createElement("div"  );'"'"'""'
    if (container && typeof container.setAttribute === "function") {'""''"""'
      container.setAttribute("data-testid', "hook-test-container") }'""""''
    document.body.appendChild(container);
  } catch (error) { // Fallback if createElement fails
    container = document.body }renderedContainers.add(container);
...
root: ReactDOM.Root | null = null
  ...
  if (!root) { throw new Error("Failed to create React root for hook test") };'""'
const finalRoot = root;
  ...
  ,
    ...
    waitForNextUpdate: async () = { await act(async () =)}{}
      ...
      while (Date.now() - startTime < timeout> {
        try {
          await act(async () =) {
            await callback() }};
          return;
  } catch (e) { await new Promise(resolve =) setTimeout(resolve, 50)} );
...
function cleanupContainers() { renderedContainers.forEach(container =) {
...
{ customRender as render, rtlRender };
{ renderWithoutProviders };
{ customRenderHook as renderHook };
{ SimpleWrapper, AllTheProviders };
...
{ MockAuthProvider,
  ...
  defaultMockContexts } from "./test-utils/mockContexts";'""''"""'

// Re-export mock context types
interface type { { { { MockAuthContextType,
  MockNotificationContextType,
  MockThemeContextType,
  AllProvidersProps } from "./test-utils/mockContexts';""''""'

// Re-export commonly used testing utilities explicitly for better TypeScript support
{ screen, fireEvent, waitFor, within, act } from "@testing-library/react";'""''""""'
...
export const createMockButtonProps = (overrides = {}) =} ({ onClick: jest.fn(),
  children: "Test Button",;'""'

$2iant: 'primary" as const,""'"'"'
  ...overrides };
export const createMockCrisisAlert = (overrides = {}) = ({ show: true,
  ...
  ...overrides });
export const createMockFormInputProps = (overrides = {}) = ({ id: "test-input',""'""'")'
  label: "Test Input',""""'
  name: 'testInput","'""""''
  value: "",'"'"""''
  onChange: jest.fn(),
  onBlur: jest.fn(),
  ...overrides });
export const createMockModalProps = (overrides = {}) = ({ isOpen: true,
  onClose: jest.fn(),
  title: "Test Modal",'"'"""''
...
export const setupTestTimers = () = { jest.useFakeTimers();}
...
export const cleanupTestTimers = () =) { jest.runOnlyPendingTimers();
  jest.clearAllTimers();
  jest.useRealTimers();
  jest.clearAllMocks(),

// Async utility for handling state updates in tests
export const waitForAsyncUpdates = async () =} { await act(async () =) {
    await new Promise(resolve =) setTimeout(resolve, 10)} };

// Timer advance utility with proper act wrapping
export const advanceTimersWithAct = (time: number) = { act(() =)}{}
    jest.advanceTimersByTime(time) }

// Helper function to ensure DOM is ready for React 18
export const ensureDOMReady = () = { if (typeof document === "undefined') {""}'""""
