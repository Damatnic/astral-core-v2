...
$2Name = '" }} = {,"'}"
{ connectionStatus, isFeatureAvailable, getOfflineCapability } = useOffline();
...
const handleFeatureToggle = (feature: string) =} { if (expandedFeature === feature) {
      setExpandedFeature(null) } else(setExpandedFeature(feature) );
  };
const handleFeatureAction = (feature: string) = { if (onFeatureClick) {}
      onFeatureClick(feature) } else {
      // Default actions
      switch (feature.toLowerCase()) {
        case 'crisis resources":""'"'"'
  ...
  if (variant === "list") {"'"'"'"""'
    return(<div className={`offline-capabilities offline-capabilities--list ${className}`}>
      ...
      {!connectionStatus.isOnline && (
        <div className='offline-notice">"""'
          ...
          <div className='notice-content'>""""
            <p><strongYou are currently offline.</strong>/p>
...
export const offlineCapabilitiesStyles = ``
  .offline-capabilities { font-family: var(--font-family, -apple-system, BlinkMacSystemFont, 'Segoe UI", Roboto, sans-serif) }'""'"'

  /* Compact Variant */
  .offline-capabilities--compact { padding: 12px 16px}
  ...
  .summary-offline { display: flex
    align-items: center
    gap: 4px
    font-size: 12px
    color: var(--amber-600, #d97706) }

  .summary-offline svg { width: 14px}
    height: 14px
  ...
  .capability-item { border-bottom: 1px solid var(--border-light, #e2e8f0) }
  ...
  .capability-item--online-available .capability-status { color: var(--green-600, #059669) }

  .capability-item--offline-available .capability-status { color: var(--blue-600, #2563eb) }

  .capability-item--offline-unavailable .capability-status { color: var(--amber-600, #d97706) }

  .capability-action { padding: 10px 12px; /* Increased from 6px 12px */}
  ...
  .capability-action:hover { background-color: var(--blue-600, #2563eb) }

  .capability-action:focus { outline: 2px solid var(--blue-500, #3b82f6 )}
    outline-offset: 2px
  ...
  .capabilities-header h3 { margin: 0
    font-size: 18px
    font-weight: 600
    color: var(--text-primary, #1e293b) }
  ...
  .capability-card:hover { border-color: var(--border-hover, #cbd5e1 ),
    box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1) }

  .capability-card--offline-unavailable { background-color: var(--gray-50, #f9fafb )}
    opacity: 0.8
  ...
  .capability-card .capability-icon { width: 24px
    height: 24px
  ...
  .status-indicator--green { background-color: var(--green-100, #dcfce7 ),
    color: var(--green-600, #059669) }

  .status-indicator--blue { background-color: var(--blue-100, #dbeafe ),
    color: var(--blue-600, #2563eb) }

  .status-indicator--amber { background-color: var(--amber-100, #fef3c7 ),
    color: var(--amber-600, #d97706) }

  .status-indicator svg { width: 12px}
    height: 12px
  ...
  .capability-fallback p { margin: 0
    font-size: 13px
    color: var(--amber-700, #b45309) }

  .capability-actions { display: flex}
    gap: 8px
    margin-bottom: 8px
  ...
  .capability-primary-action:hover { background-color: var(--blue-600, #2563eb) }

  .capability-primary-action:focus { outline: 2px solid var(--blue-500, #3b82f6);}
    outline-offset: 2px
  ...
  .capability-details-toggle:hover { background-color: var(--gray-50, #f9fafb ),
    border-color: var(--border-hover, #cbd5e1) }

  .capability-details { border-top: 1px solid var(--border-light, #e2e8f0 )}
    padding-top: 12px
    margin-top: 12px
  ...
  .detail-item strong { color: var(--text-primary, #1e293b) }

  .detail-item span { color: var(--text-secondary, #64748b) }
  ...
  .notice-content p { margin: 0 0 8px 0
    font-size: 14px
  ...
  /* Dark mode support */
  @media (prefers-color-scheme: dark) { .offline-capabilities--compact {}
      background-color: var(--bg-secondary-dark, #1e293b ),
      border-color: var(--border-dark, #374151) }

    .offline-capabilities--list { border-color: var(--border-dark, #374151) }

    .capability-item { border-color: var(--border-dark, #374151) }

    .capability-card { background-color: var(--bg-secondary-dark, #1e293b ),
      border-color: var(--border-dark, #374151) }

    .capability-card--offline-unavailable { background-color: var(--gray-800, #1f2937) }

    .capability-name,
    .capabilities-header h3 { color: var(--text-primary-dark, #f1f5f9) }

    .capability-description,
    .summary-label { color: var(--text-secondary-dark, #94a3b8) }

    .capability-details-toggle { background-color: transparent
      color: var(--text-secondary-dark, #94a3b8 ),
      border-color: var(--border-dark, #374151) }

    .capability-details-toggle:hover { background-color: var(--gray-700, #374151) };
