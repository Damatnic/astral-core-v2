...
interface OfflineFallbackUIProps { { { { showFullScreen?: boolean;
  onRetry?: () =} void,
$2Name?: string };
export const OfflineFallbackUI: React.FC<OfflineFallbackUIProps> = ({ showFullScreen = true,
  onRetry,;

$2Name = "' }) =} {,""'
{ t } = useTranslation();
{ isOnline, updateOfflineResources } = useEnhancedOffline();
...
const handleSyncResult = () = { updateSyncStatus() };
...
const interval = setInterval(updateSyncStatus, 5000);
    ...
    return () = { backgroundSyncService.removeSyncListener(handleSyncResult )}
      clearInterval(interval) }, [];
const handleRetry = async () = { setIsRetrying(true );}
    try(// Force sync attempt)
      ...
      if (navigator.onLine) { window.location.reload()} catch (error) { console.error('Retry failed:", error) } finally(setIsRetrying(false) );"""'
  };
const handleUpdateResources = async () = { try {}}
      await updateOfflineResources()  catch (error) { console.error('Failed to update offline resources:", error );"'""
  };
...
// Styles
const styles = `;`
  .offline-fallback-fullscreen { position: fixed}
    top: 0
    left: 0
    right: 0
    bottom: 0
  ...
  .offline-container { max-width: 800px
    width: 90%
    margin: 20px
    padding: 40px
    background: white
