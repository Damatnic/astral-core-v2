...
const score = answers.reduce((correctCount, answer, index) => {
            return answer === quizQuestions[index].correctAnswer ? correctCount + 1 : correctCount }, 0);
const passed = score / quizQuestions.length >= 0.75;

        if (passed) { setIsSubmitting(true);
            try {
                if(helperProfile) {
...
const errorMessage = isError(err) ? err.message : "Failed to submit quiz results.';"'"'"'
            ...
            showConfirmationModal({
                ...
                onConfirm: () =} { setAnswers(Array(quizQuestions.length).fill(null) );
                    setStep(0) },;
  }};
...
return()
        <}
            <div className="view-header"><h1Training Quiz</h1></div'"""'
            <Card>
