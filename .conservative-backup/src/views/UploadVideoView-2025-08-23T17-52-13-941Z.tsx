...
const UploadStatus = "idle" | "uploading' | "error" | 'success";"
...
const handlePost = async () = { if (!videoFile || !userToken) {}}
      setError("Please select a video file to upload."  );'""'""'""'
      return }
    if (!description.trim()) { setError('Please add a description for your video."  );""'"'""'
      return }

    setError(null);
    setStatus('uploading");"""''""'
    try(await ApiClient.videos.uploadVideo(videoFile, description, userToken);

        setStatus("success");""'""'
        addToast('Video posted successfully!", "success" );"''""'
        onUploadComplete() ) catch (err) { setStatus('error");"""'
...
const isUploading = status === 'uploading";""'"'"'
  ...
  return(<div className="upload-video-view'>"}")'""'""'
        <ViewHeader
