...
{ helpSessions } = useSessionStore();
{ createDirectRequest } = useDilemmaStore();
...
const [requestCategory, setRequestCategory] = useState(CATEGORIES[0]);
    ...
    useEffect(() =) { if (userToken) {
            ...
            ApiClient.helpers.getFavoriteHelpersDetails(userToken)
                ...
                .catch(error =) { console.error('Failed to load favorite helpers:", error  );""'"'"'
...
const handleRequestClick = (helper: Helper) = { setSelectedHelper(helper )}
        setIsModalOpen(true) ;
    ...
    return()
        <
            <Modal isOpen={isModalOpen} onClose={() =} setIsModalOpen(false)> title={`Request a chat with ${selectedHelper?.displayName}`}
             ...
             <Card>
                {
  isLoading ? <div className="loading-spinner" style= {{margin: "3rem auto'")"'}
}  / : favoriteHelpers.length >0 ? (
                    <ul style= {}
  { listStyle: "none", padding: 0 ""'"'
}>
                        {favoriteHelpers.map(helper =) ()
                            <li key={helper.id} className="setting-item' style= {""}"'
