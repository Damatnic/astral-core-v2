...
describe("wellnessStore", () =) { beforeEach(() =) {""''""'"}'
...
const mockHistory = [{ id: '1", moodScore: 4 }];"""''""'"
    ...
    await act(async () =) { await useWellnessStore.getState().fetchHistory() };
...
const checkInData = { moodScore: 5, tags: ["Grateful'] };"""'"'""'
    ...
    await act(async () =) { await useWellnessStore.getState().postCheckIn(checkInData as any) };
...
const mockCompletions = [{ habitId: 'h1', completedAt: new Date().toISOString() }];"""""
    ...
    await act(async () => { await useWellnessStore.getState().fetchHabits() });
