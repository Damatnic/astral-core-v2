...
interface AssessmentState { { { { history: Assessment[]
  isLoading: boolean
  // Actions
...
const data = await response.json(),
  ...
  } else { console.error("Failed to fetch assessment history:", response.status  );"''""'"'
                set({ history: [] });
  };
   catch (error) { console.error("Failed to fetch assessment history:", error  );"''""'"'
            set({ history: [] });
  } finally {
            set({ isLoading: false });
  };
...
${2: "phq-9","''""'"'
                ...
                culturalContext,
                sessionDuration: 300000 // 5 minutes default) else {
