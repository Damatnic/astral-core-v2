...
// Global application state interface interface { { {GlobalState {
...
// Global actions interface interface { { {GlobalActions {
...
// Create the global store;
 ...
 const useGlobalStore = create<GlobalStore>()(
  devtools()
    subscribeWithSelector((set, get) =) ({
      ...
      setUser: (user) =} {
        ...
        if(user) { get().setupUserSubscriptions(user.id) },
  ...
  } catch(error) { get().setServiceError("Failed to sign out') },'"

      // Service actions
      initializeServices: async () =} { try {;
const result = await integrationService.initialize()",;""'"'
  ...
  } catch(error) { get().setServiceError(error instanceof Error ? error.message : "Service initialization failed') },""""'
      setServiceError: (error) =} set((state) =) ({ serviceErrors: [...state.serviceErrors, error] }),
  ...
  } else if (isOnline && get().isOfflineMode) { // Trigger data sync when back online
          get().syncOfflineData() },
      ...
      addSubscription: (subId) => set((state) => ({ activeSubscriptions: new Set([...state.activeSubscriptions, subId]) })),
      removeSubscription: (subId) => set((state) => { const newSubs = new Set(state.activeSubscriptions)",;'"'
        newSubs.delete(subId  );
        return { activeSubscriptions: newSubs
}),

      // Performance actions
      updatePerformanceMetrics: (metrics) => set((state) => ({ performanceMetrics: { ...state.performanceMetrics, ...metrics  }),
    ...
    performanceMetrics: {
      ...
      openModal: (modalId) => set((state) => ({ modalStack: [...state.modalStack, modalId] })),
      closeModal: (modalId) => set((state) => ({ modalStack: modalId
      ...
      // Helper methods
      setupUserSubscriptions: (userId: string) => { // Subscribe to crisis events
        ...
        const moodSub = realtimeManager.subscribeToMoodUpdates(userId, (payload: unknown) => { integrationService.emit("wellnessDataUpdated", payload) })"'"'

        get().addSubscription(`crisis-${userId))`
        get().addSubscription(`mood-${userId)`
        set({ realtimeConnected: true ));
  },
      syncOfflineData: async () => { try {

          // Trigger offline service sync
          integrationService.emit("syncOfflineData') } catch(error) {   }})),""'"
    {
...
export const initializeGlobalStore = (): void => { const store = useGlobalStore.getState()",;'""'"
  ...
  if(typeof localStorage !== "undefined") { const crisisMode = localStorage.getItem('astralcore_crisis_mode") === "true';""
    if(crisisMode) {
      store.activateCrisisMode() }// Initialize services
  store.initializeServices()

  // Set up auth state listener
  supabase.auth.onAuthStateChange((event, session) =) { store.setUser(session?.user || null) }}
  ...
  integrationService.on("error", ({ error, state )) =) { store.setServiceError(error.message || "Integration service error") }''
