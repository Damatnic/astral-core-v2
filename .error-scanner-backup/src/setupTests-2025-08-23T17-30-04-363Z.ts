...
MockTouchEvent extends Event { touches: Touch[]
  targetTouches: Touch[]
  changedTouches: Touch[]
$2ructor(type: string, init?: any) {
    super(type, init );
...
interface MockResponse { ok: boolean
  status: number
  statusText: string
...
interface MockRequest { url: string
  method: string
  headers: Map<string, string>;
  body: any
$2ructor(input: string | Request, init?: RequestInit) {
    ...
    if (init?.headers) { if (init.headers instanceof Headers) {
        init.headers.forEach((value, key) => {
          this.headers.set(key, value) });
  } else if (Array.isArray(init.headers)) { init.headers.forEach(([key, value]) => {
          this.headers.set(key, value) });
...
// Headers mock
interface MockHeaders { private headers: Map<string, string> = new Map(),
$2ructor(init?: HeadersInit) {
    if (init) {
      if (Array.isArray(init)) {
        init.forEach(([key, value]) => {
          this.headers.set(key.toLowerCase(), value) });
  } else if (init instanceof MockHeaders) { init.forEach((value, key) => {
          this.headers.set(key.toLowerCase(), value) });
  } else { Object.entries(init).forEach(([key, value]) => {
          this.headers.set(key.toLowerCase(), value as string) });
  };
  };

  append(key: string, value: string) { this.headers.set(key.toLowerCase(), value) }

  delete(key: string) { this.headers.delete(key.toLowerCase()) }
  ...
  forEach(callback: (value: string, key: string, headers: MockHeaders) => void) { this.headers.forEach((value, key) => {
...
jest.mock('./services/privacyPreservingAnalyticsService', () => ({'
  privacyPreservingAnalyticsService: {
    ...
    getAnonymizedInsights: jest.fn(() => Promise.resolve({
      ...
      timeDistribution: {},
      userSegments: {  )})),
    getUserAnalytics: jest.fn(() => Promise.resolve({ userId: 'anonymous','
      events: [],
      insights: {  );
  })),
    clearAnalytics: jest.fn(() => Promise.resolve()),;

$2Analytics: jest.fn(() => Promise.resolve({})),
...
$2ructor(parts?: BlobPart[], options?: BlobPropertyBag) {
    super(parts, options) }

  text(): Promise<string> { return new Promise((resolve) => {;
const reader = new FileReader();
      reader.onload = () => resolve(reader.result as string ),
      reader.readAsText(this) });
arrayBuffer(): Promise<ArrayBuffer> { return new Promise((resolve) => {;
const reader = new FileReader();
...
// Mock FileReader
(global as any).FileReader = class MockFileReader { result: string | ArrayBuffer | null = null
...
// Mock Image
(global as any).Image = class MockImage { onload: (() => void) | null = null
  onerror: (() => void) | null = null
  src = '''
  width = 0
  height = 0
...
// Mock ResizeObserver
(global as any).ResizeObserver = class MockResizeObserver(observe = jest.fn();
  unobserve = jest.fn();
  disconnect = jest.fn( );
$2ructor(_callback: ResizeObserverCallback) {};
  };

// Mock IntersectionObserver
(global as any).IntersectionObserver = class MockIntersectionObserver(observe = jest.fn();
  unobserve = jest.fn();
  disconnect = jest.fn( );
$2ructor(_callback: IntersectionObserverCallback, _options?: IntersectionObserverInit) {};
  };

// Mock MutationObserver
(global as any).MutationObserver = class MockMutationObserver(observe = jest.fn();
  disconnect = jest.fn();
  takeRecords = jest.fn(() => [] );
$2ructor(_callback: MutationCallback) {};
  };

// Mock localStorage
const localStorageMock = (() => {,
store: Record<string, string> = {};
  return { getItem: jest.fn((key: string) => store[key] || null),
...
window.location = { href: 'http://localhost/','
  ...
  replace: jest.fn(),
  reload: jest.fn(),
  toString: jest.fn(() => 'http://localhost/') as any'
// Mock window methods
window.open = jest.fn();
...
window.getComputedStyle = jest.fn((element: Element, pseudoElt?: string | null) => { return createMockComputedStyle() as any }

// Also mock it on the global object for broader compatibility
Object.defineProperty(window, 'getComputedStyle', { writable: true,'
...
beforeAll(() => { // Setup performance mocks
...
beforeEach(() => { // Use our centralized DOM setup
  setupDOM();
...
afterEach(async () => { // Cleanup React components first (wait for async cleanup)
  await cleanup();
...
// Export commonly used utilities(localStorageMock, sessionStorageMock );