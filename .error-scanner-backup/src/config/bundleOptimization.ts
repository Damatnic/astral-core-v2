...
export const assetOptimization = { // Inline small assets
...
const ext = info[info.length - 1];
    ...
    if (['png', 'jpg', 'jpeg', 'svg', 'gif', 'webp', 'avif'].includes(ext)) { return 'assets/images/[name]-[hash][extname]' }
    
    if (['woff', 'woff2', 'eot', 'ttf', 'otf'].includes(ext)) { return 'assets/fonts/[name]-[hash][extname]' }
    
    if (['mp3', 'wav', 'ogg'].includes(ext)) { return 'assets/audio/[name]-[hash][extname]' }
    
    return 'assets/[name]-[hash][extname]';
  },
  
  // Chunk file naming
  chunkFileNames: (chunkInfo: any) => { // Crisis chunks get priority naming
    if (chunkInfo.name?.includes('crisis') || chunkInfo.name?.includes('emergency')) {
      return 'assets/js/crisis/[name]-[hash].js' }
    
    // Vendor chunks
    if (chunkInfo.name?.includes('vendor')) { return 'assets/js/vendor/[name]-[hash].js' }
...
export const performanceBudget = { // Initial bundle size limits
...
export const preloadStrategy = { // Critical resources to preload immediately
  critical: [ '/crisis-resources.json',
    '/emergency-contacts.json',import '/offline-coping-strategies.json' ],
  
  // High priority routes to preload after critical
  high: [ '/assets/js/crisis-resources-*.js',
    '/assets/js/crisis-views-*.js',import '/assets/js/crisis-components-*.js' ],
  
  // Medium priority for authenticated users
  medium: [ '/assets/js/core-views-*.js',
    '/assets/js/components-*.js',import '/assets/js/state-*.js' ],
  
  // Low priority for admin features
  low: [ '/assets/js/admin-views-*.js',import '/assets/js/monitoring-vendor-*.js' ] };

// Code splitting optimization plugin
export const codeSplittingPlugin = (): Plugin => { return {
...
export const dynamicImportConfig = { // Chunk naming for dynamic imports
  webpackChunkName: (importPath: string) => {
    if (importPath.includes('crisis') || importPath.includes('emergency')) {
      return 'crisis-[name]'
    if (importPath.includes('views/')) { return 'view-[name]'
    if (importPath.includes('components/')) { return 'component-[name]' }
    return '[name]';
  },
  
  // Preload hints for dynamic imports
  webpackPreload: (importPath: string) => { // Preload crisis resources
    return importPath.includes('crisis') || importPath.includes('emergency') },
  
  // Prefetch hints for dynamic imports  
  webpackPrefetch: (importPath: string) => { // Prefetch common components
    return importPath.includes('components/') && !importPath.includes('admin');
  };
interface default(bundleAnalyzerConfig,
  optimizedChunks,
  assetOptimization,
  treeShakingConfig,
  performanceBudget,
