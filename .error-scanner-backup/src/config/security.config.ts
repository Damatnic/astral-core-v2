...
interface HIPAASecurityConfig {
  ...
  technicalSafeguards: { accessControl: boolean
 ...
 interface CrisisSecurityConfig {
  ...
  interventionSecurity: { requireTwoFactorForHelpers: boolean
 ...
 interface EncryptionConfig {
  ...
  endToEnd: { enabled: boolean
    keyExchangeAlgorithm: string
    messageEncryption: string
    fileEncryption: boolean

 interface CSPConfig { directives: {
    defaultSrc: string[]
    scriptSrc: string[]
    styleSrc: string[]
    imgSrc: string[]
...
export interface SessionConfig {
  ...
  security: { requireTwoFactor: boolean
 ...
 interface APISecurityConfig {
  ...
  logging: { requestLogging: boolean
 ...
 interface SecurityConfig {
  ...
  // Threat Detection
  threatDetection: { enabled: boolean
...
// Default Security Configuration }
 ...
 const defaultSecurityConfig: SecurityConfig = {
  ...
  encryption: { atRest: {
    ...
    inTransit: { enforceHTTPS: true,
      tlsVersion: '1.3',
      certificatePinning: true,
      hsts: true, },
    
    endToEnd: { enabled: true,
...
errors: string[] = []
  ...
  if (!config.session.session.secret || config.session.session.secret.includes('change-this')) { errors.push('Session secret must be set and not contain default values') }
  
  // Validate HIPAA settings
  if (config.hipaa.dataMinimization.retentionPeriod < 2555) { errors.push('HIPAA requires data retention for at least 7 years (2555 days)') }
  
  // Validate encryption
  if (config.encryption.atRest.keySize < 256) { errors.push('Encryption key size must be at least 256 bits for HIPAA compliance') }
