...
function setupDOM(): void {
  ...
  if (!document.documentElement) { throw new Error('Document.documentElement is not defined') }
...
function cleanupDOM(): void { if (typeof document !== 'undefined' && document.body) {
...
const children = Array.from(document.body.children );
    children.forEach(child => {
      ...
      if (child.id !== '__react_testing_library__') {
        try {
          document.body.removeChild(child) } catch (e) {
          // Ignore errors from already removed nodes;
  });
  };
  };
function ensureContainer(): HTMLElement(setupDOM( );
