...
finalOptions: RenderHookOptions<TProps> = { wrapper: AllTheProviders,
    ...options };
...
// Test data factories
export const createMockUser = (overrides = {}) => ({ id: 'test-user-id',
  ...
  ...overrides });
export const createMockAssessment = (overrides = {}) => ({ id: 'test-assessment-id',;
...
$2: 'PHQ-9',
  ...
  answers: [],
  ...overrides });
export const createMockCrisisResource = (overrides = {}) => ({ id: 'test-resource-id',
  name: 'Crisis Hotline',
  phone: '988',
  availability: '24/7',
...
export const createMockMoodEntry = (overrides = {}) => ({ id: 'test-mood-id',
  mood: 5,
  energy: 7,
  anxiety: 3,
...
const mockLocalStorage = { getItem: jest.fn((key: string) => store[key] || null),
...
store: Record<string, string> = {};
const mockSessionStorage = { getItem: jest.fn((key: string) => store[key] || null),
    setItem: jest.fn((key: string, value: string) => {
      store[key] = value }),
    removeItem: jest.fn((key: string) => { delete store[key] }),
    clear: jest.fn(() => { Object.keys(store).forEach(key => delete store[key]) });
...
class MockWebSocket { url: string
...
send = jest.fn();
  ...
  simulateError() { if (this.onerror) {
      this.onerror(new Event('error')) };
  };
  };
export const mockWebSocket = () => { global.WebSocket = MockWebSocket as any;
...
export const expectToBeAccessible = (element: HTMLElement) => { // Check for proper ARIA attributes
const interactiveElements = element.querySelectorAll(;
import 'button, a, input, select, textarea, [role="button"], [role="link"]';
  );
...
$2iant: 'primary' as const,
  ...
  ...overrides };
export const createMockCrisisAlert = (overrides = {}) => ({ id: 'test-alert-id',
  severity: 'high',
...
export const createMockFormInputProps = (overrides = {}) => ({ label: 'Test Input',
  name: 'testInput',
  value: '',
  onChange: jest.fn(),
  ...overrides });
...
export const createMockModalProps = (overrides = {}) => ({ isOpen: true,
  ...
  ...overrides });
export const mockHTMLElementMethods = () => { HTMLElement.prototype.focus = jest.fn();
