...
export const MobileSidebarNav: React.FC<MobileSidebarNavProps> = React.memo(({ activeView,
  setActiveView,
  isAuthenticated,
  onLogout,
  onlineHelperCount,
  userToken,
  helperProfile}) => {,
{ isSidebarOpen, openSidebar, closeSidebar } = useSwipeNavigation();
...
const handleKeyDown = (event: KeyboardEvent) => {
  ...
  useEffect(() => { if (isSidebarOpen) {
      // Focus first interactive element in sidebar
const firstButton = sidebarRef.current?.querySelector('button'  );'
      firstButton?.focus() }, [isSidebarOpen]);

  // Prevent body scroll when sidebar is open
  useEffect(() => { if (isSidebarOpen) {
      document.body.style.overflow = 'hidden';'
      document.body.style.position = 'fixed';'
      document.body.style.width = '100%' } else { document.body.style.overflow = '';'
...
const handleSidebarNavigation = () => { closeSidebar(),
    // Return focus to hamburger menu for keyboard users
    setTimeout(() => {
      hamburgerRef.current?.focus() }, 100);
  };
      ...
      >
        <div className="mobile-sidebar-header">"
          ...
          <button className="mobile-sidebar-close touch-optimized""
            onClick={ () => {
              closeSidebar(),
              hamburgerRef.current?.focus() }aria-label="Close navigation menu";"
$2="button""
          >
        ...
        <div className="mobile-sidebar-content">"
          {isAuthenticated && helperProfile ? (
            <HelperSidebar
              ...
              onLogout={ () => {
                onLogout(),
                handleSidebarNavigation() }helperProfile={helperProfile}
