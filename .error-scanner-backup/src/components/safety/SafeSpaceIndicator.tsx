...
interface SafeSpaceIndicatorProps { isPrivateMode?: boolean;
  userName?: string;
  sessionType?: 'anonymous' | 'private' | 'public';
$2Name?: string;
  theme?: string;
  children?: React.ReactNode };
export const SafeSpaceIndicator: React.FC<SafeSpaceIndicatorProps> = ({ isPrivateMode = false,
  userName,
  sessionType = 'public',;
...
const getIndicatorText = () => { if (isPrivateMode || sessionType === 'private') {
      return '🔒 Private & Safe' }
    if (sessionType === 'anonymous') { return '👤 Anonymous Mode' }
    return '🛡️ Safe Space';
  };
...
const getBreathingDotColor = () => { if (breathingPhase === 'inhale') return 'var(--safe-success)';
...
$2terSpacing: '0.5px', }>
      ...
      <button
        ...
        style={ {
          ...
          color: 'var(--safe-gray-600)',
          transition: 'color 0.2s', }onMouseEnter={(e) => e.currentTarget.style.color = 'var(--safe-primary)'}
      ...
      {
  children && <div style={{ marginLeft: '8px' 
}>{children}</div>}
    </div>
  );
  };
