...
interface AutoSaveTextEditorProps { /** Initial content */
...
const autoSave = useAutoSave(draftId, initialContent, initialTitle, {
    ...
    onDraftLoaded: (draft) => { setContent(draft.content);
...
const [isFocused, setIsFocused] = useState(false);
  ...
  useEffect(() => { autoSave.setEnabled(enableAutoSave) }, [enableAutoSave, autoSave]);
  
  /**
   * Handle content change
   */;
const handleContentChange = (newContent: string) => { if (disabled) return;
   ...
   * Handle title change
   */;
const handleTitleChange = (newTitle: string) => { if (disabled) return;
   ...
   * Get character count
   */;
const getCharCount = (): number => { return content.length };
  
  /**
   * Get word count
   */;
const getWordCount = (): number => { if (!content.trim()) return 0;
    return content.trim().split(/\s+/).length };
  
  /**
   * Get remaining characters
   */;
const getRemainingChars = (): number => { return maxCharacters ? maxCharacters - content.length : 0 };
  
  /**
   * Get character count CSS class
   */;
const getCharCountClass = (): string => { if (!maxCharacters) return '';
...
const isNearCharLimit = content.length > maxCharacters * 0.9;
    ...
    if (isNearCharLimit) { return 'auto-save-text-editor__char-count--near-limit' }
    return '';
  };
...
const handleKeyDown = (event: React.KeyboardEvent) => { if (disabled) return;
    ...
    if ((event.ctrlKey || event.metaKey) && event.key === 'n') { event.preventDefault();
...
const handleBlur = () => { setIsFocused(false) };
   ...
   * Manual save handler
   */;
const handleManualSave = () => { autoSave.saveDraft();
const editorClasses = [;
...
className="auto-save-text-editor__content-input"
          ...
          aria-describedby={ showCharCount || showWordCount 
              ? 'editor-stats' 
              : undefined }
        />
