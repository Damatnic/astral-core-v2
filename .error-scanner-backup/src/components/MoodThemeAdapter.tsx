...
moodThemes: Record<string, ThemeConfig> = {
  excellent: {
    name: 'Joyful',
    primary: '#4CAF50',
    secondary: '#81C784',
...
selectedTheme: ThemeConfig
    if (avgMood >= 4.5) {
      selectedTheme = moodThemes.excellent } else if (avgMood >= 3.5) { selectedTheme = moodThemes.good } else if (avgMood >= 2.5) { selectedTheme = moodThemes.neutral } else if (avgMood >= 1.5) { selectedTheme = moodThemes.low } else { selectedTheme = moodThemes.struggling }
    
    if (selectedTheme.name !== currentTheme.name) { setIsTransitioning(true);
      setTimeout(() => {
...
const elements = document.querySelectorAll('.mood-adaptive');
    elements.forEach(el => { (el as HTMLElement).style.background = theme.background });
  };
  ...
  { color: currentTheme.primary 
      ...
      <p className="theme-message" style={
