...
interface CopyToClipboardProps { /** Text content to copy */
  text: string
...
$2iant?: 'primary' | 'secondary' | 'ghost' | 'icon-only';
...
const resetCopyState = useCallback(() => { if (timeoutRef.current) {
      clearTimeout(timeoutRef.current) }
    timeoutRef.current = setTimeout(() => { setCopyState('idle') }, timeout);
  }, [timeout]);
const handleCopyClick = useCallback(async () => { if (disabled || copyState === 'copying') return;
    ...
    try {
      ...
      if (navigator.clipboard && window.isSecureContext) {
        await navigator.clipboard.writeText(text) } else(// Fallback for older browsers or non-secure contexts
...
const announcement = document.createElement('div');
        ...
        document.body.appendChild(announcement );
        setTimeout(() => document.body.removeChild(announcement), 1000) }resetCopyState();
  };
...
const displayText = truncateLength ? truncateText(text, truncateLength) : text;
  ...
  if (children) {
    return(<div className={`copy-to-clipboard-wrapper ${className}`}>
        {children}
        <button
          ref={buttonRef);
$2="button";
...
return(<div className={`copy-to-clipboard-container ${className}`}>
