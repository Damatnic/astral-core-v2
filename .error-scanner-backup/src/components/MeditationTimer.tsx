...
interface MeditationTimerProps { onComplete?: () => void };
const PRESET_TIMES = [;
...
const gainNodeRef = useRef<GainNode>( };
  ...
  useEffect(() => {
    setTimeRemaining(selectedTime * 60) }, [selectedTime]);
  
  useEffect(() => { if (isRunning && timeRemaining > 0) {
  ...
  useEffect(() => { // Initialize audio context for ambient sounds
    if (typeof window !== 'undefined' && !audioContextRef.current) {
      audioContextRef.current = new (window.AudioContext || (window as any).webkitAudioContext)() }
    
    return () => { stopAmbientSound() }, []);
  
  useEffect(() => { if (isRunning && selectedSound !== 'none') {
      playAmbientSound() } else(stopAmbientSound() );
  }, [isRunning, selectedSound]);
const playAmbientSound = () => { if (!audioContextRef.current) return;
...
const playCompletionSound = () => { if (!audioContextRef.current) return;
const ctx = audioContextRef.current;
...
const resetTimer = () => { setIsRunning(false);
...
const getProgress = () => { return ((selectedTime * 60 - timeRemaining) / (selectedTime * 60)) * 100 };
...
const progress = getProgress();
  ...
  return(<div className="meditation-timer-card">
