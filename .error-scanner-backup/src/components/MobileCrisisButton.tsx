...
interface MobileCrisisButtonProps { position?: "bottom-right" | "bottom-left" | "top-right" | "top-left"
  ...
  autoExpand?: boolean };
export const MobileCrisisButton: React.FC<MobileCrisisButtonProps> = ({ position = "bottom-right",
  onEmergencyCall,
  onCrisisChat,
  onBreathingExercise,
  onGroundingExercise,
  ...
  autoExpand = false }) => {;
{ isMobile, hasNotch, touchSupport, orientation } = useResponsive();
{ getTouchTargetSize, getSafeAreaPadding } = useResponsiveStyles();
...
const triggerHapticFeedback = (type: "emergency" | "notification" | "selection"): void => { if (!navigator.vibrate || !touchSupport) return;
patterns={
 ...
 handleMainButtonPress = (): void => { triggerHapticFeedback(selection")
    setIsPressed(true)
    setTimeout(() => setIsPressed(false), 150)

    if(isExpanded) {
      setIsExpanded(false) } else(setIsExpanded(true)
      setShowPulse(false) );

  // Auto-hide after inactivity
  useEffect(() => { if (!isExpanded) return;
const timer = setTimeout(() => {
      setIsExpanded(false)
    
    return () => {
      clearTimeout(timer) }, 10000) // Hide after 10 seconds of inactivity
