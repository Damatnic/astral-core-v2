...
$2: 'call'
 ...
 CrisisAlert({
  ...
  onCrisisChat,
  userType = 'seeker';
  }: CrisisAlertProps) { const [isVisible, setIsVisible] = useState(false);
  ...
  useEffect(() => {
    if (show) {
      ...
      setTimeout(() => {
        ...
        if (alertElement) {
          alertElement.focus() }, 0);
  } else(const timer = setTimeout(() => setIsVisible(false), 300 );
      return () => clearTimeout(timer) }, [show]);
...
const handleBackdropClick = () => { if (!emergencyMode) {
      handleDismiss() };
const handleBackdropKeyDown = (event: React.KeyboardEvent) => { if (!emergencyMode && (event.key === 'Enter' || event.key === ' ')) {
...
const getContactClassName = (contactType: string) => { if (contactType === 'call') return 'crisis-alert__emergency-contact--call';
