...
$2Name = '','
  onLanguageChange }) => {,
{ language, changeLanguage, t, isRTL } = useI18n();
const [isOpen, setIsOpen] = useState(false);
const [isChanging, setIsChanging] = useState(false);
const currentLanguage = SUPPORTED_LANGUAGES.find(lang => lang.code === language) || SUPPORTED_LANGUAGES[0];
const handleLanguageChange = async (newLanguage: string) => { if (newLanguage === language) {
      setIsOpen(false ),
      return }

    setIsChanging(true);

    try(await changeLanguage(newLanguage);
...
const srElement = document.createElement('div');'
      ...
      setTimeout(() => { document.body.removeChild(srElement) }, 3000);
  } catch (error) { console.error('[LanguageSelector] Failed to change language:', error) } finally(setIsChanging(false );'
      setIsOpen(false) };
  };
