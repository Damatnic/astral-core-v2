...
interface FloatingButton { id: string
    icon: React.ReactNode
    label: string
    onClick: () => void
    badge?: number | boolean
    color?: string };
interface FloatingButtonManagerProps { onShowAIChat?: () => void };
export const FloatingButtonManager: React.FC<FloatingButtonManagerProps> = ({ onShowAIChat }) => { ;
const [isExpanded, setIsExpanded] = useState(false);
const [showHelpModal, setShowHelpModal] = useState(false);
const [showStatsModal, setShowStatsModal] = useState(false  );

    // Get stats from stores.visibleDilemmas = useDilemmaStore(),
{ reflections } = useReflectionStore();
...
const handleAIChatClick = () => { if (onShowAIChat) {
            onShowAIChat() } else {
...
const handleKeyDown = (e: KeyboardEvent) => {
    ...
    return (
        <>
            ...
            <button className="help-now-button""
                onClick={() => setShowHelpModal(true)}
                ...
                style={ {
                    ...
                    alignItems: 'center','
                    gap: '8px','
                    transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)'}onMouseEnter={ (e) => {'
                    e.currentTarget.style.transform = 'translateY(-2px) scale(1.05)';'
                    e.currentTarget.style.boxShadow = '0 8px 20px rgba(239, 68, 68, 0.4), 0 0 80px rgba(239, 68, 68, 0.2)' }onMouseLeave={ (e) => {'
  ...
  { fontSize: '1.1rem' '
            ...
            <style>{ `
                ...
                .dark-mode .help-modal { background: linear-gradient(135deg, rgba(239, 68, 68, 0.1) 0%, rgba(220, 38, 38, 0.1) 100%) }
                        ...
                        {floatingButtons.map((button, index) => (
                            <div key={button.id} className="fab-item" style={{"
                                ...
                                <button className="fab-button""
                                    ...
                                    style={
  { background: button.color || '#667eea' '
                    ...
                    <button className={isExpanded ? 'fab-main expanded' : 'fab-main'}'
