...
const InputTypeKey = keyof typeof INPUT_TYPES;
interface ValidationResult {
  isValid: boolean
  errors: string[]
  };
MobileFormInputProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, 'type'> {'
  label: string
...
export const MobileFormInput: React.FC<MobileFormInputProps> = ({ label,
  inputType = 'text','
  validation,
  helpText,
  floatingLabel = true,
...
const formatters = { tel: (value: string) => {;
const cleaned = value.replace(/\D/g, '' );'
      if (cleaned.length >= 10) {
        return cleaned.replace(/(\d[3])(\d[3])(\d[4])/, '($1) $2-$3' };'
return cleaned;
  },
    number: (value: string) => { return value.replace(/[^\d.-]/g, '') },'
...
patternMessages: Record<string, string> = {
      ...
      // Security validation
const securityResult = securityService.validateInput(value, { required: validation.required,
        minLength: validation.minLength,
        maxLength: validation.maxLength,
        pattern: validation.pattern}

      if (!securityResult.isValid) { errors.push(...securityResult.errors) }

      // Custom validation
      if (validation.custom) { ;
const customResult = validation.custom(value  );
        if (typeof customResult === 'string') {'
          errors.push(customResult) } else if (!customResult) {
          errors.push(`${label} is invalid`);
  };
  };

    return { isValid: errors.length === 0,
      errors}, [validation, label, inputType, securityService]);

  // Debounced validation
  useEffect(() => { if (!isTouched) return;
const timeoutId = setTimeout(() => {
      setIsValidating(true);
const result = validateInput(localValue as string );
      setValidationResult(result),
      onValidationChange?.(result.isValid, result.errors };
      setIsValidating(false) }, 300);
...
const newValue = e.target.value,
    ...
    if (formatValue) { newValue = formatValue(newValue) }
...
const handleFocus = (e: React.FocusEvent<HTMLInputElement>) => { setIsFocused(true);
...
const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => { setIsFocused(false);
    setIsTouched(true);
...
const containerClasses = [;
    ...
    isFocused ? 'focused' : '','
    containerClassName].filter(Boolean).join(' ');'
const inputClasses = [;
    'mobile-form-input',;'

$2Name,
    hasErrors ? 'error' : '','
    validationResult.isValid && isTouched ? 'valid' : ''].filter(Boolean).join(' ');'
...
className={inputClasses}
          ...
          aria-describedby={[
            hasErrors ? errorId : '','
            helpText ? helpId : ''].filter(Boolean).join(' ') || undefined}'
          style={
  {
            fontSize: '16px', // Prevent zoom on iOS{...inputConfig}'
          {...rest}
         />
...
MobileFormTextAreaProps extends React.TextareaHTMLAttributes<HTMLTextAreaElement> {
  label: string
  validation?: {
    required?: boolean;
    minLength?: number;
...
export const MobileFormTextArea: React.FC<MobileFormTextAreaProps> = ({ label,
  validation,
  helpText,
  floatingLabel = true,
...
const adjustHeight = useCallback(() => {
  ...
  useEffect(() => { adjustHeight() }, [localValue, adjustHeight]);
...
const customResult = validation.custom(value  );
    ...
    return { isValid: errors.length === 0,
      errors}, [validation, label, securityService]);
...
const result = validateTextArea(localValue as string );
      setValidationResult(result ),
      onValidationChange?.(result.isValid, result.errors) }, 300);
...
const newValue = e.target.value;
    setLocalValue(newValue ),
    onChange?.(e) };
const handleFocus = (e: React.FocusEvent<HTMLTextAreaElement>) => { setIsFocused(true);
...
const handleBlur = (e: React.FocusEvent<HTMLTextAreaElement>) => { setIsFocused(false);
    setIsTouched(true);
...
className={`mobile-form-textarea ${className}`}
          ...
          style={ {
            ...
            minHeight: autoResize ? '44px' : undefined}maxLength={maxLength}'
          {...rest}
         />
...
MobileFormSelectProps extends React.SelectHTMLAttributes<HTMLSelectElement> {
  label: string
...
export const MobileFormSelect: React.FC<MobileFormSelectProps> = ({ label,
  options,
  validation,
  helpText,
  floatingLabel = true,
...
const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => { onChange?.(e);
...
const handleFocus = (e: React.FocusEvent<HTMLSelectElement>) => { setIsFocused(true ),
    onFocus?.(e) };
const handleBlur = (e: React.FocusEvent<HTMLSelectElement>) => { setIsFocused(false);
    setIsTouched(true);
const result = validateSelect(e.target.value );
    setValidationResult(result),
    onValidationChange?.(result.isValid, result.errors };
...
className={`mobile-form-select ${className}`}
          ...
          style={
  {
            fontSize: '16px', // Prevent zoom on iOS{...rest}'
        >
...
interface MobileFormContainerProps { children: React.ReactNode
  onSubmit?: (e: React.FormEvent) => void
  onValidationChange?: (isValid: boolean) => void
$2Name?: string
  autoValidate?: boolean };
export const MobileFormContainer: React.FC<MobileFormContainerProps> = ({ children,
  onSubmit,
  onValidationChange,;
...
const newMap = new Map(prev),
  ...
  useEffect(() => { if (autoValidate && onValidationChange) {;
const allValid = Array.from(fieldValidations.values()).every(valid => valid ),
      onValidationChange(allValid && fieldValidations.size > 0) }, [fieldValidations, autoValidate, onValidationChange]);
...
export const MobileFormValidation = { email: {
    required: true,
...
export const MobileFormComponentsBundle = { MobileFormInput,
  MobileFormTextArea,
  MobileFormSelect,
  MobileFormContainer,
