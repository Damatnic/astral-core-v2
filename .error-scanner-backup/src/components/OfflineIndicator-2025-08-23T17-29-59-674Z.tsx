...
const CrisisIndicator = () => {
  ...
  if (variant === 'minimal') {'
    return (
      <div className={`offline-indicator offline-indicator--minimal ${className}`}
        onClick={onClick}
  ...
  if (variant === 'detailed') {'
    return (
      <div className={`offline-indicator offline-indicator--detailed ${className}`}
        onClick={onClick}
        role={onClick ? 'button' : undefined}'
        tabIndex={onClick ? 0 : undefined}
        onKeyDown={ onClick ? (e) => {
...
export const offlineIndicatorStyles = `
  .offline-indicator { position: relative
    font-family: var(--font-family, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif) }'

  /* Minimal Variant */
  .offline-indicator--minimal { display: inline-flex
    align-items: center
    cursor: pointer
  ...
  .status-indicator--green .status-dot { background-color: var(--green-500, #10b981 ),
    box-shadow: 0 0 0 2px var(--green-100, #dcfce7) }

  .status-indicator--amber .status-dot { background-color: var(--amber-500, #f59e0b ),
    box-shadow: 0 0 0 2px var(--amber-100, #fef3c7) }

  .status-indicator--red .status-dot { background-color: var(--red-500, #ef4444 ),
    box-shadow: 0 0 0 2px var(--red-100, #fee2e2) }

  .crisis-dot { position: absolute
    top: -2px
    right: -2px
    width: 4px
    height: 4px
  ...
  /* Detailed Variant */
  .offline-indicator--detailed { display: flex
  ...
  .offline-indicator--detailed:hover { background-color: var(--bg-hover, #f1f5f9 ),
  ...
  .status-content--green .status-icon { color: var(--green-600, #059669) }

  .status-content--amber .status-icon { color: var(--amber-600, #d97706) }

  .status-content--red .status-icon { color: var(--red-600, #dc2626) }
  ...
  .crisis-status--available { background-color: var(--blue-50, #eff6ff );
    color: var(--blue-700, #1d4ed8 ),
    border: 1px solid var(--blue-200, #bfdbfe) }

  .crisis-status--unavailable { background-color: var(--red-50, #fef2f2 );
  ...
  /* Banner Variant */
  .offline-indicator--banner { width: 100%
    border-radius: 8px
    overflow: hidden
  ...
  .banner-content--green { background-color: var(--green-50, #f0fdf4 ),
    border: 1px solid var(--green-200, #bbf7d0) }

  .banner-content--amber { background-color: var(--amber-50, #fffbeb ),
    border: 1px solid var(--amber-200, #fde68a) }

  .banner-content--red { background-color: var(--red-50, #fef2f2 ),
    border: 1px solid var(--red-200, #fecaca) }
  ...
  .service-worker-status { display: flex
    align-items: center
    gap: 4px
    font-size: 12px
    color: var(--text-secondary, #64748b) }

  .sw-indicator { width: 6px
    height: 6px
  ...
  .banner-action:hover { background-color: var(--bg-hover, #f8fafc ),
    border-color: var(--border-hover, #cbd5e1) }

  .banner-action:focus { outline: 2px solid var(--blue-500, #3b82f6);
    outline-offset: 2px
  ...
  /* Dark mode support */
  @media (prefers-color-scheme: dark) { .offline-indicator--detailed {
      background-color: var(--bg-secondary-dark, #1e293b  );
      border-color: var(--border-dark, #374151) }

    .offline-indicator--detailed:hover { background-color: var(--bg-hover-dark, #334155 ),
      border-color: var(--border-hover-dark, #4b5563) }

    .status-label { color: var(--text-primary-dark, #f1f5f9) }

    .status-description { color: var(--text-secondary-dark, #94a3b8) }

    .banner-action { background-color: var(--bg-secondary-dark, #1e293b );
      border-color: var(--border-dark, #374151 ),
      color: var(--text-primary-dark, #f1f5f9) }

    .banner-action:hover { background-color: var(--bg-hover-dark, #334155) };
