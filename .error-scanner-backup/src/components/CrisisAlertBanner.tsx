...
export const CrisisAlertBanner: React.FC<{ show?: boolean;
  severity?: 'low' | 'medium' | 'high';
  onClose?: () => void }> = ({ show = true, severity = 'medium', onClose }) => {;
const [isVisible, setIsVisible] = useState(show);
const resources = getCrisisResources();
  
  useEffect(() => {
    setIsVisible(show) }, [show]);
const handleClose = () => { try(setIsVisible(false );
      onClose?.() } catch (error) { console.error('Error closing crisis alert:', error );
...
const handleExpand = () => {
    try {
      setIsExpanded(true) } catch (error) { console.error('Error expanding crisis help:', error);
  };
const handleClose = () => { try {
      setIsExpanded(false) } catch (error) { console.error('Error closing crisis help:', error);
  };
