...
interface ScreenReaderAnnouncementProps { message: string
  priority?: 'polite' | 'assertive''
  clearOnUnmount?: boolean };
const ScreenReaderAnnouncement: React.FC<ScreenReaderAnnouncementProps> = ({ message,
  priority = 'polite','
  clearOnUnmount = true }) => {,
...
const timeoutId = setTimeout(() => {
  ...
  useEffect(() => { return () => {
      if (clearOnUnmount && announcementRef.current) {
        announcementRef.current.textContent = '' }, [clearOnUnmount]);'
...
const announcement = document.createElement('div');'
    ...
    setTimeout(() => { if (document.body.contains(announcement)) {
...
export const AnnouncementContext = React.createContext<{ announce: (message: string, priority?: 'polite' | 'assertive') => void }>({'
  announce: () => {};
  };
...
{ announce } = useScreenReaderAnnouncement();
...
interface StateAnnouncementProps { state: 'loading' | 'success' | 'error' | 'idle''
  loadingMessage?: string
  successMessage?: string
  errorMessage?: string
  previousState?: string };
export const StateAnnouncement: React.FC<StateAnnouncementProps> = ({ state,
