...
export const ThemeCustomizationDashboard: React.FC<ThemeCustomizationDashboardProps> = ({ onThemeChange,;
...
$2Theme,
    importTheme,
    getTherapeuticRecommendations } = useTheme();
...
const handleThemeSelect = useCallback((theme: TherapeuticTheme) => { setTherapeuticTheme(theme ),
    onThemeChange?.(theme) }, [setTherapeuticTheme, onThemeChange]);

  // Handle color override
const handleColorChange = useCallback((colorKey: string, color: string) => { setColorOverride(colorKey as keyof typeof currentColors, color ),
    setColorPickerOpen(null) }, [setColorOverride, currentColors]);
...
const success = importTheme(importData);
    if (success) {
      setImportData(''  );'
      alert('Theme imported successfully!') } else { alert('Failed to import theme. Please check the format.') }, [importTheme, importData]);'

  // Handle theme export
const handleExportTheme = useCallback(() => { ;
const themeData = exportTheme();
    navigator.clipboard.writeText(themeData).then(() => {
      alert('Theme data copied to clipboard!') }).catch(() => { alert('Failed to copy theme data. Please copy manually from the console.'  );'
