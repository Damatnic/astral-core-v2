...
import { comprehensivePerformanceMonitor,
  ...
  AlertSeverity  } from '../services/comprehensivePerformanceMonitor';'
...
export const ComprehensivePerformanceMonitorComponent: React.FC<ComprehensivePerformanceMonitorProps> = ({ showDetails = false,
...
const handleAlertUpdate = useCallback((alert: PerformanceAlert) => { setAlerts(prev => {;
const updated = [...prev];
...
unsubscribeAlert: (() => void) | undefined
    if (enableRealTimeAlerts) {
      unsubscribeAlert = comprehensivePerformanceMonitor.onAlert(handleAlertUpdate) }

    return () => { clearInterval(interval ),
      unsubscribeAlert?.() }, [updatePerformanceData, enableRealTimeAlerts, handleAlertUpdate]);
          ...
          <div className="metrics-grid">"
            <div className="metric-card">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.firstContentfulPaint, 1500, 3000)
}>
            ...
            <div className="metric-card">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.largestContentfulPaint, 2500, 4000)
}>
            ...
            <div className="metric-card">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.firstInputDelay, 50, 100)
}>
            ...
            <div className="metric-card">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.cumulativeLayoutShift, 0.05, 0.1)
}>
        ...
        <div className="metrics-section">"
          ...
          <div className="metrics-grid">"
            <div className="metric-card crisis-metric">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.crisisDetectionResponseTime, 100, 300)
}>
            ...
            <div className="metric-card">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.chatMessageLatency, 200, 500)
}>
        ...
        <div className="metrics-section">"
          ...
          <div className="metrics-grid">"
            <div className="metric-card">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.bundleSize, 500000, 1000000)
}>
            ...
            <div className="metric-card">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.memoryUsage, 50, 100)
}>
            ...
            <div className="metric-card">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.loadTime, 2000, 4000)
}>
        ...
        <div className="metrics-section">"
          ...
          <div className="metrics-grid">"
            ...
            <div className="metric-card">"
              ...
              <span className="metric-value""
                style={
  { color: getMetricStatusColor(metrics.accessibilityScore, 85, 70)
}>
...
export const comprehensivePerformanceMonitorStyles = `
  .comprehensive-performance-monitor { font-family: var(--font-family, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif);'
  ...
  .performance-grade { text-align: center
    margin-bottom: 24px
    padding: 20px
  ...
  .alert-critical { background-color: var(--red-50, #fef2f2 ),
    border-left-color: var(--red-500, #ef4444) }

  .alert-high { background-color: var(--orange-50, #fff7ed ),
    border-left-color: var(--orange-500, #f97316) }

  .alert-medium { background-color: var(--amber-50, #fffbeb ),
    border-left-color: var(--amber-500, #f59e0b) }

  .alert-header { display: flex
    align-items: center
    gap: 8px
    margin-bottom: 8px

  .alert-severity { font-size: 12px
    font-weight: 600
    padding: 2px 6px
    border-radius: 4px
  ...
  .alert-description { margin: 0 0 8px 0
    color: var(--text-primary, #1e293b ),
    font-size: 14px

  .alert-recommendations { font-size: 13px
    color: var(--text-secondary, #64748b) }

  .alert-recommendations ul { margin: 4px 0 0 0
    padding-left: 16px
  ...
  .crisis-metric { border: 2px solid var(--red-200, #fecaca ),
...
$2ter-spacing: 0.5px
  ...
  .priority-critical { border-left: 4px solid var(--red-500, #ef4444 ),
    background-color: var(--red-50, #fef2f2) }

  .priority-high { border-left: 4px solid var(--orange-500, #f97316 ),
    background-color: var(--orange-50, #fff7ed) }

  .priority-medium { border-left: 4px solid var(--amber-500, #f59e0b ),
...
$2ter-spacing: 0.5px
  ...
  .priority-badge.priority-critical { background-color: var(--red-100, #fee2e2 ),
    color: var(--red-800, #991b1b) }

  .priority-badge.priority-high { background-color: var(--orange-100, #ffedd5 ),
    color: var(--orange-800, #9a3412) }

  .priority-badge.priority-medium { background-color: var(--amber-100, #fef3c7 ),
    color: var(--amber-800, #92400e) }

  .recommendation-description { margin: 0 0 12px 0
    color: var(--text-primary, #1e293b ),
    font-size: 14px
    line-height: 1.5

  .recommendation-details { display: flex
    flex-direction: column
    gap: 8px
    font-size: 13px
    color: var(--text-secondary, #64748b) }

  .recommendation-implementation { margin-top: 12px
    padding-top: 12px
    border-top: 1px solid var(--border-light, #e2e8f0 ),
    font-size: 13px
    color: var(--text-secondary, #64748b) }

  .trends-section { padding-top: 24px
    border-top: 1px solid var(--border-light, #e2e8f0) }

  .trends-section h4 { margin: 0 0 16px 0
    color: var(--text-primary, #1e293b);
    font-size: 16px
    font-weight: 600
  ...
  /* Dark mode support */
  @media (prefers-color-scheme: dark) { .comprehensive-performance-monitor {
      background-color: var(--bg-primary-dark, #1e293b ),
      border-color: var(--border-dark, #374151) }

    .metric-card,
    .recommendation-item { background-color: var(--bg-secondary-dark, #1e293b ),
      border-color: var(--border-dark, #374151) }

    .performance-grade { background: linear-gradient(135deg, var(--primary-900, #1e3a8a), var(--primary-800, #1e40af)) }

    .grade-display { color: var(--primary-300, #93c5fd) }

    .metric-label { color: var(--text-secondary-dark, #94a3b8) }

    .metric-value { color: var(--text-primary-dark, #f1f5f9) };
