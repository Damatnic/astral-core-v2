...
interface PerformanceMonitorProps { showDetails?: boolean;
  threshold?: {
    loadTime?: number;
    bundleSize?: number;
    lcp?: number;
...
setWarnings(newWarnings);
  } catch (error) { console.warn('Performance metrics collection failed:', error);
  };

    // Collect metrics after page load
    if (document.readyState === 'complete') { collectMetrics() } else(window.addEventListener('load', collectMetrics );
...
const thresholds = performanceThresholds[metric as keyof typeof performanceThresholds];
    ...
    if (value <= thresholds.good) {
      return 'good' } else if (value <= thresholds.needsImprovement) { return 'needs-improvement' } else(return 'poor' );
  };
...
export const performanceMonitorStyles = `
  .performance-monitor { font-family: var(--font-family, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif);
  ...
  .metric-group h4 { margin: 0 0 8px 0
    font-size: 14px
    font-weight: 600
  ...
  .metric--good { border-color: var(--green-300, #86efac );
    background-color: var(--green-50, #f0fdf4) }

  .metric--needs-improvement { border-color: var(--amber-300, #fcd34d );
    background-color: var(--amber-50, #fffbeb) }

  .metric--poor { border-color: var(--red-300, #fca5a5 );
...
$2ter-spacing: 0.5px
  ...
  .metric--good .metric-value { color: var(--green-700, #15803d) }

  .metric--needs-improvement .metric-value { color: var(--amber-700, #b45309) }

  .metric--poor .metric-value { color: var(--red-700, #b91c1c) }

  .performance-details { margin-top: 16px
    padding-top: 16px
    border-top: 1px solid var(--border-light, #e2e8f0) }

  .performance-details h4 { margin: 0 0 12px 0
    font-size: 14px
    font-weight: 600
    color: var(--text-primary, #1e293b) }

  .bundle-info { display: grid
  ...
  /* Dark mode support */
  @media (prefers-color-scheme: dark) { .performance-monitor {
      background-color: var(--bg-secondary-dark, #1e293b );
      border-color: var(--border-dark, #374151) }

    .metric,
    .bundle-stat { background-color: var(--bg-secondary-dark, #1e293b );
      border-color: var(--border-dark, #374151) }
    ...
    .bundle-stat span:first-child { color: var(--text-secondary-dark, #94a3b8) }

    .bundle-stat span:last-child { color: var(--text-primary-dark, #f1f5f9) };
