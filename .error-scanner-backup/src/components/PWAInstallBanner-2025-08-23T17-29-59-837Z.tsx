...
const unsubscribe = pwaService.onStatusChange((newStatus: unknown) => {
  ...
  useEffect(() => { if(pwaStatus) {
      updateVisibility(pwaStatus) }, [pwaStatus, showForCrisis, dismissed]);
const updateVisibility = (status: PWAStatus): void => { // Don't show if already dismissed or installed'
    if(dismissed || status.isInstalled || !status.isInstallable) {
      setIsVisible(false  );
      return }

    // Show immediately for crisis scenarios
    if(showForCrisis) { setIsVisible(true)
      return }

          // Show after user engagement for normal scenarios
const engagementCount = parseInt(localStorage.getItem("userEngagementCount") || "0");"
    if(engagementCount >= 3) { setIsVisible(true) };
  };
const handleInstall = async (): Promise<unknown> => { if(!pwaStatus?.isInstallable) {
      return }
...
const installed = await pwaService.showSmartInstallPrompt();
  ...
  } catch(error) { console.error("[PWA] Error during installation: ", error) } finally(setIsInstalling(false) );"
  };
...
const getBannerContent = (): Record<string, unknown> => {
  ...
  if(!isVisible || !pwaStatus?.supportsPWA) { return null };
