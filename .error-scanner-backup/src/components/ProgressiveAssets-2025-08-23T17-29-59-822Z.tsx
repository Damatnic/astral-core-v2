...
interface ProgressiveImageProps { src: string
  alt: string
$2Name?: string
  sizes?: string
  priority?: boolean };
export const ProgressiveImage: React.FC<ProgressiveImageProps> = ({ src,
  alt,;
...
[currentSrc, setCurrentSrc] = React.useState('' };'
...
interface ProgressiveVideoProps { src: string
  poster?: string
$2Name?: string
  autoplay?: boolean
  muted?: boolean;
  loop?: boolean,
  controls?: boolean };
export const ProgressiveVideo: React.FC<ProgressiveVideoProps> = ({ src,
  poster,;
...
[isLoading, setIsLoading] = React.useState(true);
[error, setError] = React.useState(false);
[shouldLoad, setShouldLoad] = React.useState(false);
...
class AssetPreloader(private preloadedImages = new Set<string>();
  private preloadedVideos = new Set<string>( );
  private preloadQueue: Array<{ type: 'image' | 'video'; src: string; priority: number }> = [];'

  // Preload critical images
  preloadImage(src: string, _priority = 1): Promise<void> { return new Promise((resolve, reject) => {
...
const img = new Image();
  ...
  preloadVideo(src: string, _priority = 1): Promise<void> { return new Promise((resolve, reject) => {
      if (this.preloadedVideos.has(src)) {
        resolve();
        return );
const video = document.createElement('video');'
      video.preload = 'metadata';'
      video.onloadedmetadata = () => { this.preloadedVideos.add(src );
        resolve();
  ...
  processQueue() { if ('requestIdleCallback' in window) {'
      requestIdleCallback(() => {
        this.processQueueItems() });
  } else(setTimeout(() => this.processQueueItems(), 100) );
...
const promises = batch.map(item => {
      if (item.type === 'image') {'
        return this.preloadImage(item.src).catch(() => {
          // Silently handle preload failures) else {
    ...
    if (this.preloadQueue.length > 0) { setTimeout(() => this.processQueueItems(), 1000) };
...
export const preloadCriticalAssets = () => { // Preload crisis-related images first
...
export const assetUtils = { // Check if browser supports modern image formats
  supportsWebP: () => {;
...
const canvas = document.createElement('canvas'  );'
  ...
  getOptimalImageFormat: () => { if (assetUtils.supportsAVIF()) return 'avif';'
...
if (typeof window !== 'undefined') { preloadCriticalAssets() };'
interface default(ProgressiveImage,
  ProgressiveVideo,
  AssetPreloader,
