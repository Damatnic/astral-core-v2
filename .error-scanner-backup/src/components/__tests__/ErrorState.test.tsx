...
const mockOnRetry = jest.fn();
  ...
  describe('Rendering', () => { it('should render with default props', () => {
      render(<ErrorState />);
...
{ container } = render(<ErrorState className="custom-error" />);
...
const button = screen.getByRole('button');
  ...
  describe('Accessibility', () => { it('should have role="alert"', () => {
      render(<ErrorState /> );
      
      expect(screen.getByRole('alert')).toBeInTheDocument() });

    it('should have aria-live="polite"', () => { render(<ErrorState />);
...
{ container } = render(<ErrorState onRetry={mockOnRetry} />);
...
{ container } = render(<ErrorState showRetry={false} />);
...
const paths = container.querySelectorAll('svg path');
  ...
  describe('Edge Cases', () => { it('should handle empty title', () => {
...
{ container } = render(<ErrorState message="" />);
...
{ container } = render(<ErrorState className={undefined} />);
...
const children = Array.from(errorState?.children || []);
    ...
    it('should have correct heading level for title', () => { render(<ErrorState title="Error Title" /> );
const title = screen.getByRole('heading', { level: 3 });
      expect(title).toHaveTextContent('Error Title');
  });

    it('should use paragraph element for message', () => {;
{ container } = render(<ErrorState message="Error message" />);
const message = container.querySelector('.error-state-message');
      expect(message?.tagName.toLowerCase()).toBe('p');
  });
  });

  describe('Default Export', () => { it('should export ErrorState as default', async () => {;
