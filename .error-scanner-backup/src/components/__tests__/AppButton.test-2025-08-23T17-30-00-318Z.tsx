...
const icon = <span data-testid="test-icon">🔍</$1>const props = createMockButtonProps({ icon, children: 'Search' });'
      render(<AppButton {...props}  />);
...
const icon = <span data-testid="icon">🔍</$1>const props = createMockButtonProps({"
        icon,
        iconOnly: true,
...
const button = screen.getByRole('button');'
  ...
  } else if (variant === 'ghost') { // Ghost variant just gets passed through as is'
...
const button = screen.getByRole('button');'
  ...
  } else(// 'md' is default, so no size class should be applied'
...
const icon = <span data-testid="icon">🔍</$1>const complexChildren = (;"
        <>
          <span>Should</span>
          <span>Not</span>
          <span>Appear</span>
