...
interface AIChatStatusProps { userId?: string;
$2Name?: string };
...
const checkProviderStatus = async () => { setIsChecking(true);
    try {;
...
providerStatuses: ProviderStatus[] = [
        ...
        setConnectionStatus('connected') } else(setConnectionStatus('error') );
  } catch (error) { console.error('Failed to check provider status:', error );
      setConnectionStatus('error') } finally(setIsChecking(false) );
  };
