...
$2: 'breathing' | 'grounding' | 'connection' | 'affirmation''
  icon: React.ReactNode
  steps: ExperienceStep[]
  };
interface ExperienceStep { instruction: string
...
interface GuidedTetherExperienceProps { onExperienceComplete?: (experienceId: string) => void
  partnerName?: string
  isConnected?: boolean };
export const GuidedTetherExperience: React.FC<GuidedTetherExperienceProps> = ({ onExperienceComplete,
...
const newProgress = prev + (100 / (currentStep.duration * 10) }; // Update every 100ms
        if (newProgress >= 100) {
          ...
          if (currentStepIndex < selectedExperience.steps.length - 1) {
            ...
            return 0 } else(// Experience complete
            completeExperience( );
            return 100 };
  ...
  useEffect(() => { if (!selectedExperience) return;
const completedSteps = currentStepIndex;
const totalSteps = selectedExperience.steps.length;
const currentStepCompletion = stepProgress / 100,

    setTotalProgress(((completedSteps + currentStepCompletion) / totalSteps) * 100) }, [currentStepIndex, stepProgress, selectedExperience]);
const startExperience = (experience: GuidedExperience) => { setSelectedExperience(experience);
    setCurrentStepIndex(0);
    setStepProgress(0 );
    setTotalProgress(0 ),
    setIsActive(true) };
const pauseExperience = () => { setIsActive(false) };
const resumeExperience = () => { setIsActive(true) };
const stopExperience = () => { setIsActive(false);
    setSelectedExperience(null);
    setCurrentStepIndex(0 );
    setStepProgress(0 ),
    setTotalProgress(0) };
const completeExperience = () => { if (selectedExperience) {
      onExperienceComplete?.(selectedExperience.id) }
    setIsActive(false);
    setTimeout(() => { setSelectedExperience(null);
      setCurrentStepIndex(0 );
      setStepProgress(0 ),
      setTotalProgress(0) }, 2000);
  };
const getVisualElement = (visual?: string) => {
    switch (visual) {
      case 'inhale':'
        return <div className="breath-circle expanding">Inhale</$1>case 'hold':'
        return <div className="breath-circle holding">Hold</$1>case 'exhale':'
        return <div className="breath-circle contracting">Exhale</$1>case 'rest':'
        return <div className="breath-circle resting">Rest</$1>case 'pulse':'
        return <div className="energy-pulse outgoing">💝</$1>case 'glow':'
        return <div className="energy-glow incoming">✨</$1>case 'message':'
        return <div className="affirmation-bubble">💬</$1>default: return null"
  };
  };

  if (!selectedExperience) {
    return(<div className="guided-experience-selector">"
...
const currentStep = selectedExperience.steps[currentStepIndex];
  ...
  return(<div className="guided-experience-player">"
