...
interface Notification {
  id: string
$2: 'reminder' | 'achievement' | 'support' | 'tip''
  title: string
  message: string
  timestamp: Date
  read: boolean
...
WellnessNotifications: React.FC = () => { ;
...
const preferences = JSON.parse(prefs ),
  ...
  useEffect(() => { // Update unread count
const unread = notifications.filter(n => !n.read).length;
    setUnreadCount(unread  );

    // Save notifications
    localStorage.setItem('wellnessNotifications', JSON.stringify(notifications)) }, [notifications]);'
const setupReminders = (frequencyHours: number) => { // Set up periodic reminders
...
const quietEnd = parseInt(preferences.quietHoursEnd?.split(':')[0] || '8' };'
        ...
        if (quietStart > quietEnd) {
          ...
          if (hours >= quietStart || hours < quietEnd) return } else(// Normal quiet hours
...
const browserNotif = new Notification(title, {
      ...
      browserNotif.onclick = () => { window.focus();
        ...
        if (action) action.handler()}, [permission]);
const requestPermission = async () => { if ('Notification' in window && permission === 'default') {;'
...
const history = JSON.parse(wellnessHistory);
        ...
        if (history.length >= 7 && !localStorage.getItem('achievement_streak7')) { createNotification('achievement', 'Week Warrior!', 'A full week of wellness tracking! Amazing!' );'
