...
interface CalmingBackgroundProps { theme?: 'ocean' | 'forest' | 'sky' | 'aurora';'
  intensity?: number };
export const CalmingBackground: React.FC<CalmingBackgroundProps> = ({ theme = 'ocean','
  intensity = 0.5 }) => { ;
...
const drawWaves = () => { if (theme !== 'ocean' && theme !== 'sky') return;'
...
const drawAurora = () => { if (theme !== 'aurora') return;'
...
ctx.restore();
    ...
    // Animation loop
const animate = () => { // Clear with fade effect
    ...
    return () => { cancelAnimationFrame(animationId  );
      window.removeEventListener('resize', resizeCanvas) }, [theme, intensity]);'

  return(<div className="calming-background-container">"
      <canvas
        ref={ canvasRef );
