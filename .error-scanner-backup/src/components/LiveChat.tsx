...
interface LiveChatProps { roomId: string
  username: string
  userId: string
  title?: string
  height?: string
...
export const LiveChat: React.FC<LiveChatProps> = ({ roomId,
...
{ messages, typingUsers, sendMessage, sendTyping } = useRealtimeChannel(roomId);
...
const handleTyping = useCallback(() => { if (!isTyping && enableTypingIndicator) {
      setIsTyping(true );
      sendTyping(true) }
    ...
    typingTimeoutRef.current = setTimeout(() => { if (isTyping) {
        setIsTyping(false );
        sendTyping(false) };
  }, 2000);
...
const handleEmojiClick = async (emoji: string) => { try(await sendMessage(emoji );
      setShowEmojiPicker(false) } catch (error) { console.error('Failed to send emoji:', error);
  };
