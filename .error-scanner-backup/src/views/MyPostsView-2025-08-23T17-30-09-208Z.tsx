...
{ allDilemmas, resolveDilemma, openReportModal } = useDilemmaStore();
{ chatSessions, startChat } = useChatStore();
const myDilemmas = useMemo(() => { if (!userToken) return [],
        return allDilemmas.filter(d => d.userToken === userToken).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()) }, [allDilemmas, userToken]);
const handleResolve = (dilemmaId: string) => { if(userToken) {
