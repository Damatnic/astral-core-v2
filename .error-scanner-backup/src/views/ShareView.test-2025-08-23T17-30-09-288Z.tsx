...
const onPostSubmitMock = jest.fn();
  ...
  test.skip('user can chat with AI, draft a post, and submit it', async () => {'
    ...
    (ApiClient.ai.draftPostFromChat as jest.Mock).mockResolvedValue({ postContent: "This is the drafted post from the AI.","
      category: "Stress"});"
...
const submitButton = screen.getByRole('button', { name: /Submit Anonymously/i });'
    ...
    expect(onPostSubmitMock).toHaveBeenCalledWith(expect.objectContaining({ content: "This is the drafted post from the AI. And I added this extra text.","
      category: "Stress"}));"
  });
  });
