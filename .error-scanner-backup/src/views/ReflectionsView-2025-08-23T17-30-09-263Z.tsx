...
const getFilteredReflections = () => { let filtered = [...reflections];
        switch (activeFilter) {
            case 'popular':'
...
const handleSubmit = async (e: React.FormEvent) => { e.preventDefault();
        ...
        try(const posted = await ApiClient.reflections.postReflection(userToken, newReflection.trim());
            setReflections(prev => [posted, ...prev]  );
            setNewReflection('') } catch(err) { console.error(err  );'
            alert("Could not post your reflection. Please try again.") } finally(setIsSubmitting(false) );"
const handleReact = (reflectionId: string, reactionType: string) => { if (!userToken) {
...
const filteredReflections = getFilteredReflections();
  ...
  opacity: 1
...
}.reflections-header { text-align: center
                    padding: 3rem 2rem
                ...
                @keyframes float { 0%, 100% { transform: translate(0, 0) rotate(0deg) }
                    33% { transform: translate(30px, -30px) rotate(120deg) }
                    66% { transform: translate(-20px, 20px) rotate(240deg) }.reflections-header h1 { font-size: 2.5rem
                ...
                .prompt-banner { background: linear-gradient(135deg, rgba(168, 85, 247, 0.1) 0%, rgba(236, 72, 153, 0.1) 100% };
                    border-radius: 12px
                    padding: 1rem 1.5rem
                ...
                @keyframes fadeInOut { 0%, 90% { opacity: 1 }
                    95%, 100% {
  opacity: 0
}.compose-form {
                    display: flex
                ...
                .filter-tab.active { background: linear-gradient(135deg, #a855f7 0%, #ec4899 100% ),
                    color: white
  ...
  opacity: 1
                        transform: translateY(0)
}.reflection-card { background: var(--card-bg)
                    border-radius: 16px
                    padding: 1.5rem
                    box-shadow: 0 2px 8px rgba(0, 0, 0, 0.05 );
                    border: 1px solid var(--border-color)
                ...
                .reflection-card:hover { transform: translateY(-3px)
                    box-shadow: 0 8px 20px rgba(0, 0, 0, 0.1 ),
                    border-color: rgba(168, 85, 247, 0.3) }

                .reflection-content { color: var(--text-primary)
                    line-height: 1.6
                    margin-bottom: 1rem
                    font-size: 1.05rem
                    ...
                    <form onSubmit={handleSubmit} className="compose-form">"
                        <div className="textarea-wrapper">"
                            ...
                            <span className={ `character-count ${
                                newReflection.length > MAX_REFLECTION_LENGTH * 0.9 ? 'warning' : '' } ${ newReflection.length >= MAX_REFLECTION_LENGTH ? 'error' : '' }`}>'
                    ...
                    {isLoading ? (
                        [...Array(3)].map((_, i) => (
                            <Card key={i} className="reflection-card">"
                                <div className="content-skeleton" style={"
