...
const handleDonate = async () => { setIsProcessing(true  );
        try {
            // In a real app, this would integrate with a Stripe element.clientSecret = await ApiClient.payment.createDonationIntent(selectedAmount),
            alert(`Thank you for your donation of $${(selectedAmount / 100).toFixed(2)}! \nClient Secret (for demo): ${clientSecret}`);
            // Here you would use stripe.confirmCardPayment(clientSecret, ...) catch (error) { console.error(error  );
const errorMessage = isError(error) ? error.message : 'An error occurred';'
    ...
    return (
        <>
            ...
            <Card className="auth-card">"
                ...
                <p style={
  { textAlign: 'center', marginBottom: '1.5rem', color: 'var(--text-secondary)' '
}>
  ...
  { justifyContent: 'center', marginBottom: '1.5rem' '
                    ...
                    { DONATION_AMOUNTS.map(amount => (
                        <AppButton
                            key={amount  );
$2iant={selectedAmount === amount && !customAmount ? 'primary' : 'secondary'}'
