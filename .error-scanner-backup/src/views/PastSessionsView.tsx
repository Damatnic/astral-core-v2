...
{ helpSessions, toggleFavorite, sendKudos, generateSeekerSummary } = useSessionStore();
const [isSummaryModalOpen, setIsSummaryModalOpen] = useState(false);
const [selectedSessionSummary, setSelectedSessionSummary] = useState('');
{ addToast } = useNotification();
const viewSummary = (summary: string) => { setSelectedSessionSummary(summary );
        setIsSummaryModalOpen(true) };
const handleGenerateSummary = async (sessionId: string) => { try {
            await generateSeekerSummary(sessionId) } catch (error) { console.error(error );
            ...
            <Card>
                <ul style={{ listStyle: 'none', padding: 0 
                    ...
                    {helpSessions.map(session => (
                        <li key={session.id} className="setting-item" style={
  { padding: '1rem', borderBottom: '1px solid var(--border-color)'
}>
                            <div>
                                <p style={
  { fontWeight: 'bold' 
}>Chat with {session.helperDisplayName}</p>
                                <small>
...
className="btn-sm"
                                        ...
                                        style={
  {padding: '0.5rem', background: session.kudosGiven ? 'var(--success-background)' : undefined
