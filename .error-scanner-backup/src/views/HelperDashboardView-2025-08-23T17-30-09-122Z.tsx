...
const renderTabContent = () => {
  ...
  { fontWeight: 'bold' '
}>Chat with Seeker ({truncateText(session.seekerId, 12)})</p><small>Ended on {new Date(session.endedAt!).toLocaleDateString()}</small></div><div className="form-actions-group">{"
...
}><div><h1>Helper Dashboard</h1><p className="view-subheader">Welcome, {helperProfile.displayName}. Your current level is {helperProfile.level}.</p></div><AppButton onClick={() => setActiveView({ view: 'helper-profile' })}>Edit Profile</AppButton></div>'
  ...
  }>{isAvailable ? 'You are Online' : 'You are Offline'}</label><div className="theme-toggle"><input type="checkbox" id="availability-toggle" checked={isAvailable} onChange={e => onToggleAvailability(e.target.checked)} /><label htmlFor="availability-toggle" className="slider"></label></div></div>"
