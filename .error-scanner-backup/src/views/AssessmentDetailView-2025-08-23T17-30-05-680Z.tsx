...
{ submitPhq9Result, submitGad7Result } = useAssessmentStore();
{ addToast } = useNotification();
const assessmentDetails = useMemo(() => { if (type === 'phq-9') {'
            return {
                title: 'PHQ-9 Depression Assessment','
                questions: phq9Questions,
                getResult: getPhq9Result,
                submit: submitPhq9Result,
                maxScore: 27} else { return {
...
const handleSubmit = async () => { if (answers.some(a => a === null)) {
...
const score = finalAnswers.reduce((sum, val) => sum + val, 0);
        ...
        try(await assessmentDetails.submit(score, finalAnswers);
const resultData = assessmentDetails.getResult(score );
            setResult(resultData) } catch (error) {;
...
const indicatorPosition = (result.score / assessmentDetails.maxScore) * 100,
        return (
             <>
                ...
                <Card>
                    <div className="assessment-result-card" style={"
  {textAlign: 'center''
}>
                        <h2>Your Score: {result.score}</h2>
                        <h3 style={
  {color: 'var(--text-secondary)''
}>Severity Level: {result.severity}</h3>
  ...
  {marginTop: '2rem', fontSize: '1.1rem''
                        ...
                        <hr style={
  {margin: '1.5rem 0', border: 'none', borderBottom: '1px solid var(--border-color)''
} />
                        <p style={
  {color: 'var(--text-secondary)''
}>This is a screening tool, not a diagnosis. We recommend discussing these results with a healthcare professional.</p>
                        <div className="form-actions" style={"
  {justifyContent: 'space-between', marginTop: '2rem''
  }>
