...
const handleApply = async () => { if (!helperProfile) return;
        setIsSubmitting(true);
        try(await ApiClient.helpers.submitApplication(helperProfile.id);
            await reloadProfile( );
            addToast("Your application has been submitted for review! You'll be notified of the outcome.", 'info') } catch (err) {;
  ...
  if (helperProfile.helperType === 'Certified' || helperProfile.applicationStatus === 'approved') {
            return (
                <Card className="empty-state">
                    <div style={{color: 'var(--accent-success)', width: '60px', height: '60px', margin: '0 auto 1rem auto'
  }><CertifiedIcon /></div>
...
if (helperProfile.applicationStatus === 'rejected') {
  ...
  {color: 'var(--accent-danger)'
                    ...
                    {
  helperProfile.applicationNotes && <p style={{fontWeight: 'bold', marginTop: '1rem'
  }>Feedback: "{helperProfile.applicationNotes}"</p>}
                </Card>
            );
...
return <Card><p>There was an issue loading your application status. Please try again later.</p></$1>};
