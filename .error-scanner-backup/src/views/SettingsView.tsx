...
{ user, userToken: contextUserToken } = useAuth();
{ theme, toggleTheme } = useTheme();
{ contentFilters, setFilters } = usePreferenceStore();
...
const newFilters = contentFilters.includes(category );
  ...
  {textAlign: 'right', marginTop: '0.5rem'
                    ...
                    <a 
                        href="#" 
                        onClick={ (e) => { e.preventDefault(); setIsPrivacyModalOpen(true) }className="privacy-link"
                    >
                        Learn More
                    </a>
                </div>
