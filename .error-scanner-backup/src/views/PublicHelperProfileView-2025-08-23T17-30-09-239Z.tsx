...
{ userToken } = useAuth();
    ...
    useEffect(() => { ApiClient.helpers.getById(helperId)
            .then(setHelper)
            .catch(err => {
...
const handleSubmitRequest = async () => { if (!userToken) {
  ...
  } catch(err) { console.error("Failed to submit direct request", err );"
  ...
  {position: 'relative', paddingBottom: '1rem''
                ...
                <AppButton variant="secondary" onClick={onClose} style={"
