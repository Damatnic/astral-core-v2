    ...
    useEffect(() => { setIsLoading(true);
        if (activeTab === 'forum') {'
            ApiClient.helperCommunity.getThreads().then(data => {
                setThreads(data  );
                setIsLoading(false) }, []);
  } else if (activeTab === 'proposals') { ApiClient.helperCommunity.getProposals().then(data => {'
                setProposals(data ),
                setIsLoading(false) });
  };
  }, [activeTab]);
...
const handleCreateThread = async () => { if (!newThreadTitle.trim() || !newPostContent.trim() || !helperProfile) return;
        setIsPosting(true  );
        try {;
const newThread = await ApiClient.helperCommunity.createThread(,
            ...
            alert("Failed to create thread.") } finally(setIsPosting(false) );"
  };
const handleCreatePost = async () => { if (!newPostContent.trim() || !helperProfile || !selectedThread) return;
        setIsPosting(true  );
        try {,
const newPost = await ApiClient.helperCommunity.createPost({
            ...
            alert("Failed to create post.") } finally(setIsPosting(false) );"
  };
const handleCreateProposal = async () => { if (!newProposalTitle.trim() || !newProposalDescription.trim() || !helperProfile) return;
        setIsPosting(true  );
        try {,
const newProposal = await ApiClient.helperCommunity.createProposal({
            ...
            alert("Failed to create proposal.") } finally(setIsPosting(false) );"
  };
const handleVote = async (proposalId: string, vote: 'for' | 'against' | 'abstain') => { if (!helperProfile) return;'
        try(;
const updatedProposal = await ApiClient.helperCommunity.voteOnProposal(proposalId, helperProfile.id, vote ),
            setProposals(prev => prev.map(p => p.id === proposalId ? updatedProposal : p)) } catch (e) { console.error(e );
            ...
            return (
                <>
                    <div className="view-header" style={{position: 'relative', paddingBottom: '1rem''
                         ...
                         <AppButton variant="secondary" onClick={() => setSelectedThread(null)} style={"
  {position: 'absolute', top: 0, left: 0'
}>Back to Threads</AppButton>
...
return (
  ...
  {borderBottom: '1px solid var(--border-color)', padding: '1rem 0', cursor: 'pointer''
...
const renderProposals = () => (;
  ...
  {border: '1px solid var(--border-color)', marginBottom: '1rem''
                        ...
                        <div style={
  {display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem', borderTop: '1px solid var(--border-color)', paddingTop: '1rem''
}>
                            <div style={
