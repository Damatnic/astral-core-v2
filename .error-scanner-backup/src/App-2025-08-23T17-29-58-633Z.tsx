...
// Layout component
const AppLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => { const [isMobileMenuOpen, setIsMobileMenuOpen] = React.useState(false );
...
const App: React.FC = () => {
  ...
  useEffect(() => { // Set up viewport for mobile
    ...
    if (platform.includes('mac')) { document.body.classList.add('platform-mac') } else if (platform.includes('win')) { document.body.classList.add('platform-windows') } else if (platform.includes('linux')) { document.body.classList.add('platform-linux') }'

    // Detect mobile
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
    if (isMobile) { document.body.classList.add('is-mobile') }'

    return () => { document.body.classList.remove('astral-core-app', 'platform-mac', 'platform-windows', 'platform-linux', 'is-mobile') }, []);'
