...
export const useSessionStore = create<SessionState>((set, get) => ({ helpSessions: [],
...
const sessions = await ApiClient.helpSessions.getForUser(userId ),
  ...
  } catch (error) { console.error("Failed to load help sessions:", error );"
  },

  getHelpSessionByDilemmaId: (dilemmaId) => { return get().helpSessions.find(s => s.dilemmaId === dilemmaId) },
...
const result = await ApiClient.helpSessions.sendKudos(sessionId, seekerId );
  ...
  } catch (error) { console.error("Failed to send kudos:", error  );"
       set(state => ({
...
const summary = await ApiClient.ai.summarizeChat(transcript);
  ...
  } catch (err) { console.error("Failed to generate seeker summary:", err  );"
        set(state => ({
  ...
  generateHelperPerformanceSummary: async (sessionId) => {
    ...
    try(// In a real app, we would get the chat transcript from the session
const summary = await ApiClient.ai.summarizeHelperPerformance("" );"
  ...
  } catch(err) { console.error("Failed to generate helper summary:", err  );"
