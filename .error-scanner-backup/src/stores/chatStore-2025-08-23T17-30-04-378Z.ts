...
{ activeChatId, chatSessions } = get();
  ...
  startChat: async (dilemmaId, perspective) => { try(;
...
const senderId = activeChat?.perspective === 'seeker' ? dilemma?.userToken : authState.helperProfile?.id;'
    ...
    if (!senderId) { console.error("Could not determine sender ID for chat message." );"
        return  };
optimisticMessage: ChatMessage = { id: crypto.randomUUID(),
      sender,
      text,
      timestamp: new Date().toISOString()};

    set(state => ({ chatSessions: {
        ...state.chatSessions,
        [dilemmaId]: {
...
const savedMessage = await ApiClient.chat.sendMessage(dilemmaId, text, sender, senderId);
    ...
    if (sender === 'poster' && activeChat?.perspective === 'helper') { try(;'
const guidanceResult = await ApiClient.ai.getHelperGuidance(text  );
  ...
  } catch (error) { console.error("Failed to get AI helper guidance:", error };"
  );
  },
