...
describe('sessionStore', () => { beforeEach(() => {
...
const mockSessions = [{ id: 's1', dilemmaId: 'd1' }];
    ...
    await act(async () => { await useSessionStore.getState().fetchHelpSessions() });
...
const updatedSession = { ...initialSession, isFavorited: true };
    ...
    await act(async () => { await useSessionStore.getState().toggleFavorite('s1') });
...
const initialSession = { id: 's1', kudosGiven: false };
    ...
    await act(async () => { await useSessionStore.getState().sendKudos('s1') });
