...
const mockHistory = [{ id: 'a1', type: 'phq-9', score: 10 }];
    ...
    await act(async () => { await useAssessmentStore.getState().fetchHistory() });
const state = useAssessmentStore.getState();
    expect(state.isLoading).toBe(false);
    expect(state.history).toEqual(mockHistory);
    expect(mockedFetch).toHaveBeenCalledWith('/api/assessments/history', { headers: {
...
const answers = [1,2,3,1,2,3,1,2,0];
    ...
    await act(async () => { await useAssessmentStore.getState().submitPhq9Result(score, answers) });
...
const answers = [1,2,3,1,2,3,0];
    ...
    await act(async () => { await useAssessmentStore.getState().submitGad7Result(score, answers) });
