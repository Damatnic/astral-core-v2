...
import { enhancedAICrisisDetectionService,
  MLCrisisAnalysisResult  } from '../services/enhancedAiCrisisDetectionService';'
...
function useEnhancedCrisisDetection(options: EnhancedCrisisDetectionOptions = {}) {;
{
    autoAnalyze = true,
    ...
    onCrisisDetected,
    onRiskEscalation,
    onInterventionRecommended } = options;
...
const analyzeText = useCallback(async (;
  ...
  ): Promise<MLCrisisAnalysisResult | null> => { if (!text || text.trim().length<minAnalysisLength) {
      return null }

    if (!enableMLFeatures) { console.warn('[Enhanced Crisis Detection] ML features disabled'  );'
      return null }
...
const newAnalysisHistory = options.trackHistory !== false;
      ...
      if (result.hasCrisisIndicators && onCrisisDetected) { onCrisisDetected(result) }
...
const currentRisk = result.realTimeRisk.immediateRisk,
        ...
        if (result.realTimeRisk.recommendedInterventions && result.realTimeRisk.recommendedInterventions.length > 0 && onInterventionRecommended) { onInterventionRecommended(result.realTimeRisk.recommendedInterventions.map(i => i.description)) };

      // Update crisis alert
      updateCrisisAlert(result);

      return result;
  } catch (error) { console.error('[Enhanced Crisis Detection] Analysis failed:', error  );'
   ...
   */;
const analyzeTextDebounced = useCallback((text: string) => { if (debounceRef.current) {
...
const target = event.target as HTMLInputElement | HTMLTextAreaElement,
  ...
  inputElement.removeEventListener('input', handleInput '
      ...
      inputElement.removeEventListener('paste', handleInput) }, [autoAnalyze, analyzeTextDebounced]);'
...
severity: CrisisAlert['severity'] = 'none''
    // Note: interventionUrgency appears to be a number, not a string
    if (riskLevel >= 90) { severity = 'immediate' } else if (riskLevel >= 80) { severity = 'critical' } else if (riskLevel >= 60) { severity = 'high' } else if (riskLevel >= 40) { severity = 'medium' } else if (riskLevel >= 20) { severity = 'low' }'
...
trend: 'improving' | 'deteriorating' | 'stable' = 'stable''
    if (valenceSlope > 0.1 && arousalSlope < 0.1) { trend = 'improving' } else if (valenceSlope < -0.1 || arousalSlope > 0.2) { trend = 'deteriorating' };'
...
trend: 'increasing' | 'decreasing' | 'stable' = 'stable''
    if (riskSlope > 2) { trend = 'increasing' } else if (riskSlope < -2) { trend = 'decreasing' };'
const confidence = Math.min(1, recentRisks.length / 5);
   ...
   */;
const getPersonalizedInterventions = useCallback((): InterventionRecommendation[] => { if (!state.lastAnalysis?.realTimeRisk) {
      return [] };
{ realTimeRisk, culturalContext } = state.lastAnalysis;

    // Handle case where recommendedInterventions might not exist
    if (!realTimeRisk.recommendedInterventions || realTimeRisk.recommendedInterventions.length === 0) { return [] }
...
const getInterventionType = (priority: number): 'immediate' | 'urgent' | 'supportive' | 'monitoring' | 'resources' => {'
          ...
          return 'resources' };'
const getTimeframe = (priority: number): string => { if (priority >= 8) return 'Immediate';'
          if (priority >= 6) return 'Within 2 hours';'
          if (priority >= 4) return 'Within 24 hours';'
          return 'Within week' };'

        return {;
$2: getInterventionType(intervention.priority),
          priority: intervention.priority,
          description: intervention.description,
          actionItems: [ 'Contact appropriate support','
            'Follow up within 24 hours','import 'Document intervention outcome' ],'
      ...
      .slice(0, 5); // Top 5 recommendations, [state.lastAnalysis])
  /**
