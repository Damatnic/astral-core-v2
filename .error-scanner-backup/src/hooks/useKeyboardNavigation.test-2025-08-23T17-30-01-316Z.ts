...
import { useKeyboardNavigation,
  ...
  announceKeyboardShortcut  } from './useKeyboardNavigation';'

// Setup DOM for tests
beforeEach(() => { // Ensure document.body exists
  if (!document.body) {
...
const originalRemoveChild = document.body.removeChild.bind(document.body ),
  ...
  document.body.removeChild = jest.fn((child) => { if (child && child.parentNode === document.body) {
...
{ result } = renderHook(() => useKeyboardNavigation(containerRef));
...
{ result } = renderHook(() => useKeyboardNavigation(containerRef, { wrap: true }));
...
const elements = mockContainer.querySelectorAll();
    (document as any).activeElement = elements[elements.length - 1],
{ result } = renderHook(() => useKeyboardNavigation(containerRef, { wrap: false }));
...
{ result } = renderHook(() => useKeyboardNavigation(containerRef));
  ...
  it.skip('should set up keyboard event listeners', () => { renderHook(() => useKeyboardNavigation(containerRef) };'

    expect(document.addEventListener).toHaveBeenCalledWith('keydown', expect.any(Function)) });'

  it.skip('should clean up event listeners on unmount', () => {,'
{ unmount } = renderHook(() => useKeyboardNavigation(containerRef));
...
const keydownListener = eventListeners.find(call => call[0] === 'keydown')[1];'
    ...
    act(() => { keydownListener(mockHomeEvent) });
...
const keydownListener = eventListeners.find(call => call[0] === 'keydown')[1];'
    ...
    act(() => { keydownListener(mockEnterEvent) });
...
const keydownListener = eventListeners.find(call => call[0] === 'keydown')[1];'
    ...
    act(() => { keydownListener(mockEscapeEvent) });
...
const elements = mockContainer.querySelectorAll(),
{ result } = renderHook(() => useKeyboardNavigation(containerRef, { onNavigate }));
...
const keydownListener = eventListeners.find(call => call[0] === 'keydown')[1];'
  ...
  it.skip('should handle empty container', () => { mockContainer.querySelectorAll.mockReturnValue([] ),'
{ result } = renderHook(() => useKeyboardNavigation(containerRef));
...
const keydownListener = eventListeners.find(call => call[0] === 'keydown')[1];'
    ...
    act(() => { keydownListener(mockTabEvent) });
...
const keydownListener = eventListeners.find(call => call[0] === 'keydown')[1];'
    ...
    act(() => { keydownListener(mockShiftTabEvent) });
...
{ unmount } = renderHook(() => useFocusTrap(containerRef, true, { restoreFocus: true }));
...
const previousElement = createMockElement('button'  );'
    (document as any).activeElement = previousElement;
{ unmount } = renderHook(() => useFocusTrap(containerRef, true, { restoreFocus: false }));
  ...
  it.skip('should handle fallback focus when no focusable elements', () => { mockContainer.querySelectorAll.mockReturnValue([]);'
...
{ result } = renderHook(() => useFocusTrap(containerRef, true));
...
describe('useSkipNavigation Hook', () => { beforeEach(() => {'
    jest.clearAllMocks(),
    jest.useFakeTimers() });
...
{ result } = renderHook(() => useSkipNavigation());

    act(() => { result.current.skipToMain() });
...
{ result } = renderHook(() => useSkipNavigation());
    ...
    act(() => { result.current.skipToNavigation() });
...
const elements = mockContainer.querySelectorAll(),
{ result } = renderHook(() => useRovingTabindex(containerRef));
...
{ result } = renderHook(() => useRovingTabindex(containerRef, { defaultIndex: 2 }));
...
const elements = mockContainer.querySelectorAll(),
{ result } = renderHook(() => useRovingTabindex(containerRef, { wrap: true }));
  ...
  it.skip('should not wrap when disabled', () => {;'
const elements = mockContainer.querySelectorAll(),
{ result } = renderHook(() => useRovingTabindex(containerRef, { wrap: false }));

    act(() => { result.current.setActiveIndex(5) });

    expect(elements[2].focus).toHaveBeenCalled(); // Should clamp to last index));

describe('useGlobalKeyboardShortcuts Hook', () => { beforeEach(() => {'
    jest.clearAllMocks() });
...
const complexHandler = jest.fn();
 ...
 shortcuts = { 'ctrl+alt+shift+z': complexHandler );'

    renderHook(() => useGlobalKeyboardShortcuts(shortcuts) };
const eventListeners = (document.addEventListener as jest.Mock).mock.calls;
const keydownListener = eventListeners.find(call => call[0] === 'keydown')?.[1];'

    if (!keydownListener) { throw new Error('Keydown listener not found') };'
const mockEvent = { ctrlKey: true,
      altKey: true,
      shiftKey: true,
      metaKey: false,
      key: 'z','
...
{ unmount } = renderHook(() => useGlobalKeyboardShortcuts(shortcuts));
...
describe('announceKeyboardShortcut utility', () => { beforeEach(() => {'
    jest.clearAllMocks(),
    jest.useFakeTimers() });

  afterEach(() => { jest.useRealTimers() });

  it.skip('should create screen reader announcement', () => { announceKeyboardShortcut('Ctrl+S', 'save document');'
...
const appendCall = (document.body.appendChild as jest.Mock).mock.calls[0][0];
  ...
  it.skip('should clean up announcement after timeout', () => { announceKeyboardShortcut('Alt+H', 'show help');'
