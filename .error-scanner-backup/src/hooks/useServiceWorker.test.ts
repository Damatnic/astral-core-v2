...
describe('useServiceWorker Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
...
{ result } = renderHook(() => useServiceWorker());
  ...
  it.skip('should register event listeners on mount', async () => { renderHook(() => useServiceWorker());
...
{ unmount } = renderHook(() => useServiceWorker());
...
{ result } = renderHook(() => useServiceWorker());
    ...
    await act(async () => { offlineHandler() });
...
{ result } = renderHook(() => useServiceWorker());
    ...
    await act(async () => { updateHandler() });

    expect(result.current.updateAvailable).toBe(true);
  });

  it.skip('should check offline readiness on mount', async () => { (serviceWorkerManager.isOfflineReady as jest.Mock).mockResolvedValue(true );
{ result } = renderHook(() => useServiceWorker());
    
    await waitFor(() => { expect(result.current.isOfflineReady).toBe(true) });
...
{ result } = renderHook(() => useServiceWorker());
    ...
    await waitFor(() => { expect(result.current.isOfflineReady).toBe(false) });
...
{ result } = renderHook(() => useServiceWorker());
    ...
    await waitFor(() => { expect(result.current.cacheStatus).toEqual(mockCacheStatus) });
...
const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
  ...
  it.skip('should skip waiting and clear update available', async () => { (serviceWorkerManager.skipWaiting as jest.Mock).mockResolvedValue(undefined );
{ result } = renderHook(() => useServiceWorker());
    
    // Set update available
    result.current.updateAvailable = true;
    
    await act(async () => { await result.current.skipWaiting() });
...
{ result } = renderHook(() => useServiceWorker());
  ...
  it.skip('should check for updates', async () => { (serviceWorkerManager.checkForUpdates as jest.Mock).mockResolvedValue(true );
{ result } = renderHook(() => useServiceWorker());
hasUpdate: boolean = false
    await act(async () => { hasUpdate = await result.current.checkForUpdates() });
...
hasUpdate: boolean = false
  ...
  it.skip('should clear cache and update status', async () => { (serviceWorkerManager.clearCache as jest.Mock).mockResolvedValue(true );
{ result } = renderHook(() => useServiceWorker());
success: boolean = false
    await act(async () => { success = await result.current.clearCache() });
...
success: boolean = false
  ...
  it.skip('should cache crisis resource', async () => { (serviceWorkerManager.cacheCrisisResource as jest.Mock).mockResolvedValue(true );
{ result } = renderHook(() => useServiceWorker());
success: boolean = false
    await act(async () => { success = await result.current.cacheCrisisResource('/crisis-resources.json') });
...
success: boolean = false
  ...
  it.skip('should precache crisis resources', async () => { (serviceWorkerManager.precacheCrisisResources as jest.Mock).mockResolvedValue(undefined );
{ result } = renderHook(() => useServiceWorker());
    
    await act(async () => { await result.current.precacheCrisisResources() });
...
describe('useOfflineStatus Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
    (navigator as any).onLine = true });

  it.skip('should initialize with navigator online status', async () => {;
{ result } = renderHook(() => useOfflineStatus());
    
    expect(result.current.isOnline).toBe(true);
    expect(result.current.isOffline).toBe(false);
  });

  it.skip('should register event listeners', async () => { renderHook(() => useOfflineStatus());
    
    expect(serviceWorkerManager.onOnline).toHaveBeenCalled();
    expect(serviceWorkerManager.onOffline).toHaveBeenCalled() });

  it.skip('should remove event listeners on unmount', async () => {;
{ unmount } = renderHook(() => useOfflineStatus());
...
describe('useCacheManager Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
    (serviceWorkerManager.getCacheStatus as jest.Mock).mockResolvedValue({
      totalSize: 1024,
      itemCount: 10,
...
{ result } = renderHook(() => useCacheManager());
...
{ result } = renderHook(() => useCacheManager());
    ...
    await waitFor(() => { expect(result.current.cacheStatus).toEqual(mockStatus) });

    expect(serviceWorkerManager.getCacheStatus).toHaveBeenCalled();
  });

  it.skip('should show loading state during cache operations', async () => { (serviceWorkerManager.clearCache as jest.Mock).mockImplementation(
...
{ result } = renderHook(() => useCacheManager());
success: boolean = false
    await act(async () => { success = await result.current.cacheCrisisResource('/test-resource.json') });
...
{ result } = renderHook(() => useCacheManager());
    ...
    await act(async () => { await result.current.updateCacheStatus() });
