 ...
 */;
export interface ServiceWorkerMessage { type: string
  data?: any
  timestamp: number
 ...
 */ }
 ...
 interface OfflineCapability { feature: string
  available: boolean
  description: string
...
DEFAULT_CONNECTION_STATUS: ConnectionStatus={
...
DEFAULT_OFFLINE_CAPABILITIES: OfflineCapability[] = [
  {
 ...
 const useConnectionStatus = (): string => { const [connectionStatus, setConnectionStatus] = useState<ConnectionStatus>(DEFAULT_CONNECTION_STATUS);
  ...
  // Update connection quality based on navigator connection
    ...
    if(connection) {
      const { effectiveType, downlink } = connection;
              if(effectiveType === "4g") { return "excellent" } else if (effectiveType === "3g") { return "good" } else { return "poor" }// Fallback: assume good if online but no connection info
    return "good"
  }, []);
  
  // Check if crisis resources are cached
  const checkCrisisResourcesAvailability = useCallback(async () => { if (!("caches" in window)) return false;
          try(const crisisCache = await caches.open("astral-core-crisis-v1");
...
const updateStatus = (sw: ServiceWorker | null): void => {
          ...
          if(registration.installing) { updateStatus(registration.installing );
            registration.installing.addEventListener('statechange', () => updateStatus(registration.installing)) } else if (registration.waiting) { updateStatus(registration.waiting );
            registration.waiting.addEventListener('statechange', () => updateStatus(registration.waiting)) } else if (registration.active) { updateStatus(registration.active );
    ...
    return () => { navigator.serviceWorker.removeEventListener('message', handleServiceWorkerMessage) }, [handleServiceWorkerMessage]);
...
const handleOffline = (): void => {
    ...
    return () => { window.removeEventListener('online', handleOnline );
      window.removeEventListener('offline', handleOffline) }, [updateConnectionQuality]);
  ...
  useEffect(() => {
    ...
    if(connection) {
      ...
      return () => { connection.removeEventListener('change', handleConnectionChange) }, [updateConnectionQuality]);

  // Initial crisis resources check
  useEffect(() => { checkCrisisResourcesAvailability().then(available => {
      setConnectionStatus(prev => {
        const updated={
          ...prev,
          crisisResourcesAvailable: available
  ...
  const sendMessageToServiceWorker = useCallback(async (message: ChatMessage) => { if(!serviceWorkerRegistration?.active) {

      return false }

    try(serviceWorkerRegistration.active.postMessage(message );
      return true } catch(error) { return false }, [serviceWorkerRegistration]);
