...
import type { AnalyticsInsights, 
  CulturalEffectivenessMetrics,
  InterventionOutcome } from '../services/privacyPreservingAnalyticsService';
...
const privacyMetrics = privacyPreservingAnalyticsService.getPrivacyMetrics();
   ...
   */;
const loadInsights = useCallback(async () => { try(setIsLoading(true);
      setError(null);
const analyticsInsights = await privacyPreservingAnalyticsService.generateAnalyticsInsights();
      ...
      updatePrivacyBudget() } catch (err) { setError(err instanceof Error ? err.message : 'Failed to load analytics insights' );
      console.error('[Privacy Analytics Hook] Failed to load insights:', err) } finally(setIsLoading(false) );
  }, []);
...
const recordIntervention = useCallback(async (outcomeData: {
    ...
    sessionDuration: number
    feedback?: number
  }) => { try(await privacyPreservingAnalyticsService.recordInterventionOutcome(outcomeData );
      
      // Update privacy metrics
      updatePrivacyBudget() } catch (err) { setError(err instanceof Error ? err.message : 'Failed to record intervention' );
   ...
   */;
const recordFollowUp = useCallback(async (userToken: string, sessionId: string) => { try(await privacyPreservingAnalyticsService.recordFollowUpEngagement(userToken, sessionId );
      
      // Update privacy metrics
      updatePrivacyBudget() } catch (err) { setError(err instanceof Error ? err.message : 'Failed to record follow-up' );
   ...
   */;
const generateReport = useCallback(async () => { try(setIsLoading(true);
const report = await privacyPreservingAnalyticsService.generateEffectivenessReport( );
      return report } catch (err) { setError(err instanceof Error ? err.message : 'Failed to generate report');
   ...
   */;
const exportData = useCallback(async () => { try(setIsLoading(true);
const exportedData = await privacyPreservingAnalyticsService.exportAnonymizedData( );
      return exportedData } catch (err) { setError(err instanceof Error ? err.message : 'Failed to export data');
   ...
   * Refresh insights
   */;
const refreshInsights = useCallback(async () => { await loadInsights() }, [loadInsights]);

  /**
   * Reset privacy budget
   */;
const resetPrivacyBudget = useCallback(() => { privacyPreservingAnalyticsService.resetPrivacyBudget();
    updatePrivacyBudget() }, [updatePrivacyBudget]);
