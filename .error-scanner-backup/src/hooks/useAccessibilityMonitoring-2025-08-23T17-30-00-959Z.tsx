...
const errorMessage = err instanceof Error ? err.message : 'Accessibility audit failed';'
      ...
      throw err } finally(setIsLoading(false) );
  }, [wcagLevel]);
...
const result = await accessibilityAuditSystem.runAccessibilityAudit(wcagLevel);
      ...
      if (onComplianceChange && previousComplianceRef.current !== result.isCompliant) { onComplianceChange(result.isCompliant ),
...
const startMonitoring = useCallback(() => { if (isOldSignature && monitoringCallback && enabled) {
      // For old signature, use the audit system's monitoring directly'
      accessibilityAuditSystem.startMonitoring(monitoringCallback) } else if (autoRefresh && !isMonitoring) { // For new signature, use internal monitoring
...
const stopMonitoring = useCallback(() => { if (isOldSignature) {
      // For old signature, use the audit system's stop monitoring'
      accessibilityAuditSystem.stopMonitoring() } else { // For new signature, use internal monitoring
  ...
  useEffect(() => { if (!isOldSignature) {
      runAudit(), // Initial audit for new signature only, [runAudit, isOldSignature])
  useEffect(() => { if (isOldSignature && enabled) {
      startMonitoring() } else if (autoRefresh) { startMonitoring() } else { stopMonitoring() }

    return () => { stopMonitoring() }, [isOldSignature, enabled, autoRefresh, startMonitoring, stopMonitoring]);
...
const criticalCrisisIssues = crisisRelatedIssues.filter(issue =>;
  ...
  } catch (err) { console.error('Crisis accessibility check failed:', err );'
      throw err  };
  }, []);

  // Auto-check crisis accessibility every 10 seconds
  useEffect(() => { const interval = setInterval(checkCrisisAccessibility, 10000 );
    checkCrisisAccessibility(), // Initial check

    return () => clearInterval(interval) }, [checkCrisisAccessibility]);
...
const [filters, setFilters] = useState({
  ...
  useEffect(() => { if (!auditResult) {
      setFilteredIssues([] );
      return  };
const filtered = auditResult.issues;

    // Filter by severity
    if (filters.severity.length > 0) { filtered = filtered.filter(issue => filters.severity.includes(issue.severity)) }

    // Filter by WCAG principle
    if (filters.wcagPrinciple.length > 0) { filtered = filtered.filter(issue => filters.wcagPrinciple.includes(issue.wcagPrinciple)) }

    // Filter by crisis-related
    if (filters.isCrisisRelated) { filtered = filtered.filter(issue => issue.isCrisisRelated) }

    // Filter by assistive technology impact
    if (filters.assistiveTech.length > 0) { filtered = filtered.filter(issue =>
...
return () => { if (threshold) {
        accessibilityAuditSystem.teardownAlerts()}, [threshold, enabled]);
...
$2,
    ...
    if (type !== 'critical') { setTimeout(() => {'
        setAlerts(prev => prev.filter(a => a.id !== alert.id)) }, 5000);
return alert.id;
  }, []);
const removeAlert = useCallback((alertId: string) => { setAlerts(prev => prev.filter(alert => alert.id !== alertId)) }, []);
const clearAllAlerts = useCallback(() => { setAlerts([]) }, []);

  // Monitor for new critical issues
const monitorCriticalIssues = useCallback((auditResult: AccessibilityAuditResult | null) => { if (!auditResult) return;
...
const keyboardRelatedIssues = result.issues.filter(issue =>;
      ...
      return keyboardRelatedIssues } catch (err) { console.error('Keyboard navigation test failed:', err  );'
      throw err } finally(setIsTestingKeyboard(false) );
  }, []);

  return(isTestingKeyboard,
    keyboardIssues,
...
const screenReaderRelatedIssues = result.issues.filter(issue =>;
      ...
      return screenReaderRelatedIssues } catch (err) { console.error('Screen reader test failed:', err  );'
      throw err } finally(setIsTestingScreenReader(false) );
  }, []);

  return(isTestingScreenReader,
    screenReaderIssues,
...
const [tabOrder, setTabOrder] = useState<HTMLElement[]>([] };
  ...
  useEffect(() => {
    if (enabled) {
      ...
      setKeyboardNavigationEnabled(true) } else(accessibilityAuditSystem.teardownKeyboardSupport( );
      setKeyboardNavigationEnabled(false) }

    return () => { accessibilityAuditSystem.teardownKeyboardSupport() }, [enabled]);
const enableKeyboardNavigation = useCallback(() => { setKeyboardNavigationEnabled(true) }, []);
const disableKeyboardNavigation = useCallback(() => { setKeyboardNavigationEnabled(false) }, []);
...
const focusableElements = Array.from(;
  ...
  useEffect(() => { updateTabOrder();
    window.addEventListener('resize', updateTabOrder  );'
    return () => window.removeEventListener('resize', updateTabOrder) }, [updateTabOrder]);'
  ...
  return(keyboardNavigationEnabled,
    ...
    updateTabOrder );
interface default(useAccessibilityAudit,
