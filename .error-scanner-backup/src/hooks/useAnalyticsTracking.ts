...
interface TrackingOptions { category?: string;
      properties?: Record<string, any>;
    sensitivityLevel?: "public" | "private" | "sensitive" | "crisis" };
interface UseAnalyticsTrackingOptions { trackPageViews?: boolean
...
const previousPath = useRef<string>();
  ...
  // Track page views
  useEffect(() => { if (!trackPageViews) return;
const currentPath = location.pathname + location.search;
    ...
    analyticsService.trackPageView(currentPath, componentName || featureName) }, [location, navigationType, trackPageViews, componentName, featureName])
...
{ trackEvent, trackTiming } = useAnalyticsTracking({ componentName );
const mountTime = useRef<number>(Date.now());
    ...
    return () => { // Track component unmount and session duration
...
{ trackFeatureUsage } = useAnalyticsTracking({ featureName });
