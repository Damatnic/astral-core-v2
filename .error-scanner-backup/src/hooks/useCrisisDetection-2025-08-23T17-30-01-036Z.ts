...
function useCrisisDetection(options: CrisisDetectionOptions = {}) {,
{
    autoAnalyze = true,
...
const escalationActions = result.hasCrisisIndicators;
      ...
      if (result.escalationRequired && onEscalationRequired) { onEscalationRequired(escalationActions) }
...
const response = crisisDetectionService.generateCrisisResponse(result, userType  );
      ...
      return result;
  } catch (error) { console.error('Crisis analysis failed:', error  );'
   ...
   */;
const analyzeTextDebounced = useCallback((text: string, userType: 'seeker' | 'helper' = 'seeker') => { if (debounceRef.current) {'
      clearTimeout(debounceRef.current) }

    debounceRef.current = setTimeout(() => { analyzeText(text, userType).catch(console.error) }, debounceMs);
  }, [analyzeText, debounceMs]);

  /**
   * Monitor text input for crisis indicators (for real-time chat/input)
   */;
const monitorTextInput = useCallback((text: string, userType: 'seeker' | 'helper' = 'seeker') => { if (!autoAnalyze || !text) return,'
...
severityLevels: Record<string, number> = { 'none': 0, 'low': 1, 'medium': 2, 'high': 3, 'critical': 4 };'
...
resources: string[] = []
    if (analysis.detectedCategories.includes('suicidal')) { resources.push('
        'Suicide Prevention Resource Center','
        'American Foundation for Suicide Prevention','import 'Safety Planning Guide';'
      ) }

    if (analysis.detectedCategories.includes('self-harm')) { resources.push('
        'Self-Injury Outreach & Support','
        'To Write Love on Her Arms','import 'Alternative Coping Strategies';'
      ) }

    if (analysis.detectedCategories.includes('substance-abuse')) { resources.push('
        'SAMHSA National Helpline: 1-800-662-4357','
        'Narcotics Anonymous','import 'Alcoholics Anonymous';'
      ) }
