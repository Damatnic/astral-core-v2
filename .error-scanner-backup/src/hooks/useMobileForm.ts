...
fields: Record<string, FormField> = {};
    Object.keys(initialValues).forEach(key => { fields[key] = {
...
const formRef = useRef<HTMLFormElement>(null);
  ...
  // Update field value
const setFieldValue = useCallback((fieldName: string, value: string) => { setFormState(prev => ({
      ...prev,
      fields: {
        ...prev.fields,
        [fieldName]: {
...
const setFieldTouched = useCallback((fieldName: string, touched = true) => { setFormState(prev => ({
      ...prev,
      fields: {
        ...prev.fields,
        [fieldName]: {
...
const setFieldValidating = useCallback((fieldName: string, validating = true) => { setFormState(prev => ({
      ...prev,
      fields: {
        ...prev.fields,
        [fieldName]: {
...
const field = formState.fields[fieldName] || {
    ...
    return { value: field.value,
      ...
      onValidationChange: (isValid: boolean, errors: string[]) => { setFieldValidation(fieldName, isValid, errors) },;
...
values: Record<string, string> = {};
      Object.keys(formState.fields).forEach(key => { values[key] = formState.fields[key].value });

      // Only submit if form is valid
      if (formState.isValid) { await onSubmit(values) };
  } catch (error) { console.error('Form submission error:', error) } finally { setFormState(prev => ({
        ...prev,
        isSubmitting: false, }));
  };
  }, [onSubmit, formState]);
...
const field = formState.fields[fieldName];
    return field ? field.isTouched && !field.isValid : false }, [formState.fields]);
...
export const commonValidationRules = { email: {
    required: true,
    pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
  ...
  confirmPassword: (passwordField: string) => ({ required: true,
