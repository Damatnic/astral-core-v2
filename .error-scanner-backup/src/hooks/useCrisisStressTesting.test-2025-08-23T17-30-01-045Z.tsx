...
describe('useCrisisStressTesting Hook', () => { beforeEach(() => {'
    jest.clearAllMocks() });

  it.skip('should initialize with default state', async () => {,'
{ result } = renderHook(() => useCrisisStressTesting());
...
{ result } = renderHook(() => useCrisisStressTesting(onEmergencyBreak, onTestComplete));
...
{ result } = renderHook(() => useCrisisStressTesting());
    ...
    await act(async () => { await result.current.actions.runStressTests() });
...
const onEmergencyBreak = jest.fn();
{ result } = renderHook(() => useCrisisStressTesting(onEmergencyBreak));
...
{ result } = renderHook(() => useCrisisStressTesting(onEmergencyBreak));
    ...
    await waitFor(() => { expect(result.current.state.emergencyStatus.active).toBe(true);
...
{ result } = renderHook(() => useCrisisStressTesting());
    ...
    await act(async () => { await result.current.actions.runFailoverTests() });
...
{ result } = renderHook(() => useCrisisStressTesting(onEmergencyBreak));
    ...
    act(() => { result.current.actions.emergencyStop() });
...
{ result } = renderHook(() => useCrisisStressTesting());
    ...
    // Then clear it
    act(() => { result.current.actions.clearEmergencyStatus() });
...
const newConfig = {
    ...
    act(() => { result.current.actions.updateTestConfig(newConfig) });
...
{ result } = renderHook(() => useCrisisStressTesting());
    ...
    act(() => { result.current.actions.toggleScenario('scenario-1') });'
...
{ result } = renderHook(() => useCrisisStressTesting());
    ...
    // Then clear results
    act(() => { result.current.actions.clearResults() });
...
const stats = result.current.stats;
  ...
  it.skip('should prevent concurrent test execution', async () => { (crisisStressTestingSystem.runCrisisStressTests as jest.Mock).mockImplementation('
      () => new Promise(resolve => setTimeout(() => resolve([]), 100))
     };
{ result } = renderHook(() => useCrisisStressTesting());

    // Start first test
const firstTest = act(async () => { await result.current.actions.runStressTests() });

    // Try to start second test while first is running
    act(() => { result.current.actions.runStressTests() });
...
const mockResults = [;
    ...
    (crisisStressTestingSystem.runCrisisStressTests as jest.Mock).mockResolvedValue(mockResults);
const onTestComplete = jest.fn();
{ result } = renderHook(() => useCrisisStressTesting(undefined, onTestComplete));
...
{ result } = renderHook(() => useCrisisStressTesting());
    ...
    // Deselect scenario-2
    act(() => { result.current.actions.toggleScenario('scenario-2') });'
mockResults: unknown[] = []
    (crisisStressTestingSystem.runCrisisStressTests as jest.Mock).mockResolvedValue(mockResults)
    await act(async () => { await result.current.actions.runStressTests() });
...
resolveTest: () => void }
 ...
 testPromise = new Promise<any[]>(resolve => {
      resolveTest = () => resolve([]) });
...
{ result } = renderHook(() => useCrisisStressTesting());
    ...
    // Start test
    await act(async () => { result.current.actions.runStressTests() });
