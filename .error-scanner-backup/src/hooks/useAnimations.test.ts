...
import { useScrollAnimation,
  ...
  useFormAnimations,
  usePageTransition,
  useReducedMotion } from './useAnimations';
...
beforeAll(() => { mockIntersectionObserver.mockImplementation((callback, options) => ({
    observe: mockObserve,
...
describe('useScrollAnimation Hook', () => { beforeEach(() => {
    jest.clearAllMocks() });

  it.skip('should initialize with correct default values', () => {;
{ result } = renderHook(() => useScrollAnimation());
...
{ unmount } = renderHook(() => useScrollAnimation());
...
describe('useStaggeredAnimation Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
...
{ result } = renderHook(() => useStaggeredAnimation(5));
...
const itemCount = 3;
const baseDelay = 100;
{ result } = renderHook(() => useStaggeredAnimation(itemCount, baseDelay));
    
    act(() => { result.current.triggerAnimation() });
...
{ result } = renderHook(() => useStaggeredAnimation(2));
...
describe('useRippleEffect Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers('modern') });
...
{ result } = renderHook(() => useRippleEffect());
...
const mockEvent = {
    ...
    act(() => { result.current.createRipple(mockEvent) });
...
describe('useDelayedHover Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers('modern') });
...
{ result } = renderHook(() => useDelayedHover());
...
{ result } = renderHook(() => useDelayedHover(enterDelay));
...
{ result } = renderHook(() => useDelayedHover(0, leaveDelay));
...
{ result } = renderHook(() => useDelayedHover(100, 100));
...
{ result, unmount } = renderHook(() => useDelayedHover(100));
...
describe('useLoadingState Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers('modern') });
...
{ result } = renderHook(() => useLoadingState());
...
{ result } = renderHook(() => useLoadingState(minDuration));
...
describe('useAnimationSequence Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers('modern') });
...
{ result } = renderHook(() => useAnimationSequence());
...
const delays = [100, 200, 300];
    ...
    act(() => { result.current.playSequence(steps, delays) });
...
describe('useFormAnimations Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers('modern') });
...
{ result } = renderHook(() => useFormAnimations());
...
{ result } = renderHook(() => useFormAnimations());
    ...
    act(() => { result.current.showFieldError('email', 'Invalid email') });
...
{ result } = renderHook(() => useFormAnimations());
    ...
    act(() => { result.current.showFieldSuccess('email') });
...
{ result } = renderHook(() => useFormAnimations());
    ...
    act(() => { result.current.showFieldError('email', 'Error' );
      result.current.showFieldSuccess('email') });

    expect(result.current.errors.email).toBe('Error');
    expect(result.current.successFields.has('email')).toBe(true);

    act(() => { result.current.clearFieldState('email') });
...
describe('usePageTransition Hook', () => { beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers('modern') });
...
{ result } = renderHook(() => usePageTransition());
...
describe('useReducedMotion Hook', () => { beforeEach(() => {
    jest.clearAllMocks() });
...
{ result } = renderHook(() => useReducedMotion());
...
{ unmount } = renderHook(() => useReducedMotion());
