...
{ result } = renderHook(() => useMobileForm());
...
{ result } = renderHook(() => useMobileForm({ initialValues }));
...
{ result } = renderHook(() => useMobileForm({ onFieldChange }));
    
    act(() => { result.current.setFieldValue('email', 'new@example.com') });
...
{ result } = renderHook(() => useMobileForm());
    ...
    act(() => { result.current.setFieldValidation('email', false, ['Invalid email']) });
...
{ result } = renderHook(() => useMobileForm());
    ...
    act(() => { result.current.setFieldValidating('email', true) });
...
const mockEvent = {
  target: { value: 'new@example.com' 
    ...
    act(() => { fieldProps.onChange(mockEvent) });
    
    expect(result.current.formState.fields.email.value).toBe('new@example.com');
    
    // Test onBlur handler
    act(() => { fieldProps.onBlur() });
...
{ result } = renderHook(() => useMobileForm());
    ...
    act(() => { result.current.setFieldValue('email', 'invalid-email');
      result.current.setFieldValidation('email', false, ['Invalid email']);
...
{ result } = renderHook(() => useMobileForm({ initialValues }));
    ...
    // Make changes
    act(() => { result.current.setFieldValue('email', 'changed@example.com');
      result.current.setFieldTouched('email', true );
...
{ result } = renderHook(() => useMobileForm({ onSubmit }));
...
{ result } = renderHook(() => useMobileForm({ onSubmit }));
  ...
  it.skip('should prevent multiple simultaneous submissions', async () => { jest.useFakeTimers();
...
const firstSubmissionComplete = false;
    act(() => { result.current.submitForm().then(() => {
...
{ result } = renderHook(() => useMobileForm());
    ...
    // Add invalid field
    act(() => { result.current.setFieldValidation('email', false, ['Invalid']) });

    expect(result.current.isValid).toBe(false);

    // Fix the field
    act(() => { result.current.setFieldValidation('email', true, []) });

    expect(result.current.isValid).toBe(true);
  });
  });

describe('createMobileFormValidator', () => { it.skip('should validate required fields', () => {;
...
describe('commonValidationRules', () => { it.skip('should have email validation rules', () => {
  ...
  it.skip('should have password validation rules', () => { expect(commonValidationRules.password.required).toBe(true);
...
{ custom } = commonValidationRules.password;
  ...
  it.skip('should have phone validation rules', () => { expect(commonValidationRules.phone.required).toBe(false );
    expect(commonValidationRules.phone.pattern).toBeInstanceOf(RegExp) });

  it.skip('should have name validation rules', () => { expect(commonValidationRules.name.required).toBe(true);
