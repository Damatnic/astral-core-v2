...
{ result } = renderHook(() => useConnectionStatus());
    ...
    await waitFor(() => { expect(mockNavigator.serviceWorker.addEventListener).toHaveBeenCalledWith(
        'message','
        expect.any(Function)
      ) });
...
{ unmount } = renderHook(() => useConnectionStatus());
    ...
    if (mockNavigator.connection) { expect(mockNavigator.connection.removeEventListener).toHaveBeenCalledWith(
        'change','
        expect.any(Function)
      ) };
  });
