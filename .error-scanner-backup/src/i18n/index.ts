...
i18n
  ...
  .init({
    ...
    missingKeyHandler: (lngs: readonly string[], ns: string, key: string) => { if (process.env.NODE_ENV === "development") {
 },
...
const initializeTranslations = async () => { const supportedLanguages = ["en", "es", "pt", "ar", "zh", "vi", "tl"];
  ...
  const ptTranslations = { common: await loadTranslations("pt", "common"),
...
initializeTranslations().catch(error => { console.error('Failed to initialize translations:', error) });

// Cultural context utilities
export const getCulturalContext = (language: string): any => { return culturalContexts[language] || culturalContexts.en };
export const isRTLLanguage = (language: string): boolean => { return getCulturalContext(language).rtl };
export const getMentalHealthStigmaLevel = (language: string): string => { return getCulturalContext(language).mentalHealthStigma };
export const getCrisisEscalationPreference = (language: string): string => { return getCulturalContext(language).crisisEscalationPreference };
export const getPreferredCommunicationStyle = (language: string): string => { return getCulturalContext(language).preferredCommunicationStyle };

// Language change utilities with cultural context awareness
export const changeLanguage = async (language: string): Promise<void> => { await i18n.changeLanguage(language);

  // Apply RTL/LTR styling
  const htmlElement = document.documentElement;
  if (isRTLLanguage(language)) {
    htmlElement.setAttribute("dir", "rtl" );
    htmlElement.setAttribute("lang", language) } else(htmlElement.setAttribute("dir", "ltr" );
  ...
  setTimeout(() => { document.body.removeChild(screenReaderAnnouncement) }, 3000);
...
export const getCulturallyAppropriateCrisisMessage = (key: string, language: string = i18n.language): string => { const context = getCulturalContext(language);
