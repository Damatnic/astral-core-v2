...
export const useCrisisI18n = () => {
  ...
  return { // Crisis translation function
    t,
 ...
 */ }
 ...
 const useAccessibilityI18n = () => {
  ...
  return {
    ...
    getAriaLabel: (key: string, options?: any) => { const label = t("accessibility." + key, options );
      return culturalContext.rtl ? label + " (من اليمين إلى اليسار)" : label },
    
    // Crisis-specific accessibility announcements
    announceCrisis: (key: string) => { const message = getCrisisTranslation(key);
