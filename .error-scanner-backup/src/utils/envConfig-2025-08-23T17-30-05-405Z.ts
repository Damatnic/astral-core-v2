 ...
 */;
getEnvVar(key: string, defaultValue: string = ''): string { // Check if we're in a Node/Jest environment'
  ...
  if (typeof window !== 'undefined') {'
    ...
    if (globalEnv[key]) { return globalEnv[key] }return defaultValue;

/**
 * Check if we're in development mode'
 */;
export function isDev(): boolean { // Check Node environment
 ...
 */;
export function isProd(): boolean { // Check Node environment
...
if (typeof window !== 'undefined' && !window.__VITE_ENV__) { // Safe assignment for both Vite and Jest environments'
  ...
  try {
    ...
    if (importMeta?.env && typeof importMeta.env === 'object') {'
      // @ts-ignore - This will only work in Vite environment
      window.__VITE_ENV__ = importMeta.env } else {
  window.__VITE_ENV__ = {
}catch {
 ...
 */;
export const ENV = { // API Configuration
