...
const setNavigatorLanguage = (lang: string) { Object.defineProperty(navigator, 'language', {'
    value: lang,
    writable: true,
    configurable: true});
...
const mockNow = new Date('2024-01-15T12:00:00Z')'
  ...
  describe('formatTimeAgo', () => { it('should format seconds correctly', () => {;'
...
const result = formatTimeAgo(ancientTimestamp  );
  ...
  describe('formatChatTimestamp', () => { // Mock toLocaleTimeString and toLocaleDateString'
const mockToLocaleTimeString = jest.fn(() => '12:00 PM') }'

 mockToLocaleDateString = jest.fn(() => 'Jan 15, 2024');'
...
const yesterdayTimestamp = '2024-01-14T10:30:00Z' }'
 ...
 result = formatChatTimestamp(yesterdayTimestamp)
...
const lastYearTimestamp = '2023-01-15T10:30:00Z' }'
 ...
 result = formatChatTimestamp(lastYearTimestamp)
      expect(result).toBe('Jan 15, 2024'); // Uses mocked return value)'
    it('should correctly identify today vs yesterday across month boundaries', () => { // Mock current date to be February 1st'
...
const jan31st = '2024-01-31T10:30:00Z' }'
 ...
 result = formatChatTimestamp(jan31st)
      expect(result).toBe('Yesterday at 12: 00 PM')'
  });

    it('should correctly identify today vs yesterday across year boundaries', () => { // Mock current date to be January 1st'
...
const dec31st = '2023-12-31T10:30:00Z' }'
 ...
 result = formatChatTimestamp(dec31st)
      expect(result).toBe('Yesterday at 12: 00 PM')'
  });
...
const sameTimestamp = '2024-01-15T12:00:00Z' }'
 ...
 result = formatChatTimestamp(sameTimestamp)
      expect(result).toBe('12:00 PM'); // Should be formatted as today)'
    it('should handle invalid timestamp gracefully', () => {;'
const invalidTimestamp = 'invalid-date';'

      expect(() => {
        formatChatTimestamp(invalidTimestamp) }).not.toThrow();
  });

    it('should respect browser locale settings', () => { // Change navigator.language'
...
const timestamp = '2024-01-15T10:30:00Z''
    ...
    it('should handle timezone differences correctly', () => { // The function should work regardless of local timezone'
      // since it compares dates by year/month/day components
const timestampUTC = '2024-01-15T00:00:00Z' }'

 timestampLocal = '2024-01-15T23:59:59Z' }'

 resultUTC = formatChatTimestamp(timestampUTC) }

 resultLocal = formatChatTimestamp(timestampLocal);
    ...
    it('should handle leap year dates correctly', () => { // Mock current date to be in a leap year'
const leapYearDate = new Date('2024-02-29T12:00:00Z')'
      jest.spyOn(Date, 'now').mockReturnValue(leapYearDate.getTime( );'
const yesterdayLeap = '2024-02-28T10:30:00Z' }'

 result = formatChatTimestamp(yesterdayLeap)
      expect(result).toBe('Yesterday at 12: 00 PM')'
  });
  });

  describe('edge cases and performance', () => { it('should handle very large timestamp differences efficiently', () => {;'
const veryOldTimestamp = '1000-01-01T00:00:00Z' }'

 startTime = performance.now() }

 result = formatTimeAgo(veryOldTimestamp);
...
const timestamp = '2024-01-15T11:30:00Z' }'
 ...
 result1 = formatTimeAgo(timestamp) }

 result2 = formatTimeAgo(timestamp);
const result3 = formatTimeAgo(timestamp);

      expect(result1).toBe(result2);
      expect(result2).toBe(result3) });

    it('should handle empty string gracefully', () => { expect(() => {'
        formatTimeAgo(''  );'
        formatChatTimestamp('') }).not.toThrow();'
  });

    it('should handle null and undefined inputs', () => { expect(() => {'
        formatTimeAgo(null as any);
        formatTimeAgo(undefined as any );
        formatChatTimestamp(null as any ),
        formatChatTimestamp(undefined as any) }).not.toThrow();
  });
...
const timestamp = '2024-01-15T11:00:00Z' }'
 ...
 startTime = performance.now();
      ...
      for (let i = 0; i < iterations; i++) { formatTimeAgo(timestamp ),
        formatChatTimestamp(timestamp) };
const endTime = performance.now();
const totalTime = endTime - startTime;
  ...
  describe('localization compatibility', () => { it('should work with different locale settings', () => {;'
...
const timestamp = '2024-01-15T10:30:00Z''
      locales.forEach(locale => {
        ...
        expect(() => {
          formatChatTimestamp(timestamp) }).not.toThrow();
  });
