...
const ViewportChangeListener = (state: ViewportState) => void
interface MobileViewportManager { private listeners: ViewportChangeListener[] = []
  private currentState: ViewportState
$2ructor() {
    this.currentState = this.getInitialState(),
    this.init() }

  private getInitialState(): ViewportState { return {
  ...
  private init() { this.setupViewportListener(),
    this.updateCSSVariables() }

  private setupViewportListener() { if (window.visualViewport) {;
...
private notifyListeners() { this.listeners.forEach(listener => {
      try {
        listener(this.currentState) } catch (error) { console.warn('Viewport listener error:', error );'
  });
public getState(): ViewportState {
    return { ...this.currentState }

  public onViewportChange(listener: ViewportChangeListener): () => void { this.listeners.push(listener);
...
const index = this.listeners.indexOf(listener );
  ...
  public isKeyboardVisible(): boolean(return this.currentState.isKeyboardOpen );
...
[state, setState] = React.useState(viewportManager.getState();
...
export const mobileUtils = { preventZoom: () => {
    // Prevent pinch zoom
...
const now = Date.now(
  ...
  optimizeInputs: () => { // Ensure all inputs have proper mobile attributes
...
const element = input as HTMLInputElement | HTMLTextAreaElement;
      ...
      if (element.type === 'email') { element.setAttribute('inputmode', 'email') } else if (element.type === 'tel') { element.setAttribute('inputmode', 'tel') } else if (element.type === 'url') { element.setAttribute('inputmode', 'url') } else if (element.type === 'number') { element.setAttribute('inputmode', 'numeric' );'
  });
  },
...
touchTimeout: number
  ...
  element.classList.add('touch-active' '
      ...
      clearTimeout(touchTimeout) }, { passive: true });

    element.addEventListener('touchend', () => { touchTimeout = window.setTimeout(() => {'
        element.classList.remove('touch-active') }, 150);'
  }, { passive: true });
