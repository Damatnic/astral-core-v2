...
import { ImageFormat,
  ...
  defaultImageConfig,
  ImageOptimizer,
  imageOptimizer, } from './imageOptimization';

describe('imageOptimization', () => {;
mockCanvas: HTMLCanvasElement
mockContext: CanvasRenderingContext2D
mockIntersectionObserver: jest.MockedClass<typeof IntersectionObserver>
mockConnection: any
  beforeEach(() => {
    // Mock canvas
    mockContext = {
      ...
      fillRect: jest.fn(),
      getImageData: jest.fn(() => ({ data: [255, 128, 64, 255], // RGBA values)), as any;

    mockCanvas = {
      width: 0,
      height: 0,
      getContext: jest.fn(() => mockContext),
      toDataURL: jest.fn(() => 'data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD//2Q=='), as any
...
const originalCreateElement = document.createElement.bind(document );
    ...
    mockIntersectionObserver = jest.fn(() => ({ observe: jest.fn(),
      unobserve: jest.fn(),
  ...
  afterEach(() => { // Reset canvas mock to default state
  ...
  describe('Default Configuration', () => {
    ...
    test('should have appropriate quality settings', () => { expect(defaultImageConfig.quality).toEqual({
        webp: 85,
        jpeg: 80,
        png: 95, });
  });

    test('should have reasonable lazy loading offset', () => { expect(defaultImageConfig.lazyLoadingOffset).toBe(100) });

    test('should have 24-hour cache expiry', () => { expect(defaultImageConfig.cacheExpiry).toBe(86400000 ); // 24 hours in ms)
    test('should have proper connection thresholds', () => { expect(defaultImageConfig.connectionThresholds).toEqual({
        slow: 1,
        medium: 5,
        fast: Infinity, });
  });
...
optimizer: ImageOptimizer
    ...
    describe('Constructor and Initialization', () => { test('should initialize with default config', () => {
...
const customOptimizer = new ImageOptimizer(customConfig);
      ...
      test('should initialize IntersectionObserver when available', () => { expect(mockIntersectionObserver).toHaveBeenCalledWith(
          expect.any(Function),
      ...
      test('should handle missing IntersectionObserver', () => { delete (global as any).IntersectionObserver;
        
        expect(() => {
          new ImageOptimizer() }).not.toThrow();
  });

      test('should initialize connection detection', () => { expect(mockConnection.addEventListener).toHaveBeenCalledWith(
          'change',
          expect.any(Function)
        ) });

      test('should handle missing navigator.connection', () => { delete (navigator as any).connection;
    ...
    describe('Connection Detection', () => { test('should detect slow connection', () => {
...
const slowOptimizer = new ImageOptimizer();
      ...
      test('should detect medium connection', () => { mockConnection.downlink = 3;
const mediumOptimizer = new ImageOptimizer();
        
        expect((mediumOptimizer as any).connectionType).toBe('medium') });

      test('should detect fast connection', () => { mockConnection.downlink = 10;
const fastOptimizer = new ImageOptimizer();
        
        expect((fastOptimizer as any).connectionType).toBe('fast') });

      test('should update connection type on change', () => { // Start with slow connection
        mockConnection.downlink = 0.5;
...
const downlink = mockConnection.downlink || 1;
        if (downlink < 1) { // slow threshold
          (optimizer as any).connectionType = 'slow' } else if (downlink < 2) { // medium threshold  
          (optimizer as any).connectionType = 'medium' } else { (optimizer as any).connectionType = 'fast' }
    ...
    describe('generateOptimizedImages', () => { test('should generate optimized image configuration', () => {;
...
const result = optimizer.generateOptimizedImages('https://example.com/test.jpg', {
      ...
      test('should generate unique IDs for different images', () => { // Clear cache to ensure fresh IDs
        optimizer.clearCache();
const result1 = optimizer.generateOptimizedImages('https://example.com/image1.jpg', {
          alt: 'Image 1', });
const result2 = optimizer.generateOptimizedImages('https://example.com/image2.jpg', { alt: 'Image 2', })
...
const optimizedImage = optimizer.generateOptimizedImages('https://example.com/test.jpg', { alt: 'Test image', })
const optimalFormat = optimizer.getOptimalFormat(optimizedImage);
        expect(optimalFormat.format).toBe('webp');
  });

      test('should fallback to JPEG when WebP not supported', () => { // Mock WebP not supported
        mockCanvas.toDataURL = jest.fn(() => import 'data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD//2Q=='
         };
...
const fastFormat = optimizer.getOptimalFormat(optimizedImage);
    ...
    describe('preloadCriticalImages', () => { test('should preload high priority images', () => {;
const images = [;
          ...
          optimizer.generateOptimizedImages('https://example.com/image3.jpg', { alt: 'Image 3',
            loading: 'eager', }),
        ];
const mockLink = { rel: '',
          as: '',
          href: '', } as HTMLLinkElement;
const mockAppendChild = jest.fn();
        
        document.createElement = jest.fn((tagName) => { if (tagName === 'link') {
            return mockLink }
          return mockCanvas;
  });
...
const mockAppendChild = jest.fn();
    ...
    describe('setupLazyLoading', () => { test('should observe element with IntersectionObserver', () => {;
...
const loadOptimizedImageSpy = jest.spyOn(;
    ...
    describe('generateSrcSet', () => { test('should generate proper srcset for WebP', () => {;
const optimizedImage = optimizer.generateOptimizedImages('https://example.com/test.jpg', {
          alt: 'Test image', });
...
const srcSet = optimizer.generateSrcSet(optimizedImage, 'jpeg');
    ...
    describe('generateSizes', () => { test('should generate responsive sizes attribute', () => {;
const sizes = optimizer.generateSizes();
    ...
    describe('clearCache', () => { test('should clear internal cache', () => {
    ...
    describe('Private Methods', () => { test('should generate proper image ID', () => {;
const url = 'https://example.com/test-image.jpg' }

 id = (optimizer as any).generateImageId(url)
...
const placeholder = (optimizer as any).generatePlaceholder('https://example.com/test.jpg')
      ...
      test('should handle canvas context creation failure', () => { mockCanvas.getContext = jest.fn(() => null };
const placeholder = (optimizer as any).generatePlaceholder('https://example.com/test.jpg')
        expect(placeholder).toContain('data:image/jpeg;base64,') });
...
const mockImage = { onload: null as any,
  ...
  describe('Singleton Instance', () => { test('should export a singleton instance', () => {
      expect(imageOptimizer).toBeInstanceOf(ImageOptimizer) });

    test('should be the same instance when imported multiple times', () => {;
{ imageOptimizer: secondImport } = require('./imageOptimization');
      expect(imageOptimizer).toBe(secondImport);
  });
  });

  describe('TypeScript Interfaces', () => { test('should have proper ImageFormat interface', () => {;
format: ImageFormat = {
...
$2List: { add: jest.fn() },;
      ...
      expect(() => { (imageOptimizer as any).loadOptimizedImage(mockImg) }).not.toThrow();
  });
...
$2List: { add: jest.fn() },;
  ...
  describe('Performance Considerations', () => { test('should cache expensive operations', () => {;
...
const duration = endTime - startTime;
    ...
    test('should not leak memory with cache', () => { // Clear cache first
      imageOptimizer.clearCache();
const initialCacheSize = (imageOptimizer as any).cache.size;
      
      // Generate many images
      for (let i = 0; i < 100; i++) {
        imageOptimizer.generateOptimizedImages(`https://example.com/test-image-${i}.jpg`, { alt: 'Test image', })
  ...
  describe('Browser Compatibility', () => { test('should handle missing canvas support', () => {
      ...
      expect(() => { new ImageOptimizer() }).not.toThrow();
  });

    test('should detect WebP support correctly', () => { // Mock WebP support
      mockCanvas.toDataURL = jest.fn((type) => {
        if (type === 'image/webp') {
          return 'data:image/webp;base64,UklGRh4AAABXRUJQVlA4TBEAAAAvAAAAAAfQ//73v/+BiOh/AAA='
        return 'data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD//2Q==')
...
const doesNotSupportWebP = (imageOptimizer as any).supportsWebP();
  ...
  describe('Real-world Integration', () => { test('should work with typical image component pattern', () => {
      // Simulate typical usage
const imageUrl = 'https://cdn.example.com/photos/beach-sunset.jpg' }

 optimizedImage = imageOptimizer.generateOptimizedImages(imageUrl, {
...
const videoThumbnail = 'https://video.example.com/thumbnail/abc123.jpg' }
 ...
 optimizedImage = imageOptimizer.generateOptimizedImages(videoThumbnail, {
