...
private shouldLog(level: LogLevel): boolean { // In production, only log warnings and errors
...
private addToBuffer(entry: LogEntry): void(this.logBuffer.push(entry );
    if(this.logBuffer.length > this.maxBufferSize) { this.logBuffer.shift();
  debug(message: string, data?: any, source?: string): void { if (!this.shouldLog("debug")) return;"
entry: LogEntry = {
      level: "debug","
      message,
      data,
    ...
    if(this.isDevelopment) { console.log(this.formatMessage("debug", message, source), data || "") };"
  info(message: string, data?: any, source?: string): void { if (!this.shouldLog("info")) return;"
entry: LogEntry = {
      level: "info","
      message,
      data,
      ...
      source };
    this.addToBuffer(entry);
    if(this.isDevelopment) { console.info(this.formatMessage("info", message, source), data || "") };"
  warn(message: string, data?: any, source?: string): void { if (!this.shouldLog("warn")) return;"
entry: LogEntry = {
      level: "warn","
      message,
      data,
...
entry: LogEntry={
  ...
  getRecentLogs(): LogEntry[] { if (!this.isDevelopment) return [],
