...
type UserRole = 'Starkeeper' | 'Community' | 'Certified' | 'Moderator' | 'Admin';
interface RolePermissions { role: UserRole
  allowedViews: View[]
  deniedViews: View[]
  features: {
    canModerate: boolean
...
// Define comprehensive role permissions }
 ...
 const ROLE_PERMISSIONS: Record<UserRole, RolePermissions> = {
  Starkeeper: {
    role: 'Starkeeper',
    allowedViews: [ 'starkeeper-dashboard',
      'share',
      'feed',
      'crisis',
      'ai-chat',
    ...
    deniedViews: [ 'constellation-guide-dashboard',
      'helper-profile',
      'helper-training',
      'helper-community',
      'helper-application',
  ...
  Community: {
    role: 'Community',
    allowedViews: [ 'constellation-guide-dashboard',
      'helper-profile',
      'helper-training',
      'helper-community',
      'helper-application',
      ...
      'settings',
      'guidelines',
      'legal',import 'workflow-demo' ],
    deniedViews: [ 'starkeeper-dashboard',
...
export const canAccessView = (userRole: UserRole | undefined, view: View): boolean => { if (!userRole) {
...
export const isViewDenied = (userRole: UserRole | undefined, view: View): boolean => { if (!userRole) {
    // Unauthenticated users get Starkeeper permissions
    return ROLE_PERMISSIONS.Starkeeper.deniedViews.includes(view);
const permissions = getRolePermissions(userRole);
  return permissions.deniedViews.includes(view);
  };
export const getUserRole = (helper: Helper | null): UserRole => { if (!helper || !helper.role) {
    return 'Starkeeper' }
