...
async createMoodEntry(data: { mood_value: number
    mood_label: string
    notes?: string
    activities?: string[]
...
async updateMoodEntry(id: string, data: Partial<MoodEntry>): Promise<MoodEntry> {
    return this.request<MoodEntry>(`api-mood/${id}`, { method: 'PUT','
      body: JSON.stringify(data)});
...
async submitWellnessAssessment(data: { assessment_type: string
