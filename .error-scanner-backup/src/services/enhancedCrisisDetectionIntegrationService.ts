...
interface EnhancedCrisisDetectionIntegrationService { private readonly analysisVersion = '2.0.0';
...
const consolidatedResult = await this.consolidateAnalysisResults(;
  ...
  } catch (error) { console.error('[Enhanced Crisis Detection Integration] Analysis failed:', error );
...
const escalationWorkflow = undefined;
    ...
    if (escalationRequired || emergencyServicesRequired) { escalationWorkflow = await this.handleCrisisEscalationWorkflow(
        consolidatedRisk,
        overallSeverity,
        keywordAnalysis,
        aiAnalysis,
