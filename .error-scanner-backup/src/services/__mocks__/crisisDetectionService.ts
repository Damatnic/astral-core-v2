...
defaultResult: CrisisDetectionResult = {
...
const lowerText = text.toLowerCase();
  ...
  if (lowerText.includes('used to') || 
      ...
      lowerText.includes('past')) { return defaultResult }
  
  // Casual mentions - no crisis
  if (lowerText.includes('movie') || 
      lowerText.includes('book') || 
      lowerText.includes('news')) { return defaultResult }
...
export const astralCoreCrisisDetection = {
  ...
  getCopingStrategies: jest.fn(() => Promise.resolve([ 'Deep breathing exercises',
    'Grounding techniques',
    'Call a trusted friend',import 'Use safety plan' ])),
  triggerEmergencyProtocol: jest.fn(() => Promise.resolve({ notified: ['emergency_contact', 'crisis_team'],
    actions: ['location_shared', 'resources_sent'] })),
  supportedLanguages: ['en', 'es', 'fr', 'zh', 'ar', 'hi'],
  validateCrisisIndicators: jest.fn(() => true),
  assessRiskLevel: jest.fn((factors: string[]) => { if (factors.includes('suicide_ideation')) return 'critical';
