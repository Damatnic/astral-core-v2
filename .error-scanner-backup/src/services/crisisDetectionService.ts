...
$2: 'immediate' | 'urgent' | 'monitor' | 'support'
  description: string
  contacts: string[]
  resources: string[]
  timeline: string
  };
interface EnhancedCrisisDetectionService {
  ...
  private protectiveFactorsList = { ambivalence: ['but', 'except', 'although', 'however', 'maybe', 'not sure', 'part of me'],
...
triggeredKeywords: CrisisIndicator[] = []
detectedCategories = new Set<string>()

 maxSeverity = 'none';
const escalationRequired = false;
const emergencyServices = false;
const urgencyLevel = 0;

    // Analyze each crisis indicator
    for (const indicator of this.criticalKeywords) { if (this.matchesIndicator(normalizedText, indicator)) {
        ...
        if (indicator.immediateAction) { escalationRequired = true;
...
const identifiedProtectiveFactors = this.identifyProtectiveFactors(normalizedText);
    ...
    if (urgencyLevel > 4 && maxSeverity !== 'none') { escalationRequired = true;
      if (urgencyLevel > 7) {
        maxSeverity = this.escalateSeverity(maxSeverity);

    // Check if this is a past recovery story - should not flag as crisis
    if (identifiedProtectiveFactors.includes('past_recovery')) { // Past recovery stories should not trigger crisis unless there's new immediate danger
...
const protectiveScore = this.calculateProtectiveFactorScore(identifiedProtectiveFactors);
    if (protectiveScore > 0 && maxSeverity !== 'critical' && !emergencyServices) { // Reduce urgency based on protective factors
...
const originalSeverity = maxSeverity;
      ...
      if (protectiveScore > 0.5 && escalationRequired && maxSeverity !== 'high') { escalationRequired = false };

    // Generate recommended actions (now accounting for protective factors)
const recommendedActions = this.generateRecommendedActions({ severityLevel: maxSeverity,
 ...
 confidence = this.calculateConfidence({
    ...
    return { hasCrisisIndicators: triggeredKeywords.length > 0,
   ...
   */
  private matchesIndicator(text: string, indicator: CrisisIndicator): boolean(// Handle special characters and variations in the keyword
...
const keywordFound = false;
const keywordIndex = -1;
    
    for (const variation of keywordVariations) { if (normalizedText.includes(variation)) {
        keywordFound = true;
    ...
    if (indicator.context.length === 0) { return true }
...
const urgencyWords = this.urgencyModifiers.filter(modifier => text.includes(modifier));
    ...
    if (indicator.immediateAction) { urgency += 3 }
...
const sentiment = 0;
    ...
    negativeWords.forEach(word => { if (text.includes(word)) sentiment -= 1 });
...
factors: string[] = []
    ...
    // Time-based urgency
    if (/\b(tonight|today|now|soon)\b/.test(text)) { factors.push('immediate_timeframe') }
    
    // Method references
    if (/\b(pills|knife|gun|rope|bridge|jump)\b/.test(text)) { factors.push('method_reference') }
    
    // Support system
    if (/\b(family|friends|therapist|doctor|help)\b/.test(text)) { factors.push('support_available') }
...
riskFactors: string[] = []
    ...
    if (/\b(plan|method|when|how|where)\b/.test(text)) { riskFactors.push('specific_planning') }
    
    if (/\b(alone|isolated|no one knows)\b/.test(text)) { riskFactors.push('isolation') }
    
    if (/\b(failed|worthless|burden|waste)\b/.test(text)) { riskFactors.push('negative_self_perception') }
...
factors: string[] = [] }
 ...
 pastTensePatterns = [;
    ...
    for (const [factorName, keywords] of Object.entries(this.protectiveFactorsList)) { for (const keyword of keywords) {
    ...
    if (/would\s+never\s+actually|never\s+actually/.test(normalizedText)) { if (!factors.includes('ambivalence')) {
   ...
   */
  private calculateProtectiveFactorScore(protectiveFactors: string[]): number { if (protectiveFactors.length === 0) return 0;
...
weights: Record<string, number> = {
 ...
 totalScore = 0;
    for (const factor of protectiveFactors) { totalScore += weights[factor] || 0.05 }
...
actions: string[] = []
    ...
    if (params.escalationRequired) { actions.push('Escalate to crisis counselor immediately');
      actions.push('Initiate suicide risk assessment' );
      actions.push('Contact designated emergency contacts') }

    if (params.categories.includes('suicidal')) { actions.push('Provide suicide prevention resources');
      actions.push('Share safety planning tools' );
      actions.push('Connect with crisis hotline') }

    if (params.categories.includes('self-harm')) { actions.push('Offer self-harm alternatives');
      actions.push('Provide coping strategies' );
      actions.push('Connect with self-harm support resources') }

    if (params.severityLevel === 'medium' || params.severityLevel === 'high') { actions.push('Schedule follow-up check-in');
    ...
    if (params.protectiveFactors && params.protectiveFactors.length > 0) { if (params.protectiveFactors.includes('professional_support')) {
        actions.push('Encourage continued engagement with current treatment') }
      if (params.protectiveFactors.includes('help_seeking')) { actions.push('Facilitate connection to requested support resources') }
      if (params.protectiveFactors.includes('life_responsibilities')) { actions.push('Reinforce importance of responsibilities and connections') }
      if (params.protectiveFactors.includes('coping_strategies')) { actions.push('Support and expand existing coping mechanisms') }
      if (params.protectiveFactors.includes('social_connection')) { actions.push('Encourage reaching out to support network');
...
const severityWeights = { 'critical': 30, 'high': 20, 'medium': 15, 'low': 10, 'none': 0 };
    ...
    // Sentiment analysis
    if (params.sentimentScore < -2) { confidence += 10 }
    
    // Protective factors increase confidence in our assessment
    if (params.protectiveFactors && params.protectiveFactors > 0) { confidence += Math.min(params.protectiveFactors * 2, 10) }
...
actions: CrisisEscalationAction[] = []
    ...
    if (analysisResult.emergencyServices) {
      return {
        ...
        resources: [
          ...
          'Emergency Services: 911',import 'Crisis Chat: suicidepreventionlifeline.org/chat'
        ],
        followUp: [ 'Emergency services contacted',
          'Safety assessment completed',
          'Crisis intervention activated',import 'Professional follow-up scheduled' ] }

    if (analysisResult.escalationRequired) { return {
        message: isSeeker
        ...
        actions: [ 'Contact crisis hotline: 988',
          'Reach out to a mental health professional',
          'Connect with crisis chat support',import 'Contact trusted friend or family member' ],
        resources: [ '988 Suicide & Crisis Lifeline',
          'Crisis Text Line: 741741',
          'Local crisis services',import 'Mental health emergency services' ],
        followUp: [ 'Crisis counselor consultation',
          'Safety plan development',
          'Regular check-ins scheduled',import 'Professional referral provided' ] }

    // Medium/High severity response
    return { message: isSeeker
      ...
      actions: [ 'Talk to someone you trust',
        'Consider contacting a counselor or therapist',
        'Use coping strategies from your safety plan',import 'Reach out to support networks' ],
      resources: [ 'Mental health professionals',
        'Support groups',
        'Crisis resources',import 'Peer support networks' ],
      followUp: [ 'Schedule regular check-ins',
        'Review and update safety plan',
        'Connect with ongoing support',import 'Monitor mood and symptoms' ] };

// Create singleton instance
AstralCoreCrisisDetectionService extends EnhancedCrisisDetectionService {}
...
interface type(CrisisAnalysisResult, CrisisIndicator, CrisisEscalationAction );
