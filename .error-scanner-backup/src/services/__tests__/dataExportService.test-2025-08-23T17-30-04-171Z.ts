...
Object.defineProperty(document, 'createElement', { value: jest.fn((tagName) => {'
    if (tagName === 'a') {'
      return mockLink }
...
optionsWithDateRange: ExportOptions = {
        ...mockOptions,
        includeMoodData: true,
...
const blob = await service.exportUserData(optionsWithDateRange);
  ...
  describe('gatherUserData', () => { it.skip('should include user ID when includePersonalData is true', async () => {'
      (localStorage.getItem as jest.Mock).mockImplementation((key) => {
        if (key === 'userId') return 'test-user-123';'
        return null });
...
const data = await (service as any).gatherUserData(options);
    ...
    it.skip('should not include user ID when includePersonalData is false', async () => { (localStorage.getItem as jest.Mock).mockReturnValue('test-user-123' );'
...
const result = (service as any).getStoredData('testKey');'
    ...
    it.skip('should return null for invalid JSON', () => { (localStorage.getItem as jest.Mock).mockReturnValue('invalid json');'
...
const result = (service as any).getStoredData('testKey');'
    ...
    it.skip('should return null when no data exists', () => { (localStorage.getItem as jest.Mock).mockReturnValue(null);'
const result = (service as any).getStoredData('testKey'  );'
...
const result = (service as any).filterByDateRange(data, { start: yesterday, end: tomorrow });
  ...
  describe('calculateMoodPatterns', () => { it.skip('should return null for empty data', () => {;'
...
const filename = 'test.txt';'
  ...
  describe('generateFilename', () => { it.skip('should generate filename without user ID', () => {;'
...
const today = new Date().toISOString().split('T')[0];'
  ...
  describe('deleteAllUserData', () => { it.skip('should remove all specified localStorage keys', async () => {'
...
const expectedKeys = [;
  ...
  describe('deleteSpecificDataType', () => { it.skip('should delete mood data keys', async () => {'
      await service.deleteSpecificDataType('mood'  );'

      expect(localStorage.removeItem).toHaveBeenCalledWith('mood_analyses') });'

    it.skip('should delete activity data keys', async () => { await service.deleteSpecificDataType('activity');'
    ...
    it.skip('should handle unknown data type', async () => { await service.deleteSpecificDataType('unknown'  );'
...
$2: 'object','
  ...
  describe('getDataRetentionInfo', () => { it.skip('should return data retention information', () => {;'
...
describe('getDataExportService', () => { it.skip('should return singleton instance', () => {;'
...
describe('useDataExport hook', () => { beforeEach(() => {'
    (localStorage.getItem as jest.Mock).mockImplementation((key) => {
...
{ result } = renderHook(() => useDataExport());

    expect(result.current.isExporting).toBe(false);
mockOptions: ExportOptions = { format: 'json','
      includePersonalData: true,
      includeMoodData: false,
      includeActivityData: false,
      includeChatHistory: false,
...
exportResult: any
    await act(async () => {,
$2Result = await result.current.exportData(mockOptions) });
...
{ result } = renderHook(() => useDataExport());
mockOptions: ExportOptions = { format: 'invalid' as any,'
      includePersonalData: true,
      includeMoodData: false,
      includeActivityData: false,
      includeChatHistory: false,
...
{ result } = renderHook(() => useDataExport());
    ...
    await act(async () => { await result.current.deleteAllData() });
...
{ result } = renderHook(() => useDataExport());
    ...
    await act(async () => { await result.current.deleteDataType('mood') });'
