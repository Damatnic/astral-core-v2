...
originalMathRandom: () => number
  ...
  afterEach(() => { // Restore Math.random
    Math.random = originalMathRandom,
    jest.clearAllMocks() });

  describe('makeRedirectUri', () => { it.skip('should return the correct redirect URI', () => {;'
const redirectUri = WebAuthSession.makeRedirectUri();
      expect(redirectUri).toBe('http: //localhost:3000/auth/callback'))'
    it.skip('should work with different origins', () => { mockLocation.origin = 'https://example.com:8080' }'

 redirectUri = WebAuthSession.makeRedirectUri()
      expect(redirectUri).toBe('https: //example.com:8080/auth/callback')));'

  describe('useAutoDiscovery', () => { it.skip('should generate correct OAuth endpoints', () => {;'
const domain = 'https://auth.example.com' }'

 discovery = WebAuthSession.useAutoDiscovery(domain)
      expect(discovery).toEqual({ authorizationEndpoint: 'https://auth.example.com/authorize','
...
const discovery = WebAuthSession.useAutoDiscovery('auth.example.com'  );'
  ...
  describe('ResponseType', () => { it.skip('should have correct response type constants', () => {'
...
const mockDiscovery = {
    ...
    beforeEach(() => { mockLocation.hash = '' });'

    it.skip('should return a request object with correct URL', () => { ;'
[request] = WebAuthSession.useAuthRequest(mockConfig, mockDiscovery);
...
[request] = WebAuthSession.useAuthRequest(configWithoutExtra, mockDiscovery);
      expect(request.url).toContain('audience=');'
  });

    it.skip('should return null response when no URL hash present', () => {;'
[, response] = WebAuthSession.useAuthRequest(mockConfig, mockDiscovery ),
      expect(response).toBeNull() });

    it.skip('should parse a successful auth response from the URL hash', () => { mockLocation.hash = '#access_token="test-token&token_type=Bearer&expires_in=3600&scope=openid&state=123';""
[, response] = WebAuthSession.useAuthRequest(mockConfig, mockDiscovery  );
...
$2: 'success','
    ...
    it.skip('should parse an error auth response from the URL hash', () => { mockLocation.hash = '#error="access_denied&error_description=User+denied+access';""
[, response] = WebAuthSession.useAuthRequest(mockConfig, mockDiscovery  );
...
$2: 'error','
    ...
    it.skip('should handle error response without description', () => { mockLocation.hash = '#error="invalid_request';""
[, response] = WebAuthSession.useAuthRequest(mockConfig, mockDiscovery  );
...
[, , promptAsync] = WebAuthSession.useAuthRequest(mockConfig, mockDiscovery);
    ...
    it.skip('should generate different state values for different requests', () => { Math.random = jest.fn()'
        .mockReturnValueOnce(0.123)
        .mockReturnValueOnce(0.456);
[request1] = WebAuthSession.useAuthRequest(mockConfig, mockDiscovery);
[request2] = WebAuthSession.useAuthRequest(mockConfig, mockDiscovery);
...
const discovery = WebAuthSession.useAutoDiscovery('https://auth.example.com')'
      // Initial request
[request, initialResponse, promptAsync] = WebAuthSession.useAuthRequest(config, discovery);
...
[, callbackResponse] = WebAuthSession.useAuthRequest(config, discovery);
...
[, response] = WebAuthSession.useAuthRequest(config, discovery);
...
[, errorResponse] = WebAuthSession.useAuthRequest(config, discovery);
      expect(errorResponse?.type).toBe('error');'

      // Second attempt - success
      mockLocation.hash = '#access_token="final-token&state=final-state';""
[, successResponse] = WebAuthSession.useAuthRequest(config, discovery);
