...
interface CrisisEscalationRequest {
  ...
  metadata: { detectionMethod: string
...
interface EscalationResponse { escalationId: string
  tier: EscalationTier
...
$2ructor() {
 ...
 escalationRequest = await this.createEscalationRequest(
        crisisAnalysis,
        userId,
        userContext,
        sessionData,
 ...
 escalationResponse = await this.executeEscalationWorkflow(
        escalationRequest,
        selectedTier
      );
      ...
      return escalationResponse } catch (error) { console.error('[Crisis Escalation] Failed to initiate escalation:', error );
...
const urgencyLevel = this.mapUrgencyLevel(crisisAnalysis?.overallSeverity || 'emergency' );
  ...
  private async selectEscalationTier(request: CrisisEscalationRequest): Promise<EscalationTierConfig> { // Select the most appropriate tier based on risk level and emergency requirements
