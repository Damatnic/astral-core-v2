 ...
 */;
interface ModerationResult { safe: boolean
  reason?: string
...
interface AIModerationService { private safetyFilters: SafetyFilter[] = [
    // Harmful content
    {
  ...
  private contextualFactors = { // Factors that might indicate genuine distress vs harmful intent
    distressIndicators: [
...
const normalizedMessage = message.toLowerCase().trim();
    ...
    for (const filter of this.safetyFilters) { if (filter.pattern.test(normalizedMessage)) {
...
const uniqueRatio = uniqueWords.size / words.length;
   ...
   */
  public generateSafeResponse(moderationResult: ModerationResult): string { if (moderationResult.suggestedResponse) {
   ...
   */
  public needsHumanIntervention(messages: Array<{ text: string; sender: 'user' | 'ai' }>): boolean { // Check recent user messages for crisis indicators'
const recentUserMessages = messages;
      .filter(m => m.sender === 'user')'
      .slice(-5); // Last 5 user messages
const crisisCount = 0;
const harmfulCount = 0;
...
const moderation = this.moderateMessage(message.text  );
   ...
   */
  public sanitizeForDisplay(text: string): string { // Remove potential XSS attempts
    return text
