...
const response = await fetch(url, { cache: 'reload' });'
        if (response.ok) { await crisisCache.put(url, response)} catch (error) {
...
const strategy = new NetworkFirst({
  ...
  } catch (error) { console.error('[SW] API request failed:', error);'
...
registerRoute(
  ...
  new CacheFirst({ cacheName: CACHE_NAMES.CRISIS,
    plugins: [
...
registerRoute(
  ...
  new StaleWhileRevalidate({ cacheName: CACHE_NAMES.COMMUNITY,
    plugins: [
      new ExpirationPlugin({
        maxEntries: 50,
